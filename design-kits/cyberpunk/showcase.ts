...
import { CYBERPUNK_COMPONENTS } from './components';
...
/**
 * `showcase.html` kit Cyberpunk: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const SWATCHES = [
    ['primary', '#00F0FF'], ['primary-hover', '#6BF8FF'], ['secondary', '#FF2EC8'],
    ['accent', '#F9F002'], ['background', '#060913'], ['surface-2', '#141C36'],
    ['text', '#E6F1FF'], ['text-muted', '#7E8DB3'], ['border', '#24315E'],
    ['success', '#00FFA3'], ['warning', '#FFB300'], ['danger', '#FF3D5E'], ['info', '#4DA6FF']
].map(([name, hex]) =>
...
const TYPE_SCALE = [
    ...
    ['Body', 'tk-body', '17px'], ['Body Small', 'tk-body-sm', '15px'], ['Caption', 'tk-caption', '13px']
].map(([label, cls, size]) =>
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Kota tak pernah tidur</p></div>`
).join('');
...
const ICONS = ['house', 'magnifying-glass', 'user', 'gear', 'bell', 'heart', 'star', 'trash', 'pencil-simple',
    'check', 'x', 'plus', 'arrow-right', 'download-simple', 'upload-simple', 'calendar-blank', 'envelope-simple', 'lock']
    .map(name => `<span class="sc-icon" title="ph:${name}"><iconify-icon icon="ph:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const CYBERPUNK_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Cyberpunk — Design Kit Showcase</title>
...
<style>
    ...
    .sc-heading { font-family: var(--tk-font-mono); font-size: 12px; font-weight: 500; letter-spacing: 0.14em; text-transform: uppercase; color: var(--tk-color-primary); border-bottom: 1px solid var(--tk-color-border); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-space-bar { height: 16px; background: rgba(0, 240, 255, 0.15); border: 1px solid var(--tk-color-primary); border-radius: 2px; }
    ...
    .sc-glitch-demo { font-family: var(--tk-font-heading); font-weight: 900; font-size: 34px; text-transform: uppercase; letter-spacing: 0.04em; margin: 0; text-shadow: 2px 0 rgba(255, 46, 200, 0.55), -2px 0 rgba(0, 240, 255, 0.55); }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Cyberpunk', 'components', '', { themeToggle: false })}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Cyberpunk</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">High-tech dystopian: latar malam ber-grid dengan cahaya kota neon cyan-magenta, headline glitch beraberasi kromatik, garis holografik, tombol chamfer ala HUD, ikon heksagon, dan scanline CRT tipis. Kit satu-tema &mdash; gelap secara bawaan, tanpa mode terang. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Orbitron (heading geometris sci-fi) &middot; Rajdhani (body condensed techno) &middot; IBM Plex Mono (caption &amp; readout) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<div class="sc-row">${RADII}</div><p class="tk-caption" style="margin-top: 12px">Sudut kecil dan tajam; tombol utama memakai chamfer clip-path, bukan radius.</p>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>ph</code> (Phosphor) &mdash; tulis <code>&lt;iconify-icon icon="ph:nama"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-row"><span class="tk-badge">Neon</span><span class="tk-badge">Glitch</span><span class="tk-badge">Holographic</span><span class="tk-badge">High-tech dystopian</span></div><p class="sc-glitch-demo tk-glitch" style="margin: 20px 0 0">Night City</p><p class="tk-h3 tk-holo-text" style="margin: 8px 0 0">Hologram aktif &mdash; sinyal stabil</p><p class="tk-body-sm tk-muted" style="margin: 16px 0; max-width: 560px">Malam abadi kota megakorporasi: latar biru-hitam ber-grid dengan cahaya neon cyan dan magenta, headline Orbitron beraberasi kromatik yang sesekali tergelincir (<code class="tk-code">.tk-glitch</code>), teks gradien holografik (<code class="tk-code">.tk-holo-text</code>), tombol chamfer terpangkas ala HUD dengan glow neon, panel ber-garis holo di tepi atas, ikon heksagon sel data, readout terminal <code class="tk-code">.tk-readout</code>, dan scanline CRT tipis di seluruh halaman. Tidak ada mode terang &mdash; dystopia tidak mengenal siang.</p><div class="sc-row" style="align-items: center; gap: 10px"><span class="tk-readout">NET//ONLINE</span><span class="tk-readout">SYS 99.9%</span></div>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.primary, backgroundColor: t.primary + '22', fill: true, tension: 0.3, pointRadius: 3, pointBackgroundColor: t.primary }] },
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: '#FF2EC8', borderRadius: 2 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
