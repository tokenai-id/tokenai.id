...
/**
 * Halaman contoh (`examples/`) kit Cyberpunk: admin dashboard dan landing page utuh yang
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="ph:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Cyberpunk — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 900; font-size: var(--tk-text-title); letter-spacing: 0.08em; padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-foot-btn:hover { background: rgba(0, 240, 255, 0.06); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(12, 18, 34, 0.82); backdrop-filter: blur(10px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 32px; height: 32px; clip-path: polygon(50% 0, 100% 25%, 100% 75%, 50% 100%, 0 75%, 0 25%); background: rgba(255, 46, 200, 0.16); color: var(--tk-color-secondary); display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-weight: 700; font-size: 10px; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Cyberpunk', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('squares-four')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('lifebuoy')} <span class="tk-sidebar-label">Bantuan</span></a>
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('caret-up-down', 16)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user', 16)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('gear', 16)} Pengaturan</button>
    ...
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('caret-right', 14)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('caret-right', 14)}</span>
            ...
            <div class="dm-topbar-spacer">
                <div class="tk-search">
                    <span class="tk-search-icon">${ic('magnifying-glass', 16)}</span>
        ...
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('caret-down', 16)}</button>
                ...
                <button class="tk-btn tk-btn-outline">${ic('funnel', 16)} Filter</button>
            </div>
            <div class="tk-card">
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('dots-three', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil-simple', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '22', fill: true, tension: 0.3, pointRadius: 3, pointBackgroundColor: t.primary }]
        },
...
tkChart('dm-chart-traffic', function (t) {
    return {
        type: 'doughnut',
        data: { labels: ['Direct', 'Organik', 'Sosial', 'Referral'], datasets: [{ data: [38, 31, 19, 12], backgroundColor: [t.primary, '#FF2EC8', t.accent, '#7B2EFF'], borderColor: t.surface2, borderWidth: 2 }] },
...
<style>
    ...
    .lp-plan-featured { border-color: var(--tk-color-primary); box-shadow: 0 0 0 1px var(--tk-color-primary), 0 0 24px rgba(0, 240, 255, 0.25), var(--tk-shadow-lg); }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); letter-spacing: 0.02em; margin: 0 0 8px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Cyberpunk', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-badge tk-badge-info" style="margin-bottom: 16px">${ic('broadcast', 14)} Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display tk-glitch" id="lp-h-hero">Kelola proyek tanpa drama</h1>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    <h2 class="tk-h2 lp-section-title" id="lp-h-fitur">Semua yang tim Anda <span class="tk-holo-text">butuhkan</span></h2>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: t.primary, borderRadius: 2, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 900; font-size: var(--tk-text-title); letter-spacing: 0.08em; text-align: center; margin-bottom: 20px; }
    ...
    .au-link { color: var(--tk-color-secondary); text-decoration: none; font-weight: 600; }
    .au-link:hover { text-decoration: underline; text-shadow: 0 0 10px rgba(255, 46, 200, 0.5); }
...
<head>
${HEAD_COMMON}
<title>Cyberpunk — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Cyberpunk', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Cyberpunk — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #01030A; border-radius: 48px; background: #01030A; box-shadow: var(--tk-shadow-lg), 0 0 32px rgba(0, 240, 255, 0.12); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Cyberpunk', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const CYBERPUNK_EXAMPLES: Record<string, string> = {
