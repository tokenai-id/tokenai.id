...
/**
 * Fragmen `components/` kit Vaporwave: markup murni dengan kelas kontrak, teks placeholder
 * netral, ikon Iconify (set ri — Remix Icon, garis membulat yang lembut dan lengkap).
...
export const VAPORWAVE_COMPONENTS: Record<string, string> = {
...
<p class="tk-caption">Caption 15px.</p>
`,
...
<button class="tk-btn tk-btn-outline tk-btn-icon" aria-label="Tambah">${icon('add-line')}</button>
`,
...
<div class="tk-field">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih satu opsi</span> ${icon('arrow-down-s-line', 16)}</button>
...
`,
    ...
    <span class="tk-search-icon">${icon('search-line', 16)}</span>
...
`,
    ...
    <button class="tk-select-trigger" type="button">Opsi pertama ${icon('arrow-down-s-line', 16)}</button>
    <div class="tk-select-menu">
        <button class="tk-option tk-option-selected" type="button">Opsi pertama <span class="tk-option-check">${icon('check-line', 16)}</span></button>
        <button class="tk-option" type="button">Opsi kedua</button>
        <button class="tk-option" type="button">Opsi ketiga</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih anggota</span> ${icon('arrow-down-s-line', 16)}</button>
    <div class="tk-select-menu">
        <div class="tk-select-menu-search">${icon('search-line', 14)}<input type="text" placeholder="Cari anggota..."></div>
        <button class="tk-option" type="button">Jane Smith</button>
        <button class="tk-option tk-option-selected" type="button">John Doe <span class="tk-option-check">${icon('check-line', 16)}</span></button>
        <button class="tk-option" type="button">Maya Putri</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button">Peran &middot; 2 dipilih ${icon('arrow-down-s-line', 16)}</button>
    <div class="tk-select-menu">
...
`,
    ...
    <button class="tk-btn tk-btn-outline">Aksi ${icon('arrow-down-s-line', 16)}</button>
    <div class="tk-dropdown-menu">
        <span class="tk-dropdown-label">Aksi baris</span>
        <button class="tk-dropdown-item">${icon('eye-line', 16)} Lihat detail</button>
        <button class="tk-dropdown-item">${icon('pencil-line', 16)} Edit</button>
        <button class="tk-dropdown-item">${icon('file-copy-line', 16)} Duplikat</button>
        <hr class="tk-dropdown-divider">
        <button class="tk-dropdown-item tk-dropdown-item-danger">${icon('delete-bin-line', 16)} Hapus</button>
    </div>
</div>
`,
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua status ${icon('arrow-down-s-line', 16)}</button>
        <div class="tk-select-menu">
            <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${icon('check-line', 16)}</span></button>
            <button class="tk-option" type="button">Aktif</button>
            <button class="tk-option" type="button">Nonaktif</button>
        </div>
    </div>
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua peran ${icon('arrow-down-s-line', 16)}</button>
        <div class="tk-select-menu">
            <label class="tk-option"><input class="tk-checkbox" type="checkbox" checked> Designer</label>
            <label class="tk-option"><input class="tk-checkbox" type="checkbox"> Developer</label>
        </div>
    </div>
    <button class="tk-btn tk-btn-outline">${icon('filter-3-line', 16)} Filter <span class="tk-filter-count">2</span></button>
</div>
...
<nav class="tk-breadcrumb">
    <a href="#">Beranda</a>
    <span class="tk-breadcrumb-sep">${icon('arrow-right-s-line', 14)}</span>
    <a href="#">Proyek</a>
    <span class="tk-breadcrumb-sep">${icon('arrow-right-s-line', 14)}</span>
...
<aside class="tk-sidebar">
    ...
    <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${icon('dashboard-line', 18)} <span class="tk-sidebar-label">Dashboard</span></a>
    <a class="tk-sidebar-item" href="#">${icon('folder-line', 18)} <span class="tk-sidebar-label">Proyek</span></a>
    <span class="tk-sidebar-group">Analitik</span>
    <a class="tk-sidebar-item" href="#">${icon('bar-chart-2-line', 18)} <span class="tk-sidebar-label">Laporan</span></a>
    <span class="tk-sidebar-group">Lainnya</span>
    <a class="tk-sidebar-item" href="#">${icon('settings-3-line', 18)} <span class="tk-sidebar-label">Pengaturan</span></a>
</aside>
`,
    ...
    <tbody>
        <tr>
            ...
            <td class="tk-table-actions">
                <button class="tk-action-btn" aria-label="Lihat">${icon('eye-line', 16)}</button>
                <button class="tk-action-btn" aria-label="Edit">${icon('pencil-line', 16)}</button>
                <button class="tk-action-btn tk-action-btn-danger" aria-label="Hapus">${icon('delete-bin-line', 16)}</button>
            </td>
        </tr>
        <tr>
            ...
            <td class="tk-table-actions">
                <div class="tk-dropdown">
                    <button class="tk-action-btn" aria-label="Menu aksi">${icon('more-line', 16)}</button>
...
`,
    ...
    <span class="tk-empty-icon">${icon('inbox-line', 30)}</span>
...
`,
    ...
    <h1 class="tk-display tk-vhs">Bangun lebih cepat</h1>
...
<div class="tk-feature-grid">
    <div class="tk-feature">
        <span class="tk-feature-icon">${icon('flashlight-line')}</span>
