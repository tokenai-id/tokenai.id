...
/**
 * `showcase.html` kit Vaporwave: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const SWATCHES = [
    ['primary', '#D935B8'], ['primary-hover', '#B7269A'], ['secondary', '#0FB5C9'],
    ['accent', '#FF8A5C'], ['background', '#F5F0FC'], ['surface-2', '#F0E9FA'],
    ['text', '#372A54'], ['text-muted', '#8B7DA9'], ['border', '#DFD3F2'],
    ['success', '#2FB380'], ['warning', '#DB9A00'], ['danger', '#E24B78'], ['info', '#5B7CFA']
].map(([name, hex]) =>
    `<div class="sc-swatch"><div class="sc-swatch-fill" style="background: var(--tk-color-${name})"></div><span>${name}</span><code>${hex}</code></div>`
).join('');

const TYPE_SCALE = [
    ['Display', 'tk-display', '46px'], ['Heading 1', 'tk-h1', '34px'], ['Heading 2', 'tk-h2', '26px'],
    ['Heading 3', 'tk-h3', '20px'], ['Heading 4', 'tk-h4', '17px'], ['Title', 'tk-title', '15px'],
    ['Body', 'tk-body', '16px'], ['Body Small', 'tk-body-sm', '14px'], ['Caption', 'tk-caption', '15px']
].map(([label, cls, size]) =>
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Senja tak pernah usai</p></div>`
).join('');
...
const ICONS = ['home-line', 'search-line', 'user-line', 'settings-3-line', 'bell-line', 'heart-line', 'star-line', 'delete-bin-line', 'pencil-line',
    'check-line', 'close-line', 'add-line', 'arrow-right-line', 'download-cloud-2-line', 'upload-cloud-2-line', 'calendar-line', 'mail-line', 'lock-line']
    .map(name => `<span class="sc-icon" title="ri:${name}"><iconify-icon icon="ri:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const VAPORWAVE_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Vaporwave — Design Kit Showcase</title>
...
<style>
    ...
    .sc-kit-title { font-family: var(--tk-font-mono); font-size: 15px; letter-spacing: 0.14em; text-transform: uppercase; color: var(--tk-color-primary); margin: 0 0 4px; }
    .sc-section { margin-top: 56px; }
    .sc-heading { font-family: var(--tk-font-mono); font-size: 15px; font-weight: 400; letter-spacing: 0.14em; text-transform: uppercase; color: var(--tk-color-primary); border-bottom: 1px solid var(--tk-color-border); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-space-bar { height: 16px; background: var(--tk-vw-grad); opacity: 0.65; border-radius: 4px; }
    ...
    .sc-overlay-demo { position: relative; height: 340px; overflow: hidden; border-radius: var(--tk-radius-lg); }
    ...
    .sc-aesthetic { font-family: var(--tk-font-heading); font-weight: 800; font-style: italic; font-size: 44px; letter-spacing: 0.02em; margin: 0; }
    .sc-statue { font-size: 56px; line-height: 1; filter: saturate(0.4); }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Vaporwave', 'components', '')}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Vaporwave</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Nostalgia surreal 80s-90s: neon pink-ungu, gradien sunset, matahari bergaris terbit di hero, grid perspektif menuju horizon, serif klasikal miring ala patung Romawi, huruf CRT pixel untuk caption, window UI ber-title bar gradien, dan aberasi kromatik VHS. Mode terang = pastel dream; mode gelap = neon sunset. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Playfair Display (heading serif klasikal, display miring) &middot; Outfit (body geometris lembut) &middot; VT323 (caption &amp; readout CRT pixel) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<div class="sc-row">${RADII}</div><p class="tk-caption" style="margin-top: 12px">Sudut membulat lembut &mdash; kapsul untuk tombol dan pil sidebar.</p>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div><p class="tk-caption" style="margin-top: 12px">Bayangan dwiwarna: cyan dari kiri-atas, pink dari kanan-bawah &mdash; kartu melayang di senja.</p>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>ri</code> (Remix Icon, varian -line) &mdash; tulis <code>&lt;iconify-icon icon="ri:nama-line"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-row"><span class="tk-badge">Neon pink-ungu</span><span class="tk-badge">Grid perspektif</span><span class="tk-badge">Sunset</span><span class="tk-badge">Surreal nostalgic</span></div><p class="sc-aesthetic tk-vw-text tk-vhs" style="margin: 20px 0 0">ＡＥＳＴＨＥＴＩＣ</p><p class="tk-caption" style="margin: 4px 0 0">「ベイパーウェイブ」&middot; ｖａｐｏｒｗａｖｅ &middot; １９９５</p><div class="sc-row" style="margin-top: 16px; gap: 20px; align-items: center"><span class="sc-statue" role="img" aria-label="Patung klasik">🏛️</span><span class="sc-statue" role="img" aria-label="Patung">🗿</span><span class="sc-statue" role="img" aria-label="Palem">🌴</span><span class="tk-body-sm tk-muted" style="max-width: 430px">Patung klasik, palem, dan senja abadi &mdash; masa depan yang dibayangkan dari masa lalu yang tidak pernah ada. Serif Romawi dipadu pixel CRT.</span></div><p class="tk-body-sm tk-muted" style="margin: 16px 0; max-width: 560px">Tanda tangannya: teks gradien vapor <code class="tk-code">.tk-vw-text</code> (pink → ungu → cyan), aberasi kromatik VHS statis <code class="tk-code">.tk-vhs</code>, readout CRT <code class="tk-code">.tk-readout</code>, matahari sunset bergaris dan lantai grid perspektif yang dilukis otomatis di <code class="tk-code">.tk-hero</code>, title bar jendela retro bergradien pada modal, dan tautan bergaris bawah bergelombang. Foto dipaksa ke senja: saturasi naik, rona bergeser pink-ungu.</p><div class="sc-row" style="align-items: center; gap: 10px"><span class="tk-readout">ＤＲＥＡＭ//ＯＮ</span><span class="tk-readout">２４：００</span></div>`)}
...
${section('Card Components', `<div class="tk-grid tk-grid-3">${c['components/card.html']}<div class="tk-card"><div class="tk-card-body" style="text-align: center"><div class="tk-feature-icon" style="margin: 0 auto 12px"><iconify-icon icon="ri:user-line" width="22" height="22"></iconify-icon></div><p class="tk-title" style="margin: 0">Jane Doe</p><p class="tk-caption">UI/UX Designer</p></div></div></div><p class="tk-caption" style="margin-top: 12px">Angka statistik memakai teks gradien vapor.</p>`)}
${section('Navigation', `<div class="sc-col" style="max-width: none">${c['components/navigation.html'].replace('<aside', '<nav class="tk-navbar tk-navbar-dark"><span class="tk-navbar-brand">LOGO</span><a class="tk-navbar-link tk-navbar-link-active" href="#">Home</a><a class="tk-navbar-link" href="#">Fitur</a><button class="tk-btn tk-btn-primary tk-btn-sm">Masuk</button></nav><aside')}</div>`)}
...
${section('Charts', `<p class="tk-caption" style="margin-bottom: 12px">Chart.js &mdash; pustaka chart tunggal untuk semua kit. Warna dibaca dari token kit saat digambar &mdash; pink, ungu, cyan, dan peach senja; garis melengkung halus seperti mimpi.</p><div class="tk-grid tk-grid-3"><div class="tk-card"><div class="tk-card-body"><p class="tk-title" style="margin: 0 0 12px">Pengunjung</p><div class="sc-chart-box"><canvas id="sc-chart-line"></canvas></div></div></div><div class="tk-card"><div class="tk-card-body"><p class="tk-title" style="margin: 0 0 12px">Pendapatan</p><div class="sc-chart-box"><canvas id="sc-chart-bar"></canvas></div></div></div><div class="tk-card"><div class="tk-card-body"><p class="tk-title" style="margin: 0 0 12px">Sumber Trafik</p><div class="sc-chart-box"><canvas id="sc-chart-doughnut"></canvas></div></div></div></div>`)}
${section('Modal & Overlay', `<div class="sc-overlay-demo">${c['components/modal.html']}</div><p class="tk-caption" style="margin-top: 12px">Modal adalah jendela retro: title bar gradien vapor dengan tombol jendela ala window manager 90s, dilukis otomatis lewat ::before.</p>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.primary, backgroundColor: t.primary + '26', fill: true, tension: 0.45, pointRadius: 3, pointBackgroundColor: t.accent }] },
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: ['#FF3EA5', '#8A5CFF', '#26D6F0', '#FF8A5C', '#FF3EA5', '#8A5CFF'], borderRadius: 10 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
