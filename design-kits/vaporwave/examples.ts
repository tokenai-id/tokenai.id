...
/**
 * Halaman contoh (`examples/`) kit Vaporwave: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit dua-tema: terang = pastel dream, gelap = neon sunset; saklar tema bawaan exNav dipakai.
 *
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="ri:${name}" width="${size}" height="${size}"></iconify-icon>`;
}

const HEAD_COMMON = `<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;0,800;1,700;1,800&family=Outfit:wght@300;400;500;600;700&family=VT323&display=swap" rel="stylesheet">
...
<title>Vaporwave — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 800; font-style: italic; font-size: 18px; padding: 8px 16px 20px; white-space: nowrap; background: var(--tk-vw-grad); -webkit-background-clip: text; background-clip: text; color: transparent; }
    ...
    .dm-foot-btn { display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px; border: none; border-radius: var(--tk-radius); background: transparent; font-family: inherit; color: var(--tk-color-text); text-align: left; cursor: pointer; }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-color-surface); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 32px; height: 32px; border-radius: 50%; background: var(--tk-vw-grad); color: #FFFFFF; display: inline-flex; align-items: center; justify-content: center; font-weight: 700; font-size: 11px; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Vaporwave', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' } })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('dashboard-line')} <span class="tk-sidebar-label">Dashboard</span></a>
        <a class="tk-sidebar-item" href="#">${ic('team-line')} <span class="tk-sidebar-label">Pengguna</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('bar-chart-2-line')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('line-chart-line')} <span class="tk-sidebar-label">Statistik</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('expand-up-down-line', 16)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user-line', 16)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('settings-3-line', 16)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('logout-box-r-line', 16)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('arrow-right-s-line', 14)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('arrow-right-s-line', 14)}</span>
            ...
            <div class="dm-topbar-spacer">
                ...
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('bell-line', 18)}</button>
            </div>
        </header>
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('arrow-down-s-line', 16)}</button>
                ...
                <button class="tk-btn tk-btn-outline">${ic('filter-3-line', 16)} Filter</button>
            </div>
            <div class="tk-card">
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-line', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye-line', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil-line', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('delete-bin-line', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('user-add-line', 16)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('checkbox-circle-line', 16)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('error-warning-line', 16)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '26', fill: true, tension: 0.45, pointRadius: 3, pointBackgroundColor: t.accent }]
        },
...
tkChart('dm-chart-traffic', function (t) {
    return {
        type: 'doughnut',
        data: { labels: ['Direct', 'Organik', 'Sosial', 'Referral'], datasets: [{ data: [38, 31, 19, 12], backgroundColor: [t.primary, '#8A5CFF', '#26D6F0', t.accent], borderColor: t.surface2, borderWidth: 2 }] },
...
<style>
    ...
    .lp-plan-featured { border-color: transparent; box-shadow: 0 0 0 2px transparent, var(--tk-shadow-lg); background: linear-gradient(var(--tk-color-surface), var(--tk-color-surface)) padding-box, var(--tk-vw-grad) border-box; border: 2px solid transparent; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Vaporwave', 'landing', '../', { device: { base: 'landing', active: 'web' } })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-badge tk-badge-info" style="margin-bottom: 16px">${ic('magic-line', 14)} Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display tk-vhs" id="lp-h-hero">Kelola proyek tanpa drama</h1>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    <h2 class="tk-h2 lp-section-title" id="lp-h-fitur">Semua yang tim Anda <span class="tk-vw-text">butuhkan</span></h2>
    ...
    <div class="tk-feature-grid lp-features">
        ${feature('flashlight-line', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
        ${feature('shield-check-line', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('bar-chart-2-line', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ${feature('team-line', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('plug-line', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('sparkling-2-line', 'Otomatis', 'Rutinitas berulang dikerjakan oleh automasi.')}
    </div>
</section>
    ...
    <div class="lp-proof">
        <div>
            ...
            <div class="lp-proof-stats">
                <div class="lp-proof-stat"><span class="tk-readout">36RB</span><span class="tk-caption">Pengguna aktif</span></div>
                <div class="lp-proof-stat"><span class="tk-readout">99,9%</span><span class="tk-caption">Uptime</span></div>
                <div class="lp-proof-stat"><span class="tk-readout">4,8/5</span><span class="tk-caption">Rating</span></div>
            </div>
        </div>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: [t.primary, '#8A5CFF', '#26D6F0', t.accent], borderRadius: 10, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 800; font-style: italic; font-size: 20px; text-align: center; margin-bottom: 20px; background: var(--tk-vw-grad); -webkit-background-clip: text; background-clip: text; color: transparent; }
    .au-title { text-align: center; margin: 0 0 4px; }
    .au-sub { text-align: center; margin: 0 0 20px; }
    .au-full { width: 100%; box-sizing: border-box; }
    .au-divider { display: flex; align-items: center; gap: 12px; margin: 16px 0; color: var(--tk-color-text-muted); font-size: var(--tk-text-caption); font-family: var(--tk-font-mono); text-transform: uppercase; letter-spacing: 0.10em; }
    ...
    .au-link:hover { color: var(--tk-color-secondary); text-decoration: underline wavy; text-underline-offset: 3px; }
...
<head>
${HEAD_COMMON}
<title>Vaporwave — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Vaporwave', 'components', '../', { device: { base, active: 'web' } })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Vaporwave — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #2B1A5C; border-radius: 48px; background: #2B1A5C; box-shadow: var(--tk-shadow-lg), 0 0 32px rgba(255, 62, 165, 0.18); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Vaporwave', view, '../', { device: { base, active: 'mobile' } })}
<div class="ex-stage">
...
export const VAPORWAVE_EXAMPLES: Record<string, string> = {
