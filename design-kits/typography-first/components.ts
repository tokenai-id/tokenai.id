...
/**
 * Fragmen `components/` kit Typography First: markup murni dengan kelas kontrak, teks placeholder
 * netral, ikon Iconify (set teenyicons — garis ultra-tipis 1px yang tunduk pada huruf). Gayanya
 * seluruhnya di `styles.css` — fragmen inilah yang disalin agen saat merakit halaman, jadi jangan
 * menaruh style atribut di sini.
 *
...
export const TYPOGRAPHY_FIRST_COMPONENTS: Record<string, string> = {
...
<button class="tk-btn tk-btn-outline tk-btn-icon" aria-label="Tambah">${icon('add-outline')}</button>
`,
...
<div class="tk-field">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih satu opsi</span> ${icon('down-outline', 13)}</button>
...
`,
    ...
    <span class="tk-search-icon">${icon('search-outline', 15)}</span>
...
`,
    ...
    <button class="tk-select-trigger" type="button">Opsi pertama ${icon('down-outline', 13)}</button>
    <div class="tk-select-menu">
        <button class="tk-option tk-option-selected" type="button">Opsi pertama <span class="tk-option-check">${icon('tick-outline', 14)}</span></button>
        <button class="tk-option" type="button">Opsi kedua</button>
        <button class="tk-option" type="button">Opsi ketiga</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih anggota</span> ${icon('down-outline', 13)}</button>
    <div class="tk-select-menu">
        <div class="tk-select-menu-search">${icon('search-outline', 13)}<input type="text" placeholder="Cari anggota..."></div>
        <button class="tk-option" type="button">Jane Smith</button>
        <button class="tk-option tk-option-selected" type="button">John Doe <span class="tk-option-check">${icon('tick-outline', 14)}</span></button>
        <button class="tk-option" type="button">Maya Putri</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button">Peran &middot; 2 dipilih ${icon('down-outline', 13)}</button>
    <div class="tk-select-menu">
...
`,
    ...
    <button class="tk-btn tk-btn-outline">Aksi ${icon('down-outline', 13)}</button>
    <div class="tk-dropdown-menu">
        <span class="tk-dropdown-label">Aksi baris</span>
        <button class="tk-dropdown-item">${icon('eye-outline', 15)} Lihat detail</button>
        <button class="tk-dropdown-item">${icon('edit-outline', 15)} Edit</button>
        <button class="tk-dropdown-item">${icon('documents-outline', 15)} Duplikat</button>
        <hr class="tk-dropdown-divider">
        <button class="tk-dropdown-item tk-dropdown-item-danger">${icon('bin-outline', 15)} Hapus</button>
    </div>
</div>
`,
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua status ${icon('down-outline', 13)}</button>
        <div class="tk-select-menu">
            <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${icon('tick-outline', 14)}</span></button>
            <button class="tk-option" type="button">Aktif</button>
            <button class="tk-option" type="button">Nonaktif</button>
        </div>
    </div>
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua peran ${icon('down-outline', 13)}</button>
        <div class="tk-select-menu">
            <label class="tk-option"><input class="tk-checkbox" type="checkbox" checked> Designer</label>
            <label class="tk-option"><input class="tk-checkbox" type="checkbox"> Developer</label>
        </div>
    </div>
    <button class="tk-btn tk-btn-outline">${icon('filter-outline', 14)} Filter <span class="tk-filter-count">2</span></button>
</div>
...
<nav class="tk-breadcrumb">
    <a href="#">Beranda</a>
    <span class="tk-breadcrumb-sep">${icon('right-outline', 11)}</span>
    <a href="#">Proyek</a>
    <span class="tk-breadcrumb-sep">${icon('right-outline', 11)}</span>
...
<aside class="tk-sidebar">
    ...
    <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${icon('home-outline', 16)} <span class="tk-sidebar-label">Dashboard</span></a>
    <a class="tk-sidebar-item" href="#">${icon('folder-outline', 16)} <span class="tk-sidebar-label">Proyek</span></a>
    <span class="tk-sidebar-group">Analitik</span>
    <a class="tk-sidebar-item" href="#">${icon('bar-chart-outline', 16)} <span class="tk-sidebar-label">Laporan</span></a>
    <span class="tk-sidebar-group">Lainnya</span>
    <a class="tk-sidebar-item" href="#">${icon('cog-outline', 16)} <span class="tk-sidebar-label">Pengaturan</span></a>
</aside>
`,
    ...
    <tbody>
        <tr>
            ...
            <td class="tk-table-actions">
                <button class="tk-action-btn" aria-label="Lihat">${icon('eye-outline', 15)}</button>
                <button class="tk-action-btn" aria-label="Edit">${icon('edit-outline', 15)}</button>
                <button class="tk-action-btn tk-action-btn-danger" aria-label="Hapus">${icon('bin-outline', 15)}</button>
            </td>
        </tr>
        <tr>
            ...
            <td class="tk-table-actions">
                <div class="tk-dropdown">
                    <button class="tk-action-btn" aria-label="Menu aksi">${icon('more-horizontal-outline', 15)}</button>
...
`,
    ...
    <span class="tk-empty-icon">${icon('text-outline', 24)}</span>
...
`,
    ...
    <p class="tk-kicker">Bab 01 &mdash; Hero</p>
    <h1 class="tk-display">Bangun <span class="tk-ital">lebih</span> cepat</h1>
