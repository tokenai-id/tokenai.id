...
/**
 * Halaman contoh (`examples/`) kit Typography First: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — huruf paling nyaman dibaca di atas kertas), jadi semua
 * halaman memanggil exNav dengan `themeToggle: false`. Landing page-nya memamerkan perangkat
 * tipografi klasik: drop cap, pull quote, angka seksi raksasa, dan fleuron.
 *
...
function ic(name: string, size = 16): string {
    return `<iconify-icon icon="teenyicons:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Typography First — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 600; font-size: var(--tk-text-h4); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(250, 249, 246, 0.92); backdrop-filter: blur(10px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: var(--tk-color-primary); color: #FFF; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 13px; font-weight: 600; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Typography First', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item" href="#">${ic('users-outline')} <span class="tk-sidebar-label">Pengguna</span></a>
        <a class="tk-sidebar-item" href="#">${ic('folder-outline')} <span class="tk-sidebar-label">Proyek</span></a>
        <span class="tk-sidebar-group">Analitik</span>
        <a class="tk-sidebar-item" href="#">${ic('bar-chart-outline')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('text-outline')} <span class="tk-sidebar-label">Naskah</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="tk-sidebar-label" style="min-width: 0"><span class="tk-body-sm" style="display: block; font-weight: 700">Jane Doe</span><span class="tk-caption" style="text-transform: none; letter-spacing: 0">jane@contoh.com</span></span>
                <span class="dm-foot-chevron tk-sidebar-label">${ic('down-outline', 13)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user-outline', 14)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('cog-outline', 14)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('logout-outline', 14)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            ">${ic('menu-outline', 18)}</button>
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('right-outline', 11)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('right-outline', 11)}</span>
            ...
            <div class="dm-topbar-spacer">
                <div class="tk-search">
                    <span class="tk-search-icon">${ic('search-outline', 14)}</span>
                    <input class="tk-input" type="search" placeholder="Cari apa saja...">
                    <kbd class="tk-search-kbd">Ctrl K</kbd>
                </div>
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('bell-outline', 16)}</button>
            </div>
        </header>
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('down-outline', 13)}</button>
                    <div class="tk-select-menu">
                        <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${ic('tick-outline', 14)}</span></button>
                        <button class="tk-option" type="button">Aktif</button>
                        <button class="tk-option" type="button">Nonaktif</button>
                    </div>
                </div>
                <button class="tk-btn tk-btn-outline">${ic('filter-outline', 13)} Filter</button>
            </div>
            <div class="tk-card">
                <div class="tk-card-header" style="display: flex; align-items: center; justify-content: space-between"><h3 class="tk-title" style="margin: 0">Pengguna Terbaru</h3><button class="tk-btn tk-btn-primary tk-btn-sm">${ic('add-outline', 12)} Tambah</button></div>
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-horizontal-outline', 14)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye-outline', 14)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit-outline', 14)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('bin-outline', 14)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('user-outline', 14)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('tick-circle-outline', 14)}<span>Laporan Juli disetujui &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('exclamation-circle-outline', 14)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '14', fill: true, tension: 0.3, pointRadius: 3.5, pointBackgroundColor: t.text, pointBorderColor: t.surface2, pointBorderWidth: 2, borderWidth: 2.5 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('tick-outline', 14)} ${item}</li>`).join('\n                ');
...
function feature(num: string, iconName: string, title: string, text: string): string {
    return `<div class="tk-feature"><span class="tk-num">${num}</span><span class="tk-feature-icon">${ic(iconName, 20)}</span><h4 class="tk-title" style="margin: 0">${title}</h4><p class="tk-body-sm tk-muted" style="margin: 0">${text}</p></div>`;
}
...
<style>
    ...
    .lp-hero { display: grid; grid-template-columns: 5fr 2fr; gap: 48px; align-items: end; }
    .lp-hero-aside { border-left: 1px solid var(--tk-color-border); padding-left: 24px; padding-bottom: 8px; }
    .lp-hero-aside .tk-dropcap { margin: 0; font-size: var(--tk-text-body-sm); }
    .lp-proof { display: grid; grid-template-columns: 1fr 1fr; gap: 48px; align-items: center; }
    .lp-proof-stats { display: flex; gap: 0; margin-top: 24px; flex-wrap: wrap; }
    .lp-proof-stat { padding: 8px 28px 8px 0; margin-right: 28px; border-right: 1px solid var(--tk-color-border); }
    .lp-proof-stat:last-child { border-right: none; margin-right: 0; padding-right: 0; }
    .lp-proof-stat .tk-h3 { margin: 0; color: var(--tk-color-primary); font-variant-numeric: lining-nums; }
    ...
    .lp-section-title { margin-bottom: 8px; }
    .lp-section-sub { color: var(--tk-color-text-muted); max-width: 34em; margin: 0 0 48px; }
    ...
    .lp-plan-featured { border-color: var(--tk-color-primary); box-shadow: var(--tk-shadow); }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 500; font-size: var(--tk-text-h2); margin: 0 0 8px; font-variant-numeric: lining-nums; }
    ...
    .lp-footer { background: var(--tk-color-secondary); }
    ...
    .lp-footer .tk-caption { color: rgba(250, 249, 246, 0.6); }
    .lp-footer-links { display: flex; gap: 20px; }
    .lp-footer-links a { color: rgba(250, 249, 246, 0.75); text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 600; }
    .lp-footer-links a:hover { color: #FFF; text-decoration: underline; text-underline-offset: 3px; }
    @media (max-width: 860px) {
        .lp-links { display: none; }
        .lp-burger { display: inline-flex; }
        .lp-hero { grid-template-columns: 1fr; gap: 24px; }
        .lp-hero-aside { border-left: none; border-top: 1px solid var(--tk-color-border); padding: 16px 0 0; }
        .lp-features { grid-template-columns: 1fr; }
        .lp-pricing { grid-template-columns: 1fr; }
        .lp-proof { grid-template-columns: 1fr; gap: 24px; }
        .lp-chart-box { height: 220px; }
        .lp-section { padding: 64px 24px; }
        .tk-hero .tk-display { font-size: 46px; }
        .tk-cta { flex-direction: column; text-align: center; }
        .tk-pullquote { padding-left: 24px; }
        .tk-pullquote::before { display: none; }
    }
</style>
</head>
<body>
${EX_EMBED_SCRIPT}
${exNav('Typography First', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('menu-outline', 18)}</button>
</nav>
</header>
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <div class="lp-hero">
        <div>
            <p class="tk-kicker">Bab 01 &mdash; Pembuka</p>
            <h1 class="tk-display" id="lp-h-hero">Kelola proyek <span class="tk-ital">tanpa</span> drama</h1>
            <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; ditata seperti halaman buku yang enak dibaca.</p>
        ...
        <aside class="lp-hero-aside">
            <p class="tk-caption" style="margin: 0 0 8px">Dari pengantar</p>
            <p class="tk-dropcap tk-muted">Kami percaya alat kerja yang baik itu seperti tipografi yang baik: tidak berteriak, tidak menghalangi, dan membiarkan isi pekerjaan Anda menjadi hal yang paling terlihat.</p>
        </aside>
    </div>
</section>
<section id="fitur" aria-labelledby="lp-h-fitur">
    <div class="lp-section" style="padding-top: 48px">
        <p class="tk-kicker">Bab 02 &mdash; Fitur</p>
        <h2 class="tk-h1 lp-section-title" id="lp-h-fitur">Semua yang tim Anda butuhkan</h2>
        ...
        <div class="tk-feature-grid lp-features">
            ${feature('01', 'stopwatch-outline', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
            ${feature('02', 'shield-tick-outline', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
            ${feature('03', 'bar-chart-outline', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
            ${feature('04', 'users-outline', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
            ${feature('05', 'plug-outline', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
            ${feature('06', 'text-outline', 'Terbaca', 'Setiap halaman ditata dengan hierarki huruf yang jernih.')}
        </div>
        <blockquote class="tk-pullquote" style="margin-top: 64px">
            Alat terbaik adalah yang tidak terasa &mdash; seperti huruf yang baik, ia bekerja tanpa minta diperhatikan.
            <cite>Jane Doe &mdash; Kepala Produk, Contoh Studio</cite>
        </blockquote>
    </div>
</section>
<section id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-section" style="padding-top: 0">
        <div class="lp-proof">
            <div>
                <p class="tk-kicker">Bab 03 &mdash; Bukti</p>
                <h2 class="tk-h1" style="margin: 0 0 8px" id="lp-h-bukti">Tumbuh bersama ribuan tim</h2>
                <p class="tk-muted" style="margin: 0; max-width: 30em">Pengguna aktif naik empat kuartal berturut-turut. Angkanya bicara sendiri &mdash; dan tim Anda bisa jadi bagian berikutnya.</p>
...
<div class="lp-section" style="padding: 0 24px"><div class="tk-fleuron" aria-hidden="true"><span>&#10086;</span></div></div>
<section id="harga" aria-labelledby="lp-h-harga">
    <div class="lp-section" style="padding-top: 0">
        <p class="tk-kicker">Bab 04 &mdash; Harga</p>
        <h2 class="tk-h1 lp-section-title" id="lp-h-harga">Harga yang sederhana</h2>
...
<section id="cta" aria-labelledby="lp-h-cta">
    <div class="lp-section" style="padding-top: 0">
        <div class="tk-cta">
            ...
            <button class="tk-btn tk-btn-primary">Coba Sekarang ${ic('arrow-right-outline', 14)}</button>
        </div>
    </div>
</section>
</main>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#E3DFD6', '#B08D3E', '#1A1815', t.primary], borderRadius: 2, maxBarThickness: 52 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 600; font-size: var(--tk-text-h4); text-align: center; margin-bottom: 20px; }
    .au-title { text-align: center; margin: 0 0 4px; }
    .au-sub { text-align: center; margin: 0 0 20px; text-transform: none; letter-spacing: 0; }
    .au-full { width: 100%; box-sizing: border-box; }
    .au-divider { display: flex; align-items: center; gap: 12px; margin: 16px 0; color: var(--tk-color-text-muted); font-size: var(--tk-text-caption); }
    ...
    .au-link { color: var(--tk-color-primary); text-decoration: underline; text-underline-offset: 3px; font-weight: 600; }
    .au-link:hover { color: var(--tk-color-primary-hover); }
    .au-alt { text-align: center; margin: 18px 0 0; text-transform: none; letter-spacing: 0; }`;
...
<head>
${HEAD_COMMON}
<title>Typography First — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Typography First', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Typography First — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #1A1815; border-radius: 48px; background: #1A1815; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Typography First', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const TYPOGRAPHY_FIRST_EXAMPLES: Record<string, string> = {
