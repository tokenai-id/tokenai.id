...
/**
 * Halaman contoh (`examples/`) kit Cybercore: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit dua-tema: terang = kertas teknik, gelap = CRT fosfor; saklar tema bawaan exNav dipakai.
 *
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="codicon:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Cybercore — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 800; font-size: var(--tk-text-title); padding: 8px 16px 20px; white-space: nowrap; }
    .dm-sidebar-brand::before { content: '~/'; color: var(--tk-color-primary); }
    .dm-sidebar-brand-mini { display: none; }
    .tk-sidebar-collapsed .dm-sidebar-brand { text-align: center; padding: 8px 0 20px; }
    .tk-sidebar-collapsed .dm-sidebar-brand::before { content: ''; }
    ...
    .dm-foot-btn { display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px; border: none; border-radius: 0; background: transparent; font-family: inherit; color: var(--tk-color-text); text-align: left; cursor: pointer; }
    ...
    .dm-avatar { width: 32px; height: 32px; background: var(--tk-color-text); color: var(--tk-color-background); display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-mono); font-weight: 700; font-size: 11px; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Cybercore', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' } })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item" href="#">${ic('organization')} <span class="tk-sidebar-label">Pengguna</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('graph')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('graph-line')} <span class="tk-sidebar-label">Statistik</span></a>
        <span class="tk-sidebar-group">Lainnya</span>
        <a class="tk-sidebar-item" href="#">${ic('settings-gear')} <span class="tk-sidebar-label">Pengaturan</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('fold', 16)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('account', 16)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('settings-gear', 16)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('sign-out', 16)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('ellipsis', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('person-add', 16)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('pass', 16)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('warning', 16)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '22', fill: true, stepped: true, pointRadius: 3, pointStyle: 'rect', pointBackgroundColor: t.primary }]
        },
...
<style>
    ...
    ${EX_NAV_CSS}
    .lp-skip { position: absolute; left: -9999px; top: 0; z-index: 30; background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); padding: 8px 16px; border-radius: 0; text-decoration: none; font-size: var(--tk-text-body-sm); }
    ...
    .lp-proof-stats { display: flex; gap: 24px; margin: 24px 0; flex-wrap: wrap; }
    .lp-proof-stat .tk-caption { display: block; margin-top: 6px; }
    .lp-proof .tk-terminal { margin: 0; }
    ...
    .lp-plan-featured { border-color: var(--tk-color-primary); box-shadow: 0 0 0 1px var(--tk-color-primary), var(--tk-shadow-lg); }
    ...
    .lp-footer-links a { color: var(--tk-color-text-muted); text-decoration: none; font-size: var(--tk-text-body-sm); font-family: var(--tk-font-mono); }
    .lp-footer-links a:hover { color: var(--tk-color-primary); text-decoration: underline; }
    @media (max-width: 860px) {
        ...
        .tk-hero .tk-display { font-size: 26px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Cybercore', 'landing', '../', { device: { base: 'landing', active: 'web' } })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-badge tk-badge-info" style="margin-bottom: 16px">${ic('terminal', 14)} Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display tk-cursor" id="lp-h-hero">Kelola proyek tanpa drama</h1>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ${feature('pulse', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
        ${feature('shield', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('graph', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ${feature('organization', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('plug', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('sparkle', 'Otomatis', 'Rutinitas berulang dikerjakan oleh automasi.')}
    </div>
</section>
<section class="lp-section" style="padding-top: 0" id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-proof">
        <div>
            ...
            <div class="tk-terminal">
                <span class="tk-prompt">status --ringkas</span>
                <span>uptime 99.9% &middot; 36.000 pengguna online &middot; 0 insiden</span>
            </div>
        </div>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: t.primary, borderRadius: 0, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 800; font-size: var(--tk-text-title); text-align: center; margin-bottom: 20px; }
    .au-brand::before { content: '~/'; color: var(--tk-color-primary); }
    ...
    .au-link:hover { background: var(--tk-color-text); color: var(--tk-color-background); }
...
<head>
${HEAD_COMMON}
<title>Cybercore — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Cybercore', 'components', '../', { device: { base, active: 'web' } })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Cybercore — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #141414; border-radius: 6px; background: #141414; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Cybercore', view, '../', { device: { base, active: 'mobile' } })}
<div class="ex-stage">
...
export const CYBERCORE_EXAMPLES: Record<string, string> = {
