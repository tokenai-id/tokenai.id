...
/**
 * Fragmen `components/` kit Dark Futuristic: markup murni dengan kelas kontrak, teks placeholder
 * netral, ikon Iconify (set hugeicons — garis modern bersudut lembut yang pas untuk produk
 * teknologi). Gayanya seluruhnya di `styles.css` — fragmen inilah yang disalin agen saat merakit
 * halaman, jadi jangan menaruh style atribut di sini.
 *
...
export const DARK_FUTURISTIC_COMPONENTS: Record<string, string> = {
...
<button class="tk-btn tk-btn-outline tk-btn-icon" aria-label="Tambah">${icon('add-01')}</button>
`,
...
<div class="tk-field">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih satu opsi</span> ${icon('arrow-down-01', 14)}</button>
...
`,
    ...
    <span class="tk-search-icon">${icon('search-01', 15)}</span>
...
`,
    ...
    <button class="tk-select-trigger" type="button">Opsi pertama ${icon('arrow-down-01', 14)}</button>
    <div class="tk-select-menu">
        <button class="tk-option tk-option-selected" type="button">Opsi pertama <span class="tk-option-check">${icon('tick-01', 15)}</span></button>
        <button class="tk-option" type="button">Opsi kedua</button>
        <button class="tk-option" type="button">Opsi ketiga</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih anggota</span> ${icon('arrow-down-01', 14)}</button>
    <div class="tk-select-menu">
        <div class="tk-select-menu-search">${icon('search-01', 13)}<input type="text" placeholder="Cari anggota..."></div>
        <button class="tk-option" type="button">Jane Smith</button>
        <button class="tk-option tk-option-selected" type="button">John Doe <span class="tk-option-check">${icon('tick-01', 15)}</span></button>
        <button class="tk-option" type="button">Maya Putri</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button">Peran &middot; 2 dipilih ${icon('arrow-down-01', 14)}</button>
    <div class="tk-select-menu">
...
`,
    ...
    <button class="tk-btn tk-btn-outline">Aksi ${icon('arrow-down-01', 14)}</button>
    <div class="tk-dropdown-menu">
        <span class="tk-dropdown-label">Aksi baris</span>
        <button class="tk-dropdown-item">${icon('view', 15)} Lihat detail</button>
        <button class="tk-dropdown-item">${icon('edit-02', 15)} Edit</button>
        <button class="tk-dropdown-item">${icon('copy-01', 15)} Duplikat</button>
        <hr class="tk-dropdown-divider">
        <button class="tk-dropdown-item tk-dropdown-item-danger">${icon('delete-02', 15)} Hapus</button>
    </div>
</div>
`,
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua status ${icon('arrow-down-01', 14)}</button>
        <div class="tk-select-menu">
            <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${icon('tick-01', 15)}</span></button>
            <button class="tk-option" type="button">Aktif</button>
            <button class="tk-option" type="button">Nonaktif</button>
        </div>
    </div>
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua peran ${icon('arrow-down-01', 14)}</button>
        <div class="tk-select-menu">
            <label class="tk-option"><input class="tk-checkbox" type="checkbox" checked> Designer</label>
...
<nav class="tk-breadcrumb">
    <a href="#">Beranda</a>
    <span class="tk-breadcrumb-sep">${icon('arrow-right-01', 12)}</span>
    <a href="#">Proyek</a>
    <span class="tk-breadcrumb-sep">${icon('arrow-right-01', 12)}</span>
...
<aside class="tk-sidebar">
    ...
    <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${icon('dashboard-square-01', 17)} <span class="tk-sidebar-label">Dashboard</span></a>
    <a class="tk-sidebar-item" href="#">${icon('folder-01', 17)} <span class="tk-sidebar-label">Proyek</span></a>
    <span class="tk-sidebar-group">Analitik</span>
    <a class="tk-sidebar-item" href="#">${icon('chart-line-data-01', 17)} <span class="tk-sidebar-label">Laporan</span></a>
    <span class="tk-sidebar-group">Lainnya</span>
    <a class="tk-sidebar-item" href="#">${icon('settings-02', 17)} <span class="tk-sidebar-label">Pengaturan</span></a>
</aside>
`,
    ...
    <tbody>
        <tr>
            ...
            <td class="tk-table-actions">
                <button class="tk-action-btn" aria-label="Lihat">${icon('view', 15)}</button>
                <button class="tk-action-btn" aria-label="Edit">${icon('edit-02', 15)}</button>
                <button class="tk-action-btn tk-action-btn-danger" aria-label="Hapus">${icon('delete-02', 15)}</button>
            </td>
        </tr>
        <tr>
...
`,
    ...
    <span class="tk-empty-icon">${icon('ai-brain-01', 26)}</span>
...
`,
    ...
    <p class="tk-kicker"><span class="tk-dot"></span> Sistem aktif</p>
    <h1 class="tk-display">Bangun <span class="tk-gradient-text">lebih</span> cepat</h1>
