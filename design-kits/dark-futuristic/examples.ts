...
/**
 * Halaman contoh (`examples/`) kit Dark Futuristic: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (gelap bawaan — produk teknologi hidup di ruang gelap), jadi semua halaman
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="hugeicons:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Dark Futuristic — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { display: flex; align-items: center; gap: 8px; font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(7, 8, 13, 0.72); backdrop-filter: blur(16px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: var(--tk-df-gradient); color: #05070E; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 700; flex: none; }
    ...
    .dm-activity-item iconify-icon { color: var(--tk-color-accent); margin-top: 2px; flex: none; }
    .dm-status { display: inline-flex; align-items: center; gap: 8px; }
    ...
    .dm-sidebar.dm-sidebar-open { display: flex; position: fixed; top: 48px; left: 0; bottom: 0; height: auto; z-index: 90; background: #0B0D14; box-shadow: var(--tk-shadow-lg); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Dark Futuristic', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        <span class="dm-sidebar-brand">${ic('cpu', 20)}<span class="tk-sidebar-label">Logo</span><span class="dm-sidebar-brand-mini"></span></span>
        <span class="tk-sidebar-group">Umum</span>
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('dashboard-square-01')} <span class="tk-sidebar-label">Dashboard</span></a>
        <a class="tk-sidebar-item" href="#">${ic('user-multiple')} <span class="tk-sidebar-label">Pengguna</span></a>
        <a class="tk-sidebar-item" href="#">${ic('folder-01')} <span class="tk-sidebar-label">Proyek</span></a>
        <span class="tk-sidebar-group">Analitik</span>
        <a class="tk-sidebar-item" href="#">${ic('chart-line-data-01')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('ai-brain-01')} <span class="tk-sidebar-label">Model AI</span></a>
        <span class="tk-sidebar-group">Lainnya</span>
        <a class="tk-sidebar-item" href="#">${ic('settings-02')} <span class="tk-sidebar-label">Pengaturan</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('arrow-down-01', 14)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user', 15)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('settings-02', 15)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('logout-01', 15)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            ">${ic('menu-01', 20)}</button>
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('arrow-right-01', 12)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('arrow-right-01', 12)}</span>
            ...
            <div class="dm-topbar-spacer">
                <div class="tk-search">
                    <span class="tk-search-icon">${ic('search-01', 15)}</span>
                    <input class="tk-input" type="search" placeholder="Cari apa saja...">
                    <kbd class="tk-search-kbd">Ctrl K</kbd>
                </div>
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('notification-03', 17)}</button>
            </div>
        </header>
        <main class="dm-content">
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap">
                ...
                <span class="tk-badge tk-badge-success dm-status"><span class="tk-dot"></span> Semua sistem normal</span>
            </div>
            <div class="dm-stats">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('arrow-down-01', 14)}</button>
                    <div class="tk-select-menu">
                        <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${ic('tick-01', 15)}</span></button>
            ...
            <div class="tk-card">
                <div class="tk-card-header" style="display: flex; align-items: center; justify-content: space-between"><h3 class="tk-title" style="margin: 0">Pengguna Terbaru</h3><button class="tk-btn tk-btn-primary tk-btn-sm">${ic('add-01', 13)} Tambah</button></div>
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-horizontal', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('view', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit-02', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('delete-02', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        ...
                        <div class="dm-activity-item">${ic('checkmark-circle-01', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('alert-01', 15)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '24', fill: true, tension: 0.4, pointRadius: 3.5, pointBackgroundColor: '#22D3EE', pointBorderColor: t.surface2, pointBorderWidth: 2, borderWidth: 2.5 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('tick-01', 15)} ${item}</li>`).join('\n                ');
...
<style>
    ...
    .lp-term-panel { max-width: 620px; margin: 40px auto 0; text-align: left; }
    ...
    .lp-proof-stat { background: var(--tk-color-surface); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); padding: 14px 22px; }
    .lp-proof-stat .tk-h3 { margin: 0; color: var(--tk-color-accent); }
    ...
    .lp-section-sub { color: var(--tk-color-text-muted); max-width: 560px; margin: 0 auto; }
    ...
    .lp-plan-featured { border-color: rgba(77, 124, 254, 0.45); box-shadow: 0 0 40px rgba(77, 124, 254, 0.16); }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); margin: 0 0 8px; letter-spacing: -0.03em; }
    ...
    .lp-plan-list iconify-icon { color: var(--tk-color-accent); flex: none; }
    .lp-footer { background: #05060A; border-top: 1px solid var(--tk-color-border); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Dark Futuristic', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('menu-01', 20)}</button>
</nav>
</header>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <p class="tk-kicker"><span class="tk-dot"></span> Uptime 99,9% &middot; Real-time</p>
    <h1 class="tk-display" id="lp-h-hero" style="margin-top: 24px">Kelola proyek <span class="tk-gradient-text">tanpa drama</span></h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; cepat, aman, dan terukur.</p>
    ...
    <div class="tk-panel tk-hud lp-term-panel">
        <p class="tk-term" style="margin: 0">
            <b>$</b> logo deploy --env production<br>
            <span>&#10003;</span> membangun 128 modul dalam 4,2 detik<br>
            <span>&#10003;</span> uji otomatis lolos &mdash; 412/412<br>
            <b>&rarr;</b> live di <span>contoh.id</span>
        </p>
    </div>
</section>
<hr class="tk-beam" style="max-width: 1200px; margin-left: auto; margin-right: auto">
<section id="fitur" aria-labelledby="lp-h-fitur">
    <div class="lp-section" style="padding-top: 40px">
        <div class="lp-section-head">
            <p class="tk-kicker">Kemampuan</p>
            <h2 class="tk-h1" id="lp-h-fitur" style="margin: 16px 0 8px">Semua yang tim Anda butuhkan</h2>
        ...
        <div class="tk-feature-grid lp-features">
            ...
            ${feature('shield-01', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
            ${feature('chart-line-data-01', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
            ${feature('user-multiple', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
            ${feature('plug-socket', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
            ${feature('ai-brain-01', 'Cerdas', 'Ringkasan dan prioritas tugas dibantu model AI.')}
        </div>
    </div>
</section>
<section id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-section" style="padding-top: 0">
        <div class="lp-proof">
            <div>
                <p class="tk-kicker">Traksi</p>
                <h2 class="tk-h1" style="margin: 16px 0 8px" id="lp-h-bukti">Tumbuh bersama ribuan tim</h2>
...
<section id="harga" aria-labelledby="lp-h-harga">
    <div class="lp-section" style="padding-top: 0">
        <div class="lp-section-head">
            <p class="tk-kicker">Harga</p>
            <h2 class="tk-h1" id="lp-h-harga" style="margin: 16px 0 8px">Harga yang sederhana</h2>
...
<section id="cta" aria-labelledby="lp-h-cta">
    <div class="lp-section" style="padding-top: 0">
        <div class="tk-cta tk-hud">
            ...
            <button class="tk-btn tk-btn-primary">Coba Sekarang ${ic('arrow-right-01', 15)}</button>
        </div>
    </div>
</section>
</main>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#232838', '#A78BFA', '#22D3EE', t.primary], borderRadius: 6, maxBarThickness: 52 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-link { color: var(--tk-color-accent); text-decoration: none; font-weight: 500; }
...
<head>
${HEAD_COMMON}
<title>Dark Futuristic — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Dark Futuristic', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
    <div class="tk-card au-card tk-hud">
        <div class="tk-card-body">
...
<head>
${HEAD_COMMON}
<title>Dark Futuristic — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #12151F; border-radius: 48px; background: #12151F; box-shadow: var(--tk-shadow-lg), 0 0 48px rgba(77, 124, 254, 0.18); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Dark Futuristic', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const DARK_FUTURISTIC_EXAMPLES: Record<string, string> = {
