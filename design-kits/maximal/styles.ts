...
export const MAXIMAL_STYLES = `/* Maximal — kit design TOKENAI.
   Estetika maximalism, kebalikan minimalism: warna berani saling bertabrakan, tipografi
   raksasa, pola dan tekstur bertumpuk, gradasi neon, dan bayangan dua lapis — lebih itu lebih,
   tetapi tetap tertata di atas grid yang disiplin.
   Kontrak: docs/kontrak-kit-design.md. Semua nilai design tinggal di :root. */

/* == tk: tokens ================================================= */
:root {
    --tk-color-primary: #D6187E;
    --tk-color-primary-hover: #B90F69;
    --tk-color-primary-contrast: #FFFBEB;
    --tk-color-secondary: #FFDE59;
    --tk-color-secondary-hover: #F2CE33;
    --tk-color-secondary-contrast: #2A1B3D;
    --tk-color-accent: #6C2BD9;
    --tk-color-background: #F3ECFF;
    --tk-color-surface: #FFFFFF;
    --tk-color-surface-2: #FFE9F5;
    --tk-color-text: #2A1B3D;
    --tk-color-text-muted: #7A6693;
    --tk-color-border: #2A1B3D;
    --tk-color-success: #0E9F6E;
    --tk-color-success-soft: #D9F4E7;
    --tk-color-warning: #B45309;
    --tk-color-warning-soft: #FFF0C2;
    --tk-color-danger: #DC2626;
    --tk-color-danger-soft: #FFE1DC;
    --tk-color-info: #1D4ED8;
    --tk-color-info-soft: #DCE7FF;

    --tk-font-heading: 'Unbounded', system-ui, sans-serif;
    --tk-font-body: 'Sora', system-ui, sans-serif;
    ...
    --tk-border-width: 2px;
    /* Bayangan berlapis dua warna: offset ungu di dekat, tinta plum di jauh. */
    --tk-shadow-sm: 3px 3px 0 rgba(108, 43, 217, 0.85);
    --tk-shadow: 5px 5px 0 rgba(108, 43, 217, 0.9);
    --tk-shadow-lg: 6px 6px 0 #FFD400, 12px 12px 0 rgba(42, 27, 61, 0.9);

    --tk-transition: 140ms ease;
    --tk-container: 1200px;

    /* Token khas maximal (bukan kontrak), dipakai blok flavor: palet pop tambahan yang
       sengaja bertabrakan, gradasi neon untuk judul/CTA, dan pola titik latar. */
    --tk-maxi-yellow: #FFD400;
    --tk-maxi-teal: #00B8A9;
    --tk-maxi-orange: #FF7A00;
    --tk-maxi-pink: #FFD1E8;
    --tk-maxi-mint: #D7F5E9;
    --tk-maxi-lilac: #E9DEFF;
    --tk-maxi-grad: linear-gradient(100deg, #D6187E, #6C2BD9 55%, #FF7A00);
    --tk-maxi-dots: radial-gradient(rgba(108, 43, 217, 0.16) 1.5px, transparent 1.5px);
    --tk-maxi-mark: linear-gradient(180deg, transparent 55%, rgba(255, 212, 0, 0.85) 55%);
}

/* Mode gelap: maximalism tengah malam — ungu pekat dengan neon magenta, violet, dan kuning
   menyala; bayangan offset gelap plus pendar neon tipis. Aktifkan dengan atribut
   data-tk-theme="dark" pada <html> atau <body>. */
[data-tk-theme="dark"] {
    --tk-color-primary: #FF4FA3;
    --tk-color-primary-hover: #FF6FB5;
    --tk-color-primary-contrast: #1A0F26;
    --tk-color-secondary: #FFE14D;
    --tk-color-secondary-hover: #FFEA80;
    --tk-color-secondary-contrast: #1A0F26;
    --tk-color-accent: #B48CFF;
    --tk-color-background: #17102B;
    --tk-color-surface: #221740;
    --tk-color-surface-2: #2E2054;
    --tk-color-text: #F4EDFF;
    --tk-color-text-muted: #B3A5D6;
    --tk-color-border: #F4EDFF;
    --tk-color-success: #4ADE9D;
    --tk-color-success-soft: #123528;
    --tk-color-warning: #FBBF24;
    --tk-color-warning-soft: #38290A;
    --tk-color-danger: #FF7B6B;
    --tk-color-danger-soft: #3D1410;
    --tk-color-info: #7FA9FF;
    --tk-color-info-soft: #14213D;
    --tk-shadow-sm: 3px 3px 0 rgba(0, 0, 0, 0.55);
    --tk-shadow: 5px 5px 0 rgba(0, 0, 0, 0.6), 0 0 22px rgba(255, 79, 163, 0.16);
    --tk-shadow-lg: 6px 6px 0 rgba(255, 225, 77, 0.85), 12px 12px 0 rgba(0, 0, 0, 0.6);
    --tk-maxi-yellow: #FFE14D;
    --tk-maxi-teal: #2DD4BF;
    --tk-maxi-orange: #FF9A4D;
    --tk-maxi-pink: #4A2242;
    --tk-maxi-mint: #163B31;
    --tk-maxi-lilac: #2E2054;
    --tk-maxi-grad: linear-gradient(100deg, #FF4FA3, #B48CFF 55%, #FF9A4D);
    --tk-maxi-dots: radial-gradient(rgba(180, 140, 255, 0.18) 1.5px, transparent 1.5px);
    --tk-maxi-mark: linear-gradient(180deg, transparent 55%, rgba(255, 225, 77, 0.35) 55%);
}
...
.tk-display { font-size: var(--tk-text-display); font-weight: 900; letter-spacing: -0.01em; }
...
.tk-link { color: var(--tk-color-primary); font-weight: 700; text-decoration: underline; text-decoration-thickness: 2.5px; text-decoration-color: var(--tk-maxi-teal); text-underline-offset: 4px; }
.tk-link:hover { color: var(--tk-color-accent); text-decoration-color: var(--tk-maxi-yellow); }
.tk-code { font-family: var(--tk-font-mono); font-size: 0.95em; background: var(--tk-color-surface-2); padding: 2px 6px; border-radius: var(--tk-radius-sm); border: 1px solid var(--tk-color-border); }
...
.tk-btn {
    ...
    cursor: pointer;
    transition: filter var(--tk-transition), transform var(--tk-transition), box-shadow var(--tk-transition), background var(--tk-transition), color var(--tk-transition);
    text-decoration: none;
    white-space: nowrap;
}
.tk-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.tk-btn:hover:not(:disabled) { transform: translate(-1px, -2px); box-shadow: var(--tk-shadow); }
.tk-btn:not(:disabled):active { transform: translate(2px, 2px); box-shadow: none; }
/* Tombol utama adalah gradasi neon tiga warna — panggung utama palet maximal. */
.tk-btn-primary { background: var(--tk-maxi-grad); color: #FFFBEB; }
.tk-btn-primary:hover:not(:disabled) { filter: saturate(1.2) brightness(1.05); }
...
.tk-btn-danger { background: var(--tk-color-danger); color: #FFFBEB; }
...
.tk-btn-text:hover:not(:disabled) { transform: none; text-decoration: underline; text-decoration-thickness: 2.5px; text-underline-offset: 4px; }
.tk-btn-sm { font-size: var(--tk-text-caption); padding: 8px 14px; }
.tk-btn-lg { font-size: var(--tk-text-body); padding: 16px 28px; }
...
.tk-option:hover { background: var(--tk-maxi-lilac); }
.tk-option-selected { background: var(--tk-maxi-mark); font-weight: 700; }
...
.tk-checkbox:hover, .tk-radio:hover { background: var(--tk-maxi-lilac); }
...
.tk-toggle:checked { background: var(--tk-maxi-grad); }
.tk-toggle:checked::after { transform: translateX(18px); background: #FFFBEB; }
...
.tk-dropdown-item:hover { background: var(--tk-maxi-lilac); }
...
.tk-dropdown-divider { height: 1px; background: var(--tk-color-border); opacity: 0.25; border: none; margin: var(--tk-space-xs) 0; }
...
.tk-navbar-brand { font-family: var(--tk-font-heading); font-weight: 900; font-size: 20px; margin-right: auto; }
...
.tk-navbar-link-active { color: var(--tk-color-text); background: var(--tk-maxi-mark); padding: 2px 6px; border-radius: 3px; }
.tk-navbar-dark { background: #17102B; border-bottom-color: #17102B; }
.tk-navbar-dark .tk-navbar-brand { color: #F4EDFF; }
.tk-navbar-dark .tk-navbar-link { color: #F4EDFF; opacity: 0.75; }
.tk-navbar-dark .tk-navbar-link:hover, .tk-navbar-dark .tk-navbar-link-active { opacity: 1; color: #F4EDFF; }
.tk-sidebar {
    width: 240px;
    background: var(--tk-color-surface);
...
.tk-sidebar-item:hover { background: var(--tk-maxi-lilac); color: var(--tk-color-text); }
.tk-sidebar-item-active { background: var(--tk-maxi-mark); color: var(--tk-color-text); }
...
.tk-tab-active { color: var(--tk-color-text); border-image: var(--tk-maxi-grad) 1; border-bottom-width: 4px; border-bottom-style: solid; }
.tk-segmented { display: inline-flex; background: var(--tk-color-surface); border: var(--tk-border-width) solid var(--tk-color-border); border-radius: var(--tk-radius-full); padding: 3px; gap: 3px; }
.tk-segment {
    padding: 6px 14px;
    font-family: var(--tk-font-body);
    font-size: var(--tk-text-body-sm);
...
.tk-alert {
    ...
    box-shadow: 3px 3px 0 rgba(42, 27, 61, 0.25);
}
...
.tk-page-active { background: var(--tk-color-secondary); border-color: var(--tk-color-border); }
...
.tk-spinner {
    width: 24px;
    height: 24px;
    border: 3px solid var(--tk-maxi-lilac);
...
.tk-progress-bar { height: 100%; background: var(--tk-maxi-grad); transition: width var(--tk-transition); }
.tk-skeleton {
    background: linear-gradient(90deg, var(--tk-maxi-lilac) 25%, var(--tk-color-background) 50%, var(--tk-maxi-lilac) 75%);
...
/* Ikon fitur = koin gradasi neon dengan ikon terang. */
.tk-feature-icon {
    width: 48px;
    height: 48px;
    display: flex;
...
.tk-cta .tk-h3 { color: #FFFBEB; margin: 0; }
.tk-cta .tk-muted { color: #FFFBEB; opacity: 0.85; }
...
/* == tk: maximal flavor ========================================= */
/* Penimpa karakter maximal di atas struktur kontrak: menang kaskade karena berada di ekor
   berkas. Di sinilah "lebih itu lebih"-nya lahir — latar bertabur pola titik, tile fitur
   berwarna bergantian, judul hero gradasi raksasa, dan foto berbingkai tebal berbayangan. */
body {
    background-image: var(--tk-maxi-dots);
    background-size: 22px 22px;
    background-color: var(--tk-color-background);
}
/* Tile fitur = kartu permen: warna latar bergantian (pink, mint, kuning muda, lilac),
   sedikit miring bergantian, menegak saat disentuh kursor. */
.tk-feature {
    background: var(--tk-color-surface);
    border: var(--tk-border-width) solid var(--tk-color-border);
    border-radius: var(--tk-radius-lg);
...
.tk-feature:nth-child(4n+1) { background: var(--tk-maxi-pink); transform: rotate(-0.6deg); }
.tk-feature:nth-child(4n+2) { background: var(--tk-maxi-mint); transform: rotate(0.5deg); }
.tk-feature:nth-child(4n+3) { background: var(--tk-color-warning-soft); transform: rotate(-0.4deg); }
.tk-feature:nth-child(4n+4) { background: var(--tk-maxi-lilac); transform: rotate(0.6deg); }
.tk-feature:hover { transform: rotate(0deg) translateY(-2px); box-shadow: var(--tk-shadow); }
/* Judul hero: gradasi neon tiga warna pada teks raksasa Unbounded. */
.tk-hero .tk-display {
    background: var(--tk-maxi-grad);
...
/* Foto: bingkai tinta tebal, sudut membulat, dan bayangan dua lapis. */
.tk-card img, .tk-hero img, figure img {
    border: var(--tk-border-width) solid var(--tk-color-border);
    border-radius: var(--tk-radius);
    box-shadow: var(--tk-shadow-sm);
}
/* Navbar membawa pita gradasi tipis di tepi bawah — bendera palet kit. */
.tk-navbar { position: relative; }
.tk-navbar::after {
    content: '';
    position: absolute;
    left: 0;
    right: 0;
    bottom: calc(-1 * var(--tk-border-width) - 4px);
    height: 4px;
    background: var(--tk-maxi-grad);
    pointer-events: none;
}
