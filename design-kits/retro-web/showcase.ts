...
/**
 * `showcase.html` kit Retro Web: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const SWATCHES = [
    ['primary', '#0000EE'], ['primary-hover', '#0000B0'], ['secondary', '#008080'],
    ['accent', '#FF00CC'], ['background', '#C3C7CB'], ['surface-2', '#E6E6E6'],
    ['text', '#000000'], ['text-muted', '#4A4A4A'], ['border', '#808080'],
    ['success', '#007A1F'], ['warning', '#8A5A00'], ['danger', '#CC0000'], ['info', '#0000EE']
].map(([name, hex]) =>
    `<div class="sc-swatch"><div class="sc-swatch-fill" style="background: var(--tk-color-${name})"></div><span>${name}</span><code>${hex}</code></div>`
).join('');

const TYPE_SCALE = [
    ['Display', 'tk-display', '44px'], ['Heading 1', 'tk-h1', '32px'], ['Heading 2', 'tk-h2', '24px'],
...
const ICONS = ['home', 'search', 'user', 'settings-cog', 'bell', 'heart', 'star', 'trash', 'pencil',
    'check', 'close', 'plus', 'arrow-right', 'download', 'upload', 'calendar', 'mail', 'lock']
    .map(name => `<span class="sc-icon" title="pixelarticons:${name}"><iconify-icon icon="pixelarticons:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const RETRO_WEB_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Retro Web — Design Kit Showcase</title>
...
<style>
    ...
    .sc-heading { font-family: var(--tk-font-heading); font-size: 12px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--tk-color-text-muted); border-bottom: 2px solid var(--tk-color-border); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-swatch-fill { height: 48px; border: 2px solid var(--tk-color-border); }
    ...
    .sc-radius-box { width: 132px; height: 72px; background: var(--tk-color-surface-2); border: 2px solid var(--tk-color-border); }
    .sc-radius-box-pill { height: 40px; margin-top: 16px; }
    .sc-shadow-box { width: 132px; height: 88px; background: var(--tk-color-surface); border: 2px solid var(--tk-color-border); display: flex; align-items: flex-end; padding: 8px; font-size: 11px; }
    ...
    .sc-overlay-demo { position: relative; height: 320px; overflow: hidden; }
...
<body>
${exNav('Retro Web', 'components', '')}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Retro Web</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Website internet lama era dial-up: bitmap font Silkscreen, WordArt pelangi, tombol bevel Win95, warna web-safe mencolok, kursor pixel custom, dan hit counter &mdash; aesthetic GeoCities 1997. Tersedia mode terang dan gelap (starfield). Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Silkscreen (heading bitmap pixel) &middot; Comic Neue (body ala Comic Sans) &middot; VT323 (caption &amp; label terminal) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<p class="tk-caption" style="margin-bottom: 12px">Semua sudut siku &mdash; radius 0 di seluruh kit; --tk-radius-full disisakan untuk radio dan avatar.</p><div class="sc-row">${RADII}</div>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>pixelarticons</code> &mdash; tulis <code>&lt;iconify-icon icon="pixelarticons:nama"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-row"><span class="tk-badge">Bitmap font</span><span class="tk-badge">WordArt pelangi</span><span class="tk-badge">Bevel Win95</span><span class="tk-badge tk-blink">GeoCities 1997</span></div><hr class="tk-hr-rainbow"><p class="tk-body-sm tk-muted" style="margin: 16px 0; max-width: 560px">Nostalgia internet sebelum CSS dewasa: heading bitmap Silkscreen, headline WordArt bergradasi pelangi dengan bayangan keras, tombol pelat bevel yang benar-benar tenggelam saat ditekan, tabel ber-border seperti <code class="tk-code">border="1"</code>, tautan biru bergaris bawah yang ungu setelah dikunjungi, kursor panah pixel, badge berkedip, teks marquee, dan hit counter hijau terminal. Mode gelap memindahkan semuanya ke langit starfield.</p><div class="sc-row" style="align-items: center; gap: 10px"><span class="tk-caption">Pengunjung ke-</span><span class="tk-hit-counter">004917</span></div>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.accent, backgroundColor: t.accent + '26', fill: true, stepped: true, pointRadius: 3, pointBackgroundColor: t.accent }] },
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: t.primary, borderRadius: 0 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
