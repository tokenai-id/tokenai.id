...
/**
 * Halaman contoh (`examples/`) kit Retro Web: admin dashboard dan landing page utuh yang
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="pixelarticons:${name}" width="${size}" height="${size}"></iconify-icon>`;
}

const HEAD_COMMON = `<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Silkscreen:wght@400;700&family=Comic+Neue:wght@400;700&family=VT323&display=swap" rel="stylesheet">
...
<title>Retro Web — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-title); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-sidebar-foot { margin-top: auto; display: block; padding-top: 8px; border-top: 2px solid var(--tk-color-border); }
    .dm-foot-btn { display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px; border: none; border-radius: 0; background: transparent; font-family: inherit; color: var(--tk-color-text); text-align: left; cursor: var(--tk-rw-cursor-hand); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-color-surface); border-bottom: 2px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 32px; height: 32px; border-radius: 0; background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 10px; font-weight: 700; flex: none; }
    ...
    .dm-activity-item iconify-icon { color: var(--tk-color-text-muted); margin-top: 2px; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Retro Web', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' } })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('monitor')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('chart-bar-big')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('chart')} <span class="tk-sidebar-label">Statistik</span></a>
        <span class="tk-sidebar-group">Lainnya</span>
        <a class="tk-sidebar-item" href="#">${ic('settings-cog')} <span class="tk-sidebar-label">Pengaturan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('info-box')} <span class="tk-sidebar-label">Bantuan</span></a>
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('chevrons-vertical', 16)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user', 16)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('settings-cog', 16)} Pengaturan</button>
    ...
    <div class="dm-main">
        ...
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('chevron-down', 16)}</button>
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-horizontal', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('user-plus', 16)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('file-text', 16)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('square-alert', 16)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.accent, backgroundColor: t.accent + '26', fill: true, stepped: true, pointRadius: 3, pointBackgroundColor: t.accent }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    ...
    return `<div class="tk-card lp-plan${featured ? ' lp-plan-featured' : ''}">
        <div class="tk-card-body">
            ${featured ? '<span class="tk-badge tk-badge-info tk-blink">Paling populer</span>' : ''}
...
<style>
    ...
    .lp-proof-stats { display: flex; gap: 32px; margin-top: 24px; }
    .lp-proof-stat .tk-stat-value { display: block; font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); }
    ...
    .lp-plan-featured { outline: 2px solid var(--tk-color-accent); box-shadow: var(--tk-shadow-lg); }
    .lp-price { font-family: var(--tk-font-heading); font-size: var(--tk-text-h2); font-weight: 700; margin: 0 0 8px; }
    .lp-price .tk-caption { font-weight: 400; letter-spacing: 0; }
    ...
    .lp-footer { border-top: 2px solid var(--tk-color-border); background: var(--tk-color-surface); }
    ...
    .lp-counter-wrap { display: flex; align-items: center; gap: 10px; }
    @media (max-width: 860px) {
        ...
        .lp-proof-stats { gap: 20px; flex-wrap: wrap; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Retro Web', 'landing', '../', { device: { base: 'landing', active: 'web' } })}
...
<div class="tk-marquee" aria-hidden="true"><span>&#9733;&#9733;&#9733; SELAMAT DATANG DI HOMEPAGE KAMI &#9733; BEST VIEWED IN 800x600 &#9733; JANGAN LUPA TANDATANGANI BUKU TAMU &#9733;&#9733;&#9733;</span></div>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-badge tk-badge-info tk-blink" style="margin-bottom: 16px">${ic('sparkles', 14)} BARU! versi 2.0 sudah rilis</span>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ${feature('zap', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
        ${feature('shield', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('chart-bar-big', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ...
        ${feature('sparkles', 'Otomatis', 'Rutinitas berulang dikerjakan oleh automasi.')}
    </div>
</section>
    ...
    <div class="lp-proof">
        <div>
            ...
            <div class="lp-proof-stats">
                <div class="lp-proof-stat"><span class="tk-stat-value">36rb</span><span class="tk-caption">Pengguna aktif</span></div>
                <div class="lp-proof-stat"><span class="tk-stat-value">99,9%</span><span class="tk-caption">Uptime</span></div>
                <div class="lp-proof-stat"><span class="tk-stat-value">4,8/5</span><span class="tk-caption">Rating</span></div>
            </div>
        </div>
...
<section class="lp-section" style="padding-top: 0" id="cta" aria-labelledby="lp-h-cta">
    <div class="tk-cta">
        ...
        <button class="tk-btn tk-btn-primary">Coba Sekarang ${ic('arrow-right', 16)}</button>
    </div>
</section>
</main>
<footer class="lp-footer">
    <div class="lp-footer-inner">
        ...
        <div class="lp-counter-wrap"><span class="tk-caption">Pengunjung ke-</span><span class="tk-hit-counter">004917</span></div>
        <div class="lp-footer-links">
            <a class="tk-navbar-link" href="#">Privasi</a>
            <a class="tk-navbar-link" href="#">Ketentuan</a>
            <a class="tk-navbar-link" href="#">Bantuan</a>
        </div>
    </div>
</footer>
${CHART_HELPER_SCRIPT}
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: t.accent, borderRadius: 0, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    .au-wrap { min-height: calc(100vh - 48px); box-sizing: border-box; display: flex; align-items: center; justify-content: center; padding: 40px 16px; background: var(--tk-color-surface-2); }
    .ex-embedded .au-wrap { min-height: 100vh; }
    .au-card { width: 100%; max-width: 400px; }
    .au-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-title); text-align: center; margin-bottom: 20px; }
    .au-title { text-align: center; margin: 0 0 4px; }
    .au-sub { text-align: center; margin: 0 0 20px; }
    .au-full { width: 100%; box-sizing: border-box; }
    .au-divider { display: flex; align-items: center; gap: 12px; margin: 16px 0; color: var(--tk-color-text-muted); font-size: var(--tk-text-caption); font-family: var(--tk-font-mono); text-transform: uppercase; }
    .au-divider::before, .au-divider::after { content: ''; height: 2px; flex: 1; background: var(--tk-color-border); }
    ...
    .au-link { color: var(--tk-color-accent); text-decoration: underline; font-weight: 700; }
    .au-link:hover { background: var(--tk-color-accent); color: #FFFFFF; text-decoration: none; }
...
<head>
${HEAD_COMMON}
<title>Retro Web — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Retro Web', 'components', '../', { device: { base, active: 'web' } })}
<div class="au-wrap">
 ...
 * benar-benar teruji — bukan tangkapan layar statis. Saklar tema di bilah atas menjangkau dokumen
 * di dalam bingkai karena keduanya berasal dari server pratinjau yang sama.
 */
...
<head>
${HEAD_COMMON}
<title>Retro Web — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
...
<body>
${exNav('Retro Web', view, '../', { device: { base, active: 'mobile' }, extraThemeJs: FRAME_THEME_EXTRA_JS })}
<div class="ex-stage">
...
export const RETRO_WEB_EXAMPLES: Record<string, string> = {
