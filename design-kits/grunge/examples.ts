...
/**
 * Halaman contoh (`examples/`) kit Grunge: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — mesin fotokopi hanya kenal kertas), jadi semua
 * halaman memanggil exNav dengan `themeToggle: false`.
 *
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="feather:${name}" width="${size}" height="${size}"></iconify-icon>`;
}

const HEAD_COMMON = `<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Rubik+Distressed&family=Archivo:wght@400;500;600;700&family=Special+Elite&display=swap" rel="stylesheet">
...
<title>Grunge — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-size: var(--tk-text-title); text-transform: uppercase; padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-foot-btn:hover { background: rgba(28, 26, 23, 0.06); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-color-surface); border-bottom: 2px solid var(--tk-color-primary); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 32px; height: 32px; border-radius: var(--tk-radius-sm); border: 2px solid var(--tk-color-primary); background: var(--tk-color-surface-2); color: var(--tk-color-text); display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-mono); font-size: 11px; flex: none; transform: rotate(-2deg); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Grunge', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('grid')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('bar-chart-2')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('trending-up')} <span class="tk-sidebar-label">Statistik</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('chevron-down', 16)}</span>
            </button>
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-horizontal', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit-2', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash-2', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '1A', fill: true, tension: 0.15, pointRadius: 4, pointBackgroundColor: t.primary }]
        },
...
tkChart('dm-chart-traffic', function (t) {
    return {
        type: 'doughnut',
        data: { labels: ['Direct', 'Organik', 'Sosial', 'Referral'], datasets: [{ data: [38, 31, 19, 12], backgroundColor: [t.primary, '#9E2B1E', t.accent, '#6E6659'], borderColor: t.surface2, borderWidth: 2 }] },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    ...
    return `<div class="tk-card lp-plan${featured ? ' lp-plan-featured' : ''}">
        <div class="tk-card-body">
            ${featured ? '<span class="tk-stamp" style="font-size: 12px">Paling populer</span>' : ''}
...
<style>
    ...
    .lp-plan-featured { border: 2px solid var(--tk-color-secondary); box-shadow: 5px 5px 0 rgba(158, 43, 30, 0.4); transform: rotate(-0.6deg); }
    ...
    .lp-footer { border-top: 3px solid var(--tk-color-primary); background: var(--tk-color-surface); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Grunge', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-stamp" style="margin-bottom: 20px">Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display tk-xerox" id="lp-h-hero">Kelola proyek<br><span class="tk-marker">tanpa drama</span></h1>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    <h2 class="tk-h2 lp-section-title tk-xerox" id="lp-h-fitur">Semua yang tim Anda <span class="tk-marker">butuhkan</span></h2>
    ...
    <div class="tk-feature-grid lp-features">
        ...
        ${feature('bar-chart-2', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ...
        ${feature('rotate-ccw', 'Otomatis', 'Rutinitas berulang dikerjakan oleh automasi.')}
    </div>
</section>
    ...
    <div class="lp-proof">
        <div>
            ...
            <div class="lp-proof-stats">
                <div class="lp-proof-stat"><span class="tk-archive-tag">36RB</span><span class="tk-caption">Pengguna aktif</span></div>
                <div class="lp-proof-stat"><span class="tk-archive-tag">99,9%</span><span class="tk-caption">Uptime</span></div>
                <div class="lp-proof-stat"><span class="tk-archive-tag">4,8/5</span><span class="tk-caption">Rating</span></div>
            </div>
        </div>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#6E6659', t.accent, '#9E2B1E', t.primary], borderRadius: 2, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-size: var(--tk-text-title); text-transform: uppercase; text-align: center; margin-bottom: 20px; }
    ...
    .au-link:hover { text-decoration: underline; text-decoration-thickness: 2px; text-underline-offset: 3px; }
...
<head>
${HEAD_COMMON}
<title>Grunge — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Grunge', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Grunge — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #1C1A17; border-radius: 40px; background: #1C1A17; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 28px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Grunge', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const GRUNGE_EXAMPLES: Record<string, string> = {
