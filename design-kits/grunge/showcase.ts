...
/**
 * `showcase.html` kit Grunge: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const SWATCHES = [
    ['primary', '#1C1A17'], ['primary-hover', '#3A362F'], ['secondary', '#9E2B1E'],
    ['accent', '#A8821F'], ['background', '#E4DFD0'], ['surface-2', '#DCD5C2'],
    ['text', '#211E1A'], ['text-muted', '#6E6659'], ['border', '#A79D88'],
    ['success', '#4A6B3A'], ['warning', '#A87B1C'], ['danger', '#8F2318'], ['info', '#3E5E75']
].map(([name, hex]) =>
...
const TYPE_SCALE = [
    ...
    ['Heading 3', 'tk-h3', '20px'], ['Heading 4', 'tk-h4', '16px'], ['Title', 'tk-title', '15px'],
    ['Body', 'tk-body', '16px'], ['Body Small', 'tk-body-sm', '14px'], ['Caption', 'tk-caption', '12px']
].map(([label, cls, size]) =>
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Difotokopi sampai pecah</p></div>`
).join('');
...
const ICONS = ['home', 'search', 'user', 'settings', 'bell', 'heart', 'star', 'trash-2', 'edit-2',
    'check', 'x', 'plus', 'arrow-right', 'download', 'upload', 'calendar', 'mail', 'lock']
    .map(name => `<span class="sc-icon" title="feather:${name}"><iconify-icon icon="feather:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const GRUNGE_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Grunge — Design Kit Showcase</title>
...
<style>
    ...
    .sc-kit-title { font-family: var(--tk-font-mono); font-size: 13px; letter-spacing: 0.14em; text-transform: uppercase; color: var(--tk-color-secondary); margin: 0 0 4px; }
    .sc-section { margin-top: 56px; }
    .sc-heading { font-family: var(--tk-font-mono); font-size: 12px; font-weight: 400; letter-spacing: 0.14em; text-transform: uppercase; color: var(--tk-color-secondary); border-bottom: 2px solid var(--tk-color-primary); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-space-bar { height: 16px; background: rgba(158, 43, 30, 0.15); border: 1px solid var(--tk-color-secondary); border-radius: 2px; }
    ...
    .sc-xerox-demo { font-family: var(--tk-font-heading); font-size: 38px; text-transform: uppercase; margin: 0; }
    .sc-photo-row { display: flex; gap: 20px; align-items: flex-end; flex-wrap: wrap; }
    .sc-photo { width: 150px; height: 100px; border: 1px solid var(--tk-color-primary); border-radius: var(--tk-radius-sm); background: linear-gradient(135deg, #C05A3A 0%, #4A7A9E 45%, #6B9E4A 100%); }
    .sc-photo-mono { filter: grayscale(0.92) contrast(1.18) brightness(1.02); box-shadow: var(--tk-shadow-sm); }
    .sc-torn-demo { background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); padding: 24px 24px 36px; max-width: 420px; }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Grunge', 'components', '', { themeToggle: false })}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Grunge</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Zine fotokopi 90an: kertas kotor bernoda dengan grain dan goresan halus di seluruh halaman, tinta hitam pekat dan merah karat, heading Rubik Distressed yang tergerus, label mesin tik Special Elite, badge stempel karet miring, cetakan ganda xerox yang meleset, tepi sobek, dan fotografi desaturated kontras tinggi. Kit satu-tema &mdash; terang secara bawaan, tanpa mode gelap. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Rubik Distressed (heading tergerus) &middot; Archivo (body) &middot; Special Elite (caption &amp; label mesin tik) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<div class="sc-row">${RADII}</div><p class="tk-caption" style="margin-top: 12px">Sudut nyaris siku &mdash; kertas dipotong cutter, bukan dibulatkan.</p>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div><p class="tk-caption" style="margin-top: 12px">Bayangan cetak keras <code>3px 3px 0</code> tinta &mdash; kertas ditumpuk, bukan melayang.</p>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>feather</code> &mdash; tulis <code>&lt;iconify-icon icon="feather:nama"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-row"><span class="tk-stamp">Approved</span><span class="tk-stamp tk-stamp-ink">Top Secret</span><span class="tk-archive-tag">DOC-1994/07</span></div><p class="sc-xerox-demo tk-xerox" style="margin: 24px 0 0">Fotokopi generasi ketiga</p><p class="tk-h3" style="margin: 8px 0 0">Sorot dengan <span class="tk-marker">coretan spidol karat</span></p><div class="sc-photo-row" style="margin: 24px 0"><div><div class="sc-photo"></div><p class="tk-caption" style="margin: 6px 0 0">Sumber berwarna</p></div><div><div class="sc-photo sc-photo-mono"></div><p class="tk-caption" style="margin: 6px 0 0">Perlakuan kit: desaturated + kontras</p></div></div><div class="sc-torn-demo tk-torn"><p class="tk-title" style="margin: 0; color: var(--tk-color-primary-contrast)">Tepi sobek</p><p class="tk-body-sm" style="margin: 4px 0 0; color: #B5AC97">Kelas <code class="tk-code">.tk-torn</code> merobek tepi bawah panel mana pun.</p></div><p class="tk-body-sm tk-muted" style="margin: 16px 0 0; max-width: 560px">Estetika zine yang difotokopi sampai pecah: grain dan goresan menutupi seluruh halaman (otomatis lewat <code class="tk-code">body::after</code>), cetakan ganda yang meleset (<code class="tk-code">.tk-xerox</code>), stempel karet miring (<code class="tk-code">.tk-stamp</code>), coretan spidol (<code class="tk-code">.tk-marker</code>), label arsip mesin tik (<code class="tk-code">.tk-archive-tag</code>), tepi sobek (<code class="tk-code">.tk-torn</code>), dan semua foto otomatis desaturated kontras tinggi. Tidak ada mode gelap &mdash; mesin fotokopi hanya kenal kertas.</p>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.primary, backgroundColor: t.primary + '1A', fill: true, tension: 0.15, pointRadius: 4, pointBackgroundColor: t.primary }] },
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: '#9E2B1E', borderRadius: 2 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
