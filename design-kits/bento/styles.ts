...
/**
 * `styles.css` kit Bento.
 *
 * Estetika bento grid: halaman dibagi menjadi kotak-kotak modular seperti bento box —
 * kartu membulat besar (radius 16–24px) tersusun rapat di atas latar abu lembut, dipisah
 * border halus dan bayangan ambient tipis. Grid fitur asimetris (tile sorotan melebar dua
 * kolom dengan latar pastel), tombol dan chip berbentuk pil, judul hero bergradien
 * indigo→pink. Sangat cocok untuk dashboard, SaaS, startup, portfolio, dan landing page
 * karena tiap jenis informasi mendapat kotaknya sendiri. Struktur selektor dan blok
 * `== tk: ... ==` mengikuti kontrak `docs/kontrak-kit-design.md`; wajahnya lahir dari
 * nilai token plus blok "flavor" di ekor berkas.
 */
export const BENTO_STYLES = `/* Bento — kit design TOKENAI.
   Estetika bento grid: kotak modular membulat besar di latar lembut, border halus,
   bayangan ambient tipis, grid asimetris.
   Kontrak: docs/kontrak-kit-design.md. Semua nilai design tinggal di :root. */

/* == tk: tokens ================================================= */
:root {
    --tk-color-primary: #4F46E5;
    --tk-color-primary-hover: #4338CA;
    --tk-color-primary-contrast: #FFFFFF;
    --tk-color-secondary: #FFFFFF;
    --tk-color-secondary-hover: #F2F3F7;
    --tk-color-secondary-contrast: #17181C;
    --tk-color-accent: #EC4899;
    --tk-color-background: #F2F3F5;
    --tk-color-surface: #FFFFFF;
    --tk-color-surface-2: #F6F7F9;
    --tk-color-text: #17181C;
    --tk-color-text-muted: #62666E;
    --tk-color-border: #E4E6EA;
    --tk-color-success: #16A34A;
    --tk-color-success-soft: #E9F7EE;
    --tk-color-warning: #B45309;
    --tk-color-warning-soft: #FCF1DF;
    --tk-color-danger: #DC2626;
    --tk-color-danger-soft: #FCE9E9;
    --tk-color-info: #2563EB;
    --tk-color-info-soft: #E8EFFD;

    --tk-font-heading: 'Manrope', system-ui, sans-serif;
    ...
    /* Radius besar adalah tanda tangan bento: setiap kotak terasa seperti sekat bento box. */
    --tk-radius-sm: 12px;
    --tk-radius: 16px;
    --tk-radius-lg: 24px;
    ...
    --tk-border-width: 1px;
    --tk-shadow-sm: 0 1px 2px rgba(23, 24, 28, 0.05);
    --tk-shadow: 0 8px 24px rgba(23, 24, 28, 0.07);
    --tk-shadow-lg: 0 20px 48px rgba(23, 24, 28, 0.12);

    --tk-transition: 160ms ease;
    --tk-container: 1200px;
}

/* Mode gelap: bento malam — latar arang biru, kotak-kotak abu gelap dengan border samar,
   indigo menyala lebih terang. Aktifkan dengan atribut data-tk-theme="dark" pada <html>
   atau <body>. */
[data-tk-theme="dark"] {
    --tk-color-primary: #6366F1;
    --tk-color-primary-hover: #7C86F8;
    --tk-color-primary-contrast: #FFFFFF;
    --tk-color-secondary: #1F2128;
    --tk-color-secondary-hover: #262932;
    --tk-color-secondary-contrast: #F2F3F5;
    --tk-color-accent: #F472B6;
    --tk-color-background: #0E0F13;
    --tk-color-surface: #17181C;
    --tk-color-surface-2: #1F2128;
    --tk-color-text: #F2F3F5;
    --tk-color-text-muted: #9AA0AC;
    --tk-color-border: #262932;
    --tk-color-success: #4ADE80;
    --tk-color-success-soft: #12291B;
    --tk-color-warning: #FBBF24;
    --tk-color-warning-soft: #2E2312;
    --tk-color-danger: #F87171;
    --tk-color-danger-soft: #321616;
    --tk-color-info: #60A5FA;
    --tk-color-info-soft: #131F33;
    --tk-shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.4);
    --tk-shadow: 0 8px 24px rgba(0, 0, 0, 0.45);
    --tk-shadow-lg: 0 20px 48px rgba(0, 0, 0, 0.55);
}
...
.tk-display { font-size: var(--tk-text-display); letter-spacing: -0.03em; }
.tk-h1 { font-size: var(--tk-text-h1); }
.tk-h2 { font-size: var(--tk-text-h2); }
.tk-h3 { font-size: var(--tk-text-h3); }
.tk-h4 { font-size: var(--tk-text-h4); }
.tk-title { font-size: var(--tk-text-title); font-weight: 700; font-family: var(--tk-font-heading); margin: 0 0 var(--tk-space-sm); }
...
.tk-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: var(--tk-shadow); }
.tk-btn:not(:disabled):active { transform: translateY(0) scale(0.98); box-shadow: none; }
...
.tk-btn-secondary { background: var(--tk-color-secondary); color: var(--tk-color-secondary-contrast); border-color: var(--tk-color-border); box-shadow: var(--tk-shadow-sm); }
...
.tk-btn-outline:hover:not(:disabled) { background: var(--tk-color-surface); border-color: var(--tk-color-text-muted); }
...
.tk-btn-text:hover:not(:disabled) { transform: none; box-shadow: none; text-decoration: underline; text-underline-offset: 3px; }
...
.tk-field-success .tk-input { border-color: var(--tk-color-success); box-shadow: 0 0 0 3px rgba(22, 163, 74, 0.14); }
...
.tk-option-selected { background: var(--tk-color-info-soft); font-weight: 600; }
...
.tk-toggle { position: relative; width: 42px; height: 24px; flex: none; margin: 0; appearance: none; border: 1.5px solid var(--tk-color-border); background: var(--tk-color-surface-2); border-radius: var(--tk-radius-full); cursor: pointer; transition: background var(--tk-transition), border-color var(--tk-transition); }
...
.tk-dropdown-label { padding: 6px 12px; font-family: var(--tk-font-mono); font-size: var(--tk-text-caption); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: var(--tk-color-text-muted); }
.tk-dropdown-item {
    display: flex;
    align-items: center;
    gap: var(--tk-space-sm);
...
.tk-navbar-link { color: var(--tk-color-text-muted); text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 600; padding: 7px 14px; border-radius: var(--tk-radius-full); }
.tk-navbar-link:hover { color: var(--tk-color-text); background: var(--tk-color-surface-2); }
.tk-navbar-link-active { color: var(--tk-color-primary); background: var(--tk-color-info-soft); }
.tk-navbar-dark { background: #0E0F13; border-bottom-color: #262932; }
.tk-navbar-dark .tk-navbar-brand { color: #F2F3F5; }
.tk-navbar-dark .tk-navbar-link { color: #9AA0AC; }
.tk-navbar-dark .tk-navbar-link:hover { color: #F2F3F5; background: #1F2128; }
.tk-navbar-dark .tk-navbar-link-active { color: #FFFFFF; background: #262932; }
.tk-sidebar {
    width: 240px;
    background: var(--tk-color-surface);
...
.tk-sidebar-item-active { background: var(--tk-color-info-soft); color: var(--tk-color-primary); }
...
/* == tk: tabs =================================================== */
.tk-tabs { display: flex; gap: var(--tk-space-xs); border-bottom: var(--tk-border-width) solid var(--tk-color-border); }
.tk-tab {
    padding: var(--tk-space-sm) 14px 12px;
    font-size: var(--tk-text-body-sm);
    font-weight: 600;
    color: var(--tk-color-text-muted);
    text-decoration: none;
...
.tk-segmented { display: inline-flex; background: var(--tk-color-surface-2); border: var(--tk-border-width) solid var(--tk-color-border); border-radius: var(--tk-radius-full); padding: 3px; gap: 3px; }
.tk-segment {
    padding: 6px 14px;
    font-size: var(--tk-text-body-sm);
    font-weight: 600;
...
.tk-progress { height: 10px; background: var(--tk-color-surface-2); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius-full); overflow: hidden; }
...
.tk-feature-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--tk-space-md); }
...
.tk-cta {
    background: linear-gradient(135deg, var(--tk-color-primary), var(--tk-color-accent));
    color: #FFFFFF;
    border-radius: var(--tk-radius-lg);
...
.tk-cta .tk-muted { color: #FFFFFF; opacity: 0.85; }
.tk-cta .tk-btn-primary { background: #FFFFFF; color: var(--tk-color-primary); }

/* == tk: layout ================================================= */
.tk-grid { display: grid; gap: var(--tk-space-md); }
...
/* == tk: bento flavor =========================================== */
/* Penimpa karakter bento di atas struktur kontrak: menang kaskade karena berada di ekor
   berkas. Tanda tangan kit ini — grid fitur menjadi susunan bento box asimetris: setiap
   fitur adalah kotak membulat bersekat, tile pertama dan keempat melebar dua kolom dengan
   latar pastel sebagai sorotan, dan seluruh tile terangkat halus saat disentuh kursor. */
.tk-feature-grid { grid-auto-flow: dense; }
.tk-feature {
    background: var(--tk-color-surface);
    border: var(--tk-border-width) solid var(--tk-color-border);
    border-radius: var(--tk-radius-lg);
...
.tk-feature:hover { transform: translateY(-3px); box-shadow: var(--tk-shadow); }
/* Tile lebar hanya muncul bila fiturnya cukup banyak (guard :nth-last-child), supaya grid
   3 fitur dari arketipe scaffolder tetap simetris — tanpa tile yatim di baris kedua. */
.tk-feature:first-child:nth-last-child(n+4) { grid-column: span 2; background: var(--tk-color-info-soft); border-color: transparent; }
.tk-feature:nth-child(4):nth-last-child(n+3) { grid-column: span 2; background: var(--tk-color-warning-soft); border-color: transparent; }
.tk-feature:first-child:nth-last-child(n+4) .tk-feature-icon, .tk-feature:nth-child(4):nth-last-child(n+3) .tk-feature-icon { background: var(--tk-color-surface); }
@media (max-width: 860px) {
    .tk-feature:first-child, .tk-feature:nth-child(4) { grid-column: auto; }
}
/* Judul hero: gradien indigo ke pink, aksen SaaS modern. */
.tk-hero .tk-display {
    background: linear-gradient(120deg, var(--tk-color-text) 30%, var(--tk-color-primary) 70%, var(--tk-color-accent));
    -webkit-background-clip: text;
    background-clip: text;
    -webkit-text-fill-color: transparent;
    color: transparent;
}
/* Gambar konten ikut membulat seperti sekat bento. */
.tk-card img, .tk-hero img, figure img { border-radius: var(--tk-radius-sm); }
`;
