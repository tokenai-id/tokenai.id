...
/**
 * Halaman contoh (`examples/`) kit Punk: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — poster ditempel di tiang listrik, bukan di layar),
 * jadi semua halaman memanggil exNav dengan `themeToggle: false`.
 *
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="la:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Punk — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-size: var(--tk-text-title); text-transform: uppercase; transform: skew(-4deg); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-sidebar-foot { margin-top: auto; display: block; padding-top: 8px; border-top: 2px solid var(--tk-color-primary); }
    ...
    .dm-foot-btn:hover { background: var(--tk-color-accent); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-color-surface); border-bottom: 3px solid var(--tk-color-primary); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 32px; height: 32px; border-radius: var(--tk-radius-sm); border: 2px solid var(--tk-color-primary); background: var(--tk-color-secondary); color: #FFF; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-mono); font-weight: 700; font-size: 11px; flex: none; transform: rotate(-3deg); }
    ...
    .dm-activity-item iconify-icon { color: var(--tk-color-secondary); margin-top: 2px; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Punk', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('th-large')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            ...
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                ...
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('sign-out-alt', 16)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '14', fill: true, tension: 0, pointRadius: 5, pointBackgroundColor: '#E31577', borderWidth: 3 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    ...
    return `<div class="tk-card lp-plan${featured ? ' lp-plan-featured' : ''}">
        <div class="tk-card-body">
            ${featured ? '<span class="tk-patch tk-patch-pink" style="font-size: 12px">Paling populer</span>' : ''}
...
<style>
    ...
    .lp-plan-featured { border: 3px solid var(--tk-color-primary); box-shadow: 6px 6px 0 var(--tk-color-secondary); transform: rotate(-1deg); }
    .lp-price { font-family: var(--tk-font-heading); font-size: var(--tk-text-h2); margin: 0 0 8px; }
    ...
    .lp-footer { border-top: 3px solid var(--tk-color-primary); box-shadow: inset 0 3px 0 var(--tk-color-secondary); background: var(--tk-color-surface); }
    ...
    .lp-footer-links a:hover { color: var(--tk-color-secondary); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Punk', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-patch tk-patch-yellow" style="margin-bottom: 24px">Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display tk-ransom" id="lp-h-hero"><span>Kelola</span> <span>proyek</span> <span>tanpa</span> <span>drama</span></h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda — <span class="tk-scrawl">tanpa rapat yang tidak perlu!</span></p>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    <h2 class="tk-h2 lp-section-title" id="lp-h-fitur">Semua yang tim Anda <span class="tk-highlight">butuhkan</span></h2>
    ...
    <div class="tk-feature-grid lp-features">
        ...
        ${feature('shield-alt', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ...
        ${feature('bullhorn', 'Otomatis', 'Rutinitas berulang dikerjakan oleh automasi.')}
    </div>
</section>
    ...
    <div class="lp-proof">
        <div>
            ...
            <div class="lp-proof-stats">
                <div class="lp-proof-stat"><span class="tk-patch">36RB</span><span class="tk-caption">Pengguna aktif</span></div>
                <div class="lp-proof-stat"><span class="tk-patch tk-patch-pink">99,9%</span><span class="tk-caption">Uptime</span></div>
                <div class="lp-proof-stat"><span class="tk-patch tk-patch-yellow">4,8/5</span><span class="tk-caption">Rating</span></div>
            </div>
        </div>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#5F5B52', t.accent, '#E31577', t.primary], borderRadius: 0, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-size: var(--tk-text-title); text-transform: uppercase; transform: skew(-4deg); text-align: center; margin-bottom: 20px; }
    ...
    .au-divider::before, .au-divider::after { content: ''; height: 2px; flex: 1; background: var(--tk-color-primary); }
    ...
    .au-link { color: var(--tk-color-secondary); text-decoration: none; font-weight: 700; }
    .au-link:hover { background: var(--tk-color-accent); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Punk', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<style>
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #141312; border-radius: 40px; background: #141312; box-shadow: var(--tk-pk-shadow-pink); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 28px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Punk', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const PUNK_EXAMPLES: Record<string, string> = {
