...
/**
 * `showcase.html` kit Clay: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const SWATCHES = [
    ['primary', '#6C5CE7'], ['primary-hover', '#5A4BD1'], ['secondary', '#F97AA6'],
    ['accent', '#FFC75F'], ['background', '#F2EFFB'], ['surface-2', '#E9E4F6'],
    ['text', '#3A3555'], ['text-muted', '#7A7593'], ['border', '#E2DCF2'],
    ['success', '#3FB77E'], ['warning', '#E8A13C'], ['danger', '#F06A5D'], ['info', '#5AA7E8']
].map(([name, hex]) =>
...
const TYPE_SCALE = [
    ...
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Lembut seperti clay</p></div>`
).join('');
...
const ICONS = ['estate', 'search', 'user', 'setting', 'bell', 'heart', 'star', 'trash-alt', 'edit',
    'check', 'times', 'plus', 'arrow-right', 'download-alt', 'upload', 'calendar-alt', 'envelope', 'lock']
    .map(name => `<span class="sc-icon" title="uil:${name}"><iconify-icon icon="uil:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const CLAY_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Clay — Design Kit Showcase</title>
...
<style>
    ...
    .sc-kit-title { font-size: 13px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; color: var(--tk-color-primary); margin: 0 0 4px; }
    .sc-section { margin-top: 56px; }
    .sc-heading { font-family: var(--tk-font-body); font-size: 12px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; color: var(--tk-color-primary); border-bottom: 2px solid var(--tk-color-surface-2); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-swatch-fill { height: 48px; border-radius: var(--tk-radius-sm); box-shadow: inset 0 -4px 8px rgba(0, 0, 0, 0.12), inset 0 4px 8px rgba(255, 255, 255, 0.4); }
    ...
    .sc-space-bar { height: 16px; background: var(--tk-color-primary); opacity: 0.35; border-radius: var(--tk-radius-full); }
    ...
    .sc-radius-box { width: 132px; height: 72px; background: var(--tk-color-surface); box-shadow: var(--tk-shadow-sm); }
    .sc-radius-box-pill { height: 40px; margin-top: 16px; }
    .sc-shadow-box { width: 132px; height: 88px; background: var(--tk-color-surface); border-radius: var(--tk-radius); display: flex; align-items: flex-end; padding: 8px; font-size: 11px; }
    .sc-shadow-box code { color: var(--tk-color-text-muted); font-family: var(--tk-font-mono); }
    .sc-icon { color: var(--tk-color-text); padding: 8px; }
    .sc-cell { background: var(--tk-color-surface-2); border-radius: var(--tk-radius-sm); height: 48px; box-shadow: var(--tk-cl-pressed); }
    ...
    .sc-ball-row { display: flex; gap: 32px; align-items: center; flex-wrap: wrap; padding: 12px 0; }
    .sc-clay-demo { display: flex; gap: 24px; flex-wrap: wrap; margin-top: 24px; }
    .sc-clay-demo > div { width: 150px; height: 96px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: var(--tk-color-text-muted); }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Clay', 'components', '', { themeToggle: false })}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Clay</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Claymorphism: kartu dan objek terlihat seperti tanah liat atau plastik lembut yang bisa dipencet &mdash; gembur, bulat, 3D, dan playful. Satu resep relief dipakai di mana-mana: bayangan luar berwarna, bibir atas menangkap cahaya, perut bawah menggelap. Palet permen lavender-pink-kuning di atas lila pucat, tipografi bulat Baloo 2 + Quicksand. Kit satu-tema &mdash; terang secara bawaan, tanpa mode gelap. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Baloo 2 (heading bulat gembur) &middot; Quicksand (body membulat) &middot; Red Hat Mono (kode &amp; kbd) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<div class="sc-row">${RADII}</div><p class="tk-caption" style="margin-top: 12px">Radius minimum 16px &mdash; tidak ada satu pun sudut tajam; clay tidak pernah bersudut.</p>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div><p class="tk-caption" style="margin-top: 12px">Relief clay tiga-lapis: bayangan luar lavender + inset terang di bibir atas + inset gelap di perut bawah.</p>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>uil</code> (Unicons) &mdash; tulis <code>&lt;iconify-icon icon="uil:nama"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-ball-row"><span class="tk-clay-ball"></span><span class="tk-clay-ball tk-clay-ball-pink"></span><span class="tk-clay-ball tk-clay-ball-yellow"></span></div><div class="sc-clay-demo"><div class="tk-clay">.tk-clay (pop)</div><div class="tk-clay-pressed">.tk-clay-pressed</div></div><p class="tk-body-sm tk-muted" style="margin: 24px 0 0; max-width: 560px">Semuanya terbuat dari clay yang sama: tombol menonjol lalu penyok saat ditekan, input dipencet ke dalam permukaan, checkbox dan radio adalah bola clay mini yang mengembang saat terisi, bola clay permen melayang pelan di hero (animasi <code class="tk-code">tk-bob</code>), dan ikon fitur bergiliran warna lavender &rarr; pink &rarr; kuning otomatis. Utilitas <code class="tk-code">.tk-clay</code> / <code class="tk-code">.tk-clay-pressed</code> menerapkan relief itu ke elemen apa pun, dan <code class="tk-code">.tk-clay-ball</code> (varian <code class="tk-code">-pink</code> / <code class="tk-code">-yellow</code>) adalah bola dekoratifnya. Tidak ada mode gelap &mdash; clay dipajang di rak yang terang.</p>`)}
${section('Buttons', `<div class="sc-row">${c['components/button.html']}</div>`)}
${section('Input & Form', `<div class="sc-col">${c['components/form.html']}</div><p class="tk-caption" style="margin: 24px 0 12px">Search bar &mdash; ikon dan pintasan menyatu di dalam field</p><div style="max-width: 480px">${c['components/search.html']}</div><p class="tk-caption" style="margin: 24px 0 12px">Select custom (ditampilkan terbuka) &mdash; satu pilihan &middot; dengan pencarian &middot; multi dengan checkbox &amp; aksi. Listbox digambar kit, bukan popup bawaan browser.</p><div class="sc-row" style="align-items: flex-start; gap: 24px; min-height: 340px">${c['components/select.html'].split('<div class="tk-select">').join('<div class="tk-select tk-select-open" style="width: 250px">')}</div><p class="tk-caption" style="margin: 0 0 12px">Dropdown aksi (ditampilkan terbuka; aslinya terbuka saat trigger diklik)</p><div style="min-height: 240px">${c['components/dropdown.html'].replace('class="tk-dropdown"', 'class="tk-dropdown tk-dropdown-open"')}</div><p class="tk-caption">Semua kontrol digambar sendiri &mdash; select, checkbox bola clay, radio, dan toggle tidak memakai tampilan bawaan browser.</p>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.primary, backgroundColor: t.primary + '24', fill: true, tension: 0.4, pointRadius: 5, pointBackgroundColor: '#F97AA6', pointBorderColor: '#FFF', pointBorderWidth: 2, borderWidth: 4 }] },
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: '#F97AA6', borderRadius: 14 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
