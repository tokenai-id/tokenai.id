...
/**
 * Halaman contoh (`examples/`) kit Clay: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — clay dipajang di rak yang terang), jadi semua halaman
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="uil:${name}" width="${size}" height="${size}"></iconify-icon>`;
}

const HEAD_COMMON = `<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Baloo+2:wght@600;700;800&family=Quicksand:wght@500;600;700&family=Red+Hat+Mono:wght@400;500&display=swap" rel="stylesheet">
...
<title>Clay — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-foot { margin-top: auto; display: block; padding-top: 8px; border-top: 2px solid var(--tk-color-surface-2); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-color-surface); box-shadow: 0 8px 20px rgba(108, 92, 231, 0.08); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 36px; height: 36px; border-radius: var(--tk-radius-full); background: var(--tk-color-secondary); box-shadow: var(--tk-cl-pop-pink); color: #FFF; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 800; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Clay', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('apps')} <span class="tk-sidebar-label">Dashboard</span></a>
        <a class="tk-sidebar-item" href="#">${ic('users-alt')} <span class="tk-sidebar-label">Pengguna</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('chart-line')} <span class="tk-sidebar-label">Statistik</span></a>
        <span class="tk-sidebar-group">Lainnya</span>
        <a class="tk-sidebar-item" href="#">${ic('setting')} <span class="tk-sidebar-label">Pengaturan</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('angle-down', 16)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user', 16)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('setting', 16)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('signout', 16)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('angle-right', 14)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('angle-right', 14)}</span>
        ...
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('angle-down', 16)}</button>
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('ellipsis-h', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash-alt', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        ...
                        <div class="dm-activity-item">${ic('exclamation-triangle', 16)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '24', fill: true, tension: 0.4, pointRadius: 5, pointBackgroundColor: '#F97AA6', pointBorderColor: '#FFF', pointBorderWidth: 2, borderWidth: 4 }]
        },
...
tkChart('dm-chart-traffic', function (t) {
    return {
        type: 'doughnut',
        data: { labels: ['Direct', 'Organik', 'Sosial', 'Referral'], datasets: [{ data: [38, 31, 19, 12], backgroundColor: [t.primary, '#F97AA6', '#FFC75F', '#CBC3E8'], borderColor: t.surface2, borderWidth: 4, borderRadius: 10 }] },
...
<style>
    ...
    .lp-proof-stat { background: var(--tk-color-surface); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm); padding: 14px 22px; }
    ...
    /* Paket unggulan mengembang lebih besar — clay yang paling banyak dipencet. */
    .lp-plan-featured { box-shadow: var(--tk-shadow-lg); transform: scale(1.045); }
    .lp-plan-featured:hover { transform: scale(1.045) translateY(-4px); }
    ...
    .lp-footer { background: var(--tk-color-surface); box-shadow: 0 -10px 26px rgba(108, 92, 231, 0.08); }
    ...
    @media (max-width: 860px) {
        ...
        .tk-hero::before { width: 90px; height: 90px; right: -20px; }
        .tk-hero::after { width: 70px; height: 70px; left: -16px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Clay', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('bars', 20)}</button>
</nav>
</header>
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    ...
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; selembut memencet clay.</p>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ...
        ${feature('users-alt', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('plug', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('smile', 'Menyenangkan', 'Antarmuka playful yang bikin kerja terasa ringan.')}
    </div>
</section>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: [t.surface2, '#FFC75F', '#F97AA6', t.primary], borderRadius: 14, maxBarThickness: 58 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-divider { display: flex; align-items: center; gap: 12px; margin: 16px 0; color: var(--tk-color-text-muted); font-size: var(--tk-text-caption); font-weight: 600; }
    .au-divider::before, .au-divider::after { content: ''; height: 2px; flex: 1; background: var(--tk-color-surface-2); border-radius: var(--tk-radius-full); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Clay', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<style>
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #3A3555; border-radius: 48px; background: #3A3555; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Clay', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const CLAY_EXAMPLES: Record<string, string> = {
