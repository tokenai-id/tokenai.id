...
/**
 * Fragmen `components/` kit Luxury Editorial: markup murni dengan kelas kontrak, teks
 * placeholder netral, ikon Iconify (set `material-symbols-light` — Material Symbols pada bobot
 * paling tipis, garis 1px yang senada dengan serif kontras tinggi kit ini). Gayanya seluruhnya
 * di `styles.css` — fragmen inilah yang disalin agen saat merakit halaman, jadi jangan menaruh
 * atribut style di sini.
 *
...
export const LUXURY_EDITORIAL_COMPONENTS: Record<string, string> = {
    'components/typography.html': `<p class="tk-eyebrow">Rubrik</p>
<h1 class="tk-h1">Judul Halaman</h1>
<h2 class="tk-h2">Judul Seksi</h2>
<h3 class="tk-h3">Judul Kartu</h3>
<h4 class="tk-h4">Subjudul</h4>
<p class="tk-title">Title / Subtitle</p>
<div class="tk-prose">
    <p class="tk-body tk-drop-cap">Paragraf pembuka dengan drop cap: huruf pertamanya diturunkan tiga baris seperti pembuka artikel cetak. Teks isi biasa memuat <a href="#" class="tk-link">tautan</a> dan <code class="tk-code">kode inline</code>.</p>
    <blockquote class="tk-quote">Kutipan bergaya serif miring, dibuka satu garis emas di sisi kiri.</blockquote>
</div>
<p class="tk-body-sm tk-muted">Teks kecil sekunder.</p>
<p class="tk-caption">Caption 12px berjarak lebar.</p>
<hr class="tk-rule">
`,
...
<button class="tk-btn tk-btn-outline tk-btn-icon" aria-label="Tambah">${icon('add')}</button>
`,
...
<div class="tk-field">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih satu opsi</span> ${icon('keyboard-arrow-down', 18)}</button>
...
`,
    ...
    <button class="tk-select-trigger" type="button">Opsi pertama ${icon('keyboard-arrow-down', 18)}</button>
    <div class="tk-select-menu">
...
<div class="tk-select">
    <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih anggota</span> ${icon('keyboard-arrow-down', 18)}</button>
    <div class="tk-select-menu">
        <div class="tk-select-menu-search">${icon('search', 16)}<input type="text" placeholder="Cari anggota..."></div>
...
<div class="tk-select">
    <button class="tk-select-trigger" type="button">Peran &middot; 2 dipilih ${icon('keyboard-arrow-down', 18)}</button>
    <div class="tk-select-menu">
...
`,
    ...
    <button class="tk-btn tk-btn-outline">Aksi ${icon('keyboard-arrow-down', 18)}</button>
    <div class="tk-dropdown-menu">
        ...
        <button class="tk-dropdown-item">${icon('edit', 18)} Edit</button>
        <button class="tk-dropdown-item">${icon('content-copy', 18)} Duplikat</button>
        <hr class="tk-dropdown-divider">
        <button class="tk-dropdown-item tk-dropdown-item-danger">${icon('delete', 18)} Hapus</button>
    </div>
</div>
`,
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua status ${icon('keyboard-arrow-down', 18)}</button>
        <div class="tk-select-menu">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua peran ${icon('keyboard-arrow-down', 18)}</button>
        <div class="tk-select-menu">
            <label class="tk-option"><input class="tk-checkbox" type="checkbox" checked> Designer</label>
...
<article class="tk-card tk-card-bare">
    <figure class="tk-plate" style="aspect-ratio: 4 / 3; margin: 0">
        <img src="https://placehold.co/720x540?text=Foto" alt="Foto contoh" loading="lazy">
    </figure>
    <div class="tk-card-body">
        <p class="tk-caption">Koleksi</p>
        <h3 class="tk-h3">Kartu Tanpa Kotak</h3>
        <p class="tk-body-sm tk-muted">Kartu editorial: dibuka satu garis rambut, fotonya membesar perlahan saat disentuh.</p>
        <a class="tk-gild tk-body-sm" href="#">Baca selengkapnya</a>
    </div>
</article>
`,
    ...
    <a class="tk-navbar-link" href="#">Jurnal</a>
...
<aside class="tk-sidebar">
    ...
    <a class="tk-sidebar-item" href="#">${icon('bar-chart-4-bars', 18)} <span class="tk-sidebar-label">Laporan</span></a>
...
<nav class="tk-mobile-nav">
    <span class="tk-navbar-brand">LOGO</span>
    <button class="tk-btn tk-btn-ghost tk-btn-icon" aria-label="Menu">${icon('menu')}</button>
</nav>
`,
...
`,
    'components/alert.html': `<div class="tk-alert tk-alert-success">${icon('check-circle-outline', 20)}<span><span class="tk-alert-title">Berhasil</span> Perubahan Anda sudah tersimpan.</span></div>
<div class="tk-alert tk-alert-warning">${icon('warning', 20)}<span><span class="tk-alert-title">Perhatian</span> Periksa kembali informasi Anda.</span></div>
<div class="tk-alert tk-alert-danger">${icon('error', 20)}<span><span class="tk-alert-title">Gagal</span> Terjadi kesalahan, coba lagi.</span></div>
<div class="tk-alert tk-alert-info">${icon('info', 20)}<span><span class="tk-alert-title">Info</span> Ada pembaruan penting untuk Anda.</span></div>
`,
...
`,
    ...
    <div class="tk-modal">
        <div class="tk-card-header"><p class="tk-eyebrow">Konfirmasi</p><h3 class="tk-h3">Judul Modal</h3></div>
...
`,
    ...
    <span class="tk-empty-icon">${icon('inbox', 34)}</span>
...
`,
    ...
    <p class="tk-eyebrow tk-eyebrow-center">Edisi Perdana</p>
    <h1 class="tk-display">Karya yang dibuat perlahan</h1>
    <p class="tk-hero-sub">Satu kalimat pendukung yang menjelaskan nilai produk ini bagi penggunanya.</p>
    <div class="tk-hero-actions">
        <button class="tk-btn tk-btn-primary tk-btn-lg">Lihat Koleksi</button>
        <button class="tk-btn tk-btn-outline tk-btn-lg">Kisah Kami</button>
    </div>
</section>
<div class="tk-feature-grid">
    <div class="tk-feature">
        <span class="tk-feature-icon">${icon('diamond')}</span>
        <h4 class="tk-title">Fitur Utama</h4>
        <p class="tk-body-sm tk-muted">Penjelasan singkat manfaat fitur ini.</p>
    </div>
    <div class="tk-feature">
        <span class="tk-feature-icon">${icon('handshake')}</span>
        <h4 class="tk-title">Fitur Kedua</h4>
        <p class="tk-body-sm tk-muted">Penjelasan singkat manfaat fitur ini.</p>
    </div>
    <div class="tk-feature">
        <span class="tk-feature-icon">${icon('workspace-premium')}</span>
        <h4 class="tk-title">Fitur Ketiga</h4>
        <p class="tk-body-sm tk-muted">Penjelasan singkat manfaat fitur ini.</p>
    </div>
</div>
