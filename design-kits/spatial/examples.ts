...
/**
 * Halaman contoh (`examples/`) kit Spatial: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (gelap bawaan — ruang hanya terasa dalam gelap), jadi semua halaman
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="akar-icons:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Spatial — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-foot-btn:hover { background: rgba(237, 241, 255, 0.07); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(20, 26, 45, 0.75); backdrop-filter: blur(18px); border-bottom: 1px solid var(--tk-color-border); box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: radial-gradient(circle at 32% 28%, rgba(240, 232, 255, 0.9) 0%, #9D7BFF 40%, #3A2478 100%); box-shadow: var(--tk-sp-glow-violet); color: #14082E; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 700; flex: none; }
    ...
    .dm-sidebar.dm-sidebar-open { display: flex; position: fixed; top: 48px; left: 0; bottom: 0; height: auto; z-index: 90; background: rgba(13, 17, 32, 0.95); box-shadow: var(--tk-shadow-lg); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Spatial', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item" href="#">${ic('people-group')} <span class="tk-sidebar-label">Pengguna</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('statistic-up')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('augmented-reality')} <span class="tk-sidebar-label">Statistik</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('question')} <span class="tk-sidebar-label">Bantuan</span></a>
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="tk-sidebar-label" style="min-width: 0"><span class="tk-body-sm" style="display: block; font-weight: 600">Jane Doe</span><span class="tk-caption" style="text-transform: none; letter-spacing: 0">jane@contoh.com</span></span>
    ...
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            ">${ic('three-line-horizontal', 20)}</button>
        ...
        <main class="dm-content">
            ...
            <div class="tk-card">
                <div class="tk-card-header" style="display: flex; align-items: center; justify-content: space-between"><h3 class="tk-title" style="margin: 0">Pengguna Terbaru</h3><button class="tk-btn tk-btn-primary tk-btn-sm">${ic('plus', 13)} Tambah</button></div>
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-horizontal', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye-open', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash-can', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        ...
                        <div class="dm-activity-item">${ic('circle-check', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('triangle-alert', 15)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '2E', fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: '#4FE0D2', pointBorderColor: t.surface2, pointBorderWidth: 2, borderWidth: 3 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('check', 15)} ${item}</li>`).join('\n                ');
...
<style>
    ...
    /* Objek 3D di hero: kubus isometrik melayang di sisi kiri. */
    .lp-hero-cube { position: absolute; left: 8%; bottom: 22%; z-index: 1; }
    .lp-hero-cube-sm { position: absolute; left: 16%; top: 18%; transform: scale(0.55); }
    ...
    .lp-proof-stat { background: var(--tk-sp-glass); backdrop-filter: blur(12px); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm), var(--tk-sp-edge); padding: 14px 22px; }
    ...
    .lp-pricing { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; align-items: start; perspective: 1400px; }
    /* Paket unggulan berdiri paling dekat ke mata; dua lainnya mundur sedikit ke dalam ruang. */
    .lp-plan { transition: transform var(--tk-transition), box-shadow var(--tk-transition), border-color var(--tk-transition); }
    .lp-pricing .lp-plan:first-child { transform: perspective(1400px) rotateY(6deg) scale(0.97); }
    .lp-pricing .lp-plan:last-child { transform: perspective(1400px) rotateY(-6deg) scale(0.97); }
    .lp-plan-featured { border-color: rgba(91, 140, 255, 0.5); box-shadow: var(--tk-shadow-lg), var(--tk-sp-edge), 0 0 48px rgba(91, 140, 255, 0.22); transform: scale(1.04); }
    .lp-pricing .lp-plan:hover { transform: translateY(-6px); }
    .lp-pricing .lp-plan-featured:hover { transform: scale(1.04) translateY(-6px); }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); margin: 0 0 8px; }
    ...
    .lp-footer { background: rgba(8, 11, 22, 0.85); border-top: 1px solid var(--tk-color-border); }
    ...
    .lp-footer-links a { color: var(--tk-color-text-muted); text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 600; }
    ...
    @media (max-width: 860px) {
        ...
        .tk-hero { padding: 64px 20px 96px; }
        .tk-hero .tk-display { font-size: 36px; }
        .lp-hero-cube { left: -8px; bottom: 12%; transform: scale(0.7); }
        .lp-hero-cube-sm { display: none; }
        .tk-hero::after { width: 72px; height: 72px; right: -14px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Spatial', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('three-line-horizontal', 20)}</button>
</nav>
</header>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-cube lp-hero-cube" aria-hidden="true"></span>
    <span class="tk-cube tk-cube-violet lp-hero-cube-sm" aria-hidden="true"></span>
    ...
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; sebuah ruang kerja dengan kedalaman sungguhan.</p>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ${feature('thunder', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
        ${feature('shield', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('statistic-up', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ${feature('people-group', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('link-chain', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('augmented-reality', 'Berdimensi', 'Ruang kerja berlapis yang terasa hidup, bukan halaman datar.')}
    </div>
</section>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#333F63', '#4FE0D2', '#9D7BFF', t.primary], borderRadius: 8, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); text-align: center; margin-bottom: 20px; }
    ...
    .au-link { color: var(--tk-color-primary); text-decoration: none; font-weight: 600; }
...
<head>
${HEAD_COMMON}
<title>Spatial — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Spatial', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Spatial — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #202944; border-radius: 48px; background: #202944; box-shadow: var(--tk-shadow-lg), var(--tk-sp-glow-azure); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Spatial', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const SPATIAL_EXAMPLES: Record<string, string> = {
