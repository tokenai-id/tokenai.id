 ...
 * `styles.css` kit Zine.
 *
 * Estetika zine fotokopian: kertas pucat + tinta pekat, aksen merah riot, sudut tajam tanpa
 * radius, border tebal, bayangan offset keras, judul Archivo Black uppercase. Struktur selektor
 * dan blok `== tk: ... ==` mengikuti kontrak `docs/kontrak-kit-design.md`; wajah zine lahir dari
 * nilai token plus blok "flavor" di ekor berkas.
 */
export const ZINE_STYLES = `/* Zine — kit design TOKENAI.
   Estetika zine fotokopian: kertas + tinta, merah riot, sudut tajam, bayangan offset keras.
   Kontrak: docs/kontrak-kit-design.md. Semua nilai design tinggal di :root. */

/* == tk: tokens ================================================= */
:root {
    --tk-color-primary: #16130E;
    --tk-color-primary-hover: #000000;
    --tk-color-primary-contrast: #FBF7EE;
    --tk-color-secondary: #FBF7EE;
    --tk-color-secondary-hover: #F0E9D9;
    --tk-color-secondary-contrast: #16130E;
    --tk-color-accent: #E63228;
    --tk-color-background: #F6F1E5;
    --tk-color-surface: #FBF7EE;
    --tk-color-surface-2: #EFE8D8;
    --tk-color-text: #16130E;
    --tk-color-text-muted: #6B6455;
    --tk-color-border: #16130E;
    --tk-color-success: #1F7A33;
    --tk-color-success-soft: #DDEEDD;
    --tk-color-warning: #A85E00;
    --tk-color-warning-soft: #F4E4C2;
    --tk-color-danger: #C81E1E;
    --tk-color-danger-soft: #F6DCDC;
    --tk-color-info: #1D4ED8;
    --tk-color-info-soft: #DCE4F6;

    --tk-font-heading: 'Archivo Black', 'Arial Black', system-ui, sans-serif;
    ...
    --tk-border-width: 2px;
    --tk-shadow-sm: 2px 2px 0 var(--tk-color-border);
    --tk-shadow: 4px 4px 0 var(--tk-color-border);
    --tk-shadow-lg: 7px 7px 0 var(--tk-color-border);

    --tk-transition: 120ms ease;
    --tk-container: 1200px;
}

/* Mode gelap: kertas dan tinta bertukar tempat — halaman jadi tinta, garis jadi kertas.
   Aktifkan dengan atribut data-tk-theme="dark" pada <html> atau <body>. */
[data-tk-theme="dark"] {
    --tk-color-primary: #FBF7EE;
    --tk-color-primary-hover: #FFFFFF;
    --tk-color-primary-contrast: #16130E;
    --tk-color-secondary: #221E17;
    --tk-color-secondary-hover: #2E2921;
    --tk-color-secondary-contrast: #FBF7EE;
    --tk-color-accent: #FF5148;
    --tk-color-background: #121009;
    --tk-color-surface: #1B1811;
    --tk-color-surface-2: #26221A;
    --tk-color-text: #FBF7EE;
    --tk-color-text-muted: #A79E8B;
    --tk-color-border: #FBF7EE;
    --tk-color-success: #4ADE80;
    --tk-color-success-soft: #17351F;
    --tk-color-warning: #FBBF24;
    --tk-color-warning-soft: #3A2C0C;
    --tk-color-danger: #FF6B61;
    --tk-color-danger-soft: #3E1512;
    --tk-color-info: #7C9EF8;
    --tk-color-info-soft: #161F3A;
    --tk-shadow-sm: 2px 2px 0 var(--tk-color-border);
    --tk-shadow: 4px 4px 0 var(--tk-color-border);
    --tk-shadow-lg: 7px 7px 0 var(--tk-color-border);
}
...
.tk-title { font-size: var(--tk-text-title); font-weight: 500; margin: 0 0 var(--tk-space-sm); }
...
.tk-link { color: var(--tk-color-accent); text-decoration: underline; text-underline-offset: 3px; }
...
.tk-btn-sm { font-size: var(--tk-text-caption); padding: 6px 10px; }
.tk-btn-lg { font-size: var(--tk-text-body); padding: 14px 24px; border-radius: var(--tk-radius); }
...
.tk-input:focus, .tk-textarea:focus {
    ...
    box-shadow: 0 0 0 3px color-mix(in srgb, var(--tk-color-primary) 12%, transparent);
}
...
.tk-field-success .tk-input { border-color: var(--tk-color-success); box-shadow: 0 0 0 3px rgba(22, 163, 74, 0.1); }
...
.tk-checkbox:hover, .tk-radio:hover { border-color: var(--tk-color-text-muted); }
...
.tk-dropdown-label { padding: 6px 10px; font-size: var(--tk-text-caption); font-weight: 500; color: var(--tk-color-text-muted); }
.tk-dropdown-item {
    display: flex;
    align-items: center;
    gap: var(--tk-space-sm);
...
.tk-card-stat .tk-stat-value { font-size: var(--tk-text-h2); font-weight: var(--tk-weight-heading); letter-spacing: -0.02em; }
...
.tk-navbar-dark { background: var(--tk-color-primary); border-bottom: none; }
.tk-navbar-dark .tk-navbar-brand { color: var(--tk-color-primary-contrast); }
.tk-navbar-dark .tk-navbar-link { color: var(--tk-color-primary-contrast); opacity: 0.72; }
.tk-navbar-dark .tk-navbar-link:hover, .tk-navbar-dark .tk-navbar-link-active { color: var(--tk-color-primary-contrast); opacity: 1; }
.tk-sidebar {
    width: 240px;
    background: var(--tk-color-surface);
...
.tk-segmented { display: inline-flex; background: var(--tk-color-surface-2); border-radius: var(--tk-radius-sm); padding: 3px; gap: 2px; }
.tk-segment {
    padding: 6px 14px;
    font-size: var(--tk-text-body-sm);
    font-weight: 500;
...
.tk-page-active { border-color: var(--tk-color-border); background: var(--tk-color-surface); box-shadow: var(--tk-shadow-sm); }
...
.tk-cta .tk-muted { color: var(--tk-color-primary-contrast); opacity: 0.72; }
...
/* == tk: zine flavor ============================================ */
/* Penimpa karakter zine di atas struktur kontrak: menang kaskade karena berada di ekor berkas.
   Semuanya tetap hanya membaca token --tk-*. */
.tk-display, .tk-h1, .tk-h2 {
    text-transform: uppercase;
    letter-spacing: 0.01em;
}
.tk-h3, .tk-h4 { letter-spacing: 0; }
.tk-caption, .tk-overline, .tk-badge, .tk-table th, .tk-sidebar-label, .tk-breadcrumb {
    font-family: var(--tk-font-mono);
}
.tk-btn {
    font-family: var(--tk-font-mono);
...
.tk-btn-primary { border-color: var(--tk-color-primary); }
...
.tk-card { border: var(--tk-border-width) solid var(--tk-color-border); box-shadow: var(--tk-shadow); }
.tk-cta { box-shadow: var(--tk-shadow-lg); }
.tk-navbar { border-bottom-width: var(--tk-border-width); }
.tk-modal { border: var(--tk-border-width) solid var(--tk-color-border); box-shadow: var(--tk-shadow-lg); }
.tk-input:focus, .tk-select-trigger:focus, .tk-search input:focus {
    outline: var(--tk-border-width) solid var(--tk-color-accent);
    outline-offset: 2px;
    box-shadow: none;
}
.tk-feature-icon { border: var(--tk-border-width) solid var(--tk-color-border); background: var(--tk-color-surface); }
.tk-hero .tk-display { text-decoration: underline; text-decoration-color: var(--tk-color-accent); text-decoration-thickness: 6px; text-underline-offset: 10px; }
`;
