...
/**
 * Halaman contoh (`examples/`) kit Skeuomorph: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — benda fisik di bawah lampu kerja), jadi semua halaman
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="bi:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Skeuomorph — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); padding: 8px 16px 20px; white-space: nowrap; text-shadow: var(--tk-sk-emboss); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-sk-metal); border-bottom: 1px solid var(--tk-color-border); box-shadow: 0 2px 5px rgba(59, 55, 47, 0.18), inset 0 1px 0 rgba(255, 255, 255, 0.8); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: linear-gradient(180deg, #B98A50 0%, #96683A 100%); border: 1px solid #6E4B26; box-shadow: 0 1px 3px rgba(59, 55, 47, 0.35), inset 0 1px 0 rgba(255, 255, 255, 0.35); color: #FFF; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 700; text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.3); flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Skeuomorph', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('speedometer2')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('gear')} <span class="tk-sidebar-label">Pengaturan</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            ...
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                ...
                <button class="tk-dropdown-item" type="button">${ic('gear', 15)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('box-arrow-right', 15)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <div class="dm-topbar-spacer">
                ...
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('bell', 17)}</button>
            </div>
        </header>
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    ...
                    <div class="tk-select-menu">
                        <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${ic('check-lg', 15)}</span></button>
                ...
                <div class="tk-card-body dm-table-wrap" style="padding: 0">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('three-dots', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil-square', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        ...
                        <div class="dm-activity-item">${ic('check-circle', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('exclamation-triangle', 15)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '22', fill: true, tension: 0.3, pointRadius: 4, pointBackgroundColor: t.primary, pointBorderColor: '#FFF', pointBorderWidth: 2, borderWidth: 3 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('check-lg', 15)} ${item}</li>`).join('\n                ');
    return `<div class="tk-card lp-plan${featured ? ' lp-plan-featured' : ''}">
        <div class="tk-card-body">
            ${featured ? '<span class="tk-badge tk-badge-success">Paling populer</span>' : ''}
...
function feature(iconName: string, title: string, text: string): string {
    return `<div class="tk-feature tk-plate"><span class="tk-feature-icon">${ic(iconName, 24)}</span><h4 class="tk-title" style="margin: 0">${title}</h4><p class="tk-body-sm tk-muted" style="margin: 0">${text}</p></div>`;
}
...
<style>
    ...
    ${EX_NAV_CSS}
    .lp-skip { position: absolute; left: -9999px; top: 0; z-index: 30; background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); padding: 8px 16px; border-radius: var(--tk-radius); text-decoration: none; font-size: var(--tk-text-body-sm); }
    ...
    /* Hero: panel logam dengan kenop dan LED — mesin sungguhan di atas meja. */
    .lp-hero-panel { max-width: 880px; margin: 48px auto 0; padding: 0 24px; }
    .lp-hero-machine { display: flex; align-items: center; justify-content: center; gap: 40px; padding: 24px 32px; background: var(--tk-sk-metal); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius-lg); box-shadow: 0 4px 10px rgba(59, 55, 47, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.85); flex-wrap: wrap; }
    .lp-machine-item { display: flex; flex-direction: column; align-items: center; gap: 10px; }
    .lp-led-row { display: flex; gap: 14px; align-items: center; }
    ...
    .lp-proof-stat { background: var(--tk-sk-metal); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm); padding: 14px 22px; }
    ...
    .lp-plan-featured { box-shadow: 0 8px 20px rgba(59, 55, 47, 0.35), inset 0 1px 0 rgba(255, 255, 255, 0.75); transform: scale(1.04); border-color: #2C5480; }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); text-shadow: var(--tk-sk-emboss); margin: 0 0 8px; }
    ...
    .lp-footer { background: var(--tk-sk-metal-dark); border-top: 1px solid #2C2925; color: #C9C4B8; }
    ...
    .lp-footer-inner .tk-caption { color: #A39B85; }
    .lp-footer-links { display: flex; gap: 20px; }
    .lp-footer-links a { color: #C9C4B8; text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 700; text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.5); }
    ...
    @media (max-width: 860px) {
        ...
        .tk-hero .tk-display { font-size: 34px; }
        .lp-hero-machine { gap: 24px; padding: 20px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Skeuomorph', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('list', 20)}</button>
</nav>
</header>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero" style="padding-bottom: 32px">
    <span class="tk-badge tk-badge-success" style="margin-bottom: 20px">Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display" id="lp-h-hero">Kelola proyek tanpa drama</h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; sekokoh perkakas di meja kerja.</p>
    <div class="tk-hero-actions">
        <button class="tk-btn tk-btn-primary tk-btn-lg">Mulai Gratis ${ic('arrow-right', 15)}</button>
        <button class="tk-btn tk-btn-outline tk-btn-lg">Lihat Demo</button>
    </div>
</section>
<div class="lp-hero-panel">
    <div class="lp-hero-machine tk-plate">
        <div class="lp-machine-item"><span class="tk-knob" aria-hidden="true"></span><span class="tk-caption">Rencana</span></div>
        <div class="lp-machine-item"><span class="tk-knob tk-knob-45" aria-hidden="true"></span><span class="tk-caption">Lacak</span></div>
        <div class="lp-machine-item"><span class="tk-knob tk-knob-90" aria-hidden="true"></span><span class="tk-caption">Selesai</span></div>
        <div class="lp-machine-item">
            <div class="lp-led-row"><span class="tk-led" aria-hidden="true"></span><span class="tk-led tk-led-amber" aria-hidden="true"></span><span class="tk-led tk-led-off" aria-hidden="true"></span></div>
            <span class="tk-caption">Status Sistem</span>
        </div>
        <div class="lp-machine-item"><label class="tk-check"><input class="tk-toggle" type="checkbox" checked aria-label="Mode otomatis"></label><span class="tk-caption">Otomatis</span></div>
    </div>
</div>
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ...
        ${feature('bar-chart', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ...
        ${feature('sliders', 'Terkendali', 'Setiap detail bisa disetel seperti panel mesin.')}
    </div>
</section>
<section class="lp-section" style="padding-top: 0" id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-proof">
        ...
        <div class="tk-card"><div class="tk-card-header"><p class="tk-title" style="margin: 0">Pengguna aktif per kuartal</p></div><div class="tk-card-body">
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#8F8873', '#E8A33D', '#96683A', t.primary], borderColor: ['#6E6858', '#B07C22', '#6E4B26', '#2C5480'], borderWidth: 1, borderRadius: 4, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); text-align: center; margin-bottom: 20px; text-shadow: var(--tk-sk-emboss); }
    ...
    .au-divider::before, .au-divider::after { content: ''; height: 1px; flex: 1; background: var(--tk-color-border); box-shadow: 0 1px 0 rgba(255, 255, 255, 0.6); }
...
<head>
${HEAD_COMMON}
<title>Skeuomorph — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Skeuomorph', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Skeuomorph — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #3B372F; border-radius: 44px; background: #3B372F; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 32px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Skeuomorph', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const SKEUOMORPH_EXAMPLES: Record<string, string> = {
