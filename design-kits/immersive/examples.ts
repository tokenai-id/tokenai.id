...
/**
 * Halaman contoh (`examples/`) kit Immersive: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (gelap bawaan — pengalaman ditonton di ruang gelap), jadi semua halaman
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="ion:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Immersive — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 800; font-size: var(--tk-text-h4); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-foot-btn:hover { background: rgba(244, 241, 234, 0.06); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(10, 10, 15, 0.85); backdrop-filter: blur(14px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: linear-gradient(140deg, #FF8A3D 0%, #38C8B4 100%); color: #140A05; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 800; flex: none; }
    ...
    .dm-sidebar.dm-sidebar-open { display: flex; position: fixed; top: 48px; left: 0; bottom: 0; height: auto; z-index: 90; background: #0C0C12; box-shadow: var(--tk-shadow-lg); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Immersive', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('speedometer')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('stats-chart')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('film')} <span class="tk-sidebar-label">Statistik</span></a>
    ...
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('chevron-forward', 12)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('chevron-forward', 12)}</span>
            ...
            <div class="dm-topbar-spacer">
                ...
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('notifications', 17)}</button>
            </div>
        </header>
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <button class="tk-btn tk-btn-outline">${ic('funnel', 15)} Filter</button>
            </div>
            <div class="tk-card">
                <div class="tk-card-header" style="display: flex; align-items: center; justify-content: space-between"><h3 class="tk-title" style="margin: 0">Pengguna Terbaru</h3><button class="tk-btn tk-btn-primary tk-btn-sm">${ic('add', 13)} Tambah</button></div>
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('ellipsis-horizontal', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('person', 15)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('checkmark-circle', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('warning', 15)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '26', fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: '#38C8B4', pointBorderColor: t.surface2, pointBorderWidth: 2, borderWidth: 3 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('checkmark', 15)} ${item}</li>`).join('\n                ');
...
function feature(iconName: string, title: string, text: string): string {
    return `<div class="tk-feature tk-reveal"><span class="tk-feature-icon">${ic(iconName, 24)}</span><h4 class="tk-title" style="margin: 0">${title}</h4><p class="tk-body-sm tk-muted" style="margin: 0">${text}</p></div>`;
}
...
<style>
    ...
    .lp-hero-foot { position: absolute; left: 0; right: 0; bottom: 64px; display: flex; justify-content: center; }
    .lp-hero-sound { position: absolute; top: 64px; right: 24px; }
    ...
    .lp-proof-stat { background: var(--tk-color-surface); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm); padding: 14px 22px; }
    .lp-proof-stat .tk-h3 { margin: 0; color: var(--tk-color-primary); }
    ...
    .lp-section { max-width: var(--tk-container); margin: 0 auto; padding: 0 24px; width: 100%; box-sizing: border-box; }
    .lp-section-title { margin-bottom: 8px; }
    .lp-section-sub { color: var(--tk-color-text-muted); max-width: 520px; margin: 0 0 48px; }
    ...
    .lp-plan-featured { border-color: rgba(255, 138, 61, 0.5); box-shadow: var(--tk-shadow-lg), var(--tk-im-glow); }
    ...
    .lp-footer { background: #050508; border-top: 1px solid var(--tk-color-border); }
    ...
    .lp-footer-links a { color: var(--tk-color-text-muted); text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 700; }
    ...
    @media (max-width: 860px) {
        ...
        .tk-scene { min-height: 0; padding: 64px 0; }
        .tk-hero { min-height: 88vh; }
        .tk-hero .tk-display { font-size: 40px; }
        .lp-hero-sound { top: 56px; right: 12px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Immersive', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <button class="tk-sound-chip lp-hero-sound" type="button" aria-pressed="true" onclick="
        var eq = this.querySelector('.tk-eq');
        var on = eq.classList.toggle('tk-eq-muted');
        this.querySelector('span').textContent = on ? 'Suara mati' : 'Suara aktif';
        this.setAttribute('aria-pressed', String(!on));
    "><span class="tk-eq" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></span><span>Suara aktif</span></button>
    <p class="tk-kicker" style="justify-content: center">Scene 01 &mdash; Pembuka</p>
    <h1 class="tk-display" id="lp-h-hero">Kelola proyek tanpa drama</h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; sebuah pengalaman, bukan sekadar halaman.</p>
    <div class="tk-hero-actions">
        <button class="tk-btn tk-btn-primary tk-btn-lg">${ic('play', 15)} Mulai Pengalaman</button>
        <button class="tk-btn tk-btn-outline tk-btn-lg">Lihat Demo</button>
    </div>
    <div class="lp-hero-foot"><span class="tk-scroll-cue">Gulir</span></div>
</section>
<div class="tk-marquee" aria-hidden="true">
    <div class="tk-marquee-track"><span>Rencanakan <em>&#9679;</em> Lacak <em>&#9679;</em> Selesaikan <em>&#9679;</em> Rayakan <em>&#9679;</em></span><span>Rencanakan <em>&#9679;</em> Lacak <em>&#9679;</em> Selesaikan <em>&#9679;</em> Rayakan <em>&#9679;</em></span></div>
    <div class="tk-marquee-track" aria-hidden="true"><span>Rencanakan <em>&#9679;</em> Lacak <em>&#9679;</em> Selesaikan <em>&#9679;</em> Rayakan <em>&#9679;</em></span><span>Rencanakan <em>&#9679;</em> Lacak <em>&#9679;</em> Selesaikan <em>&#9679;</em> Rayakan <em>&#9679;</em></span></div>
</div>
<section class="tk-scene" id="fitur" aria-labelledby="lp-h-fitur">
    <div class="lp-section">
        <p class="tk-kicker tk-reveal">Scene 02 &mdash; Fitur</p>
        <h2 class="tk-h2 lp-section-title tk-reveal" id="lp-h-fitur">Semua yang tim Anda butuhkan</h2>
        <p class="lp-section-sub tk-reveal">Fitur yang benar-benar dipakai, bukan daftar panjang yang membingungkan.</p>
        <div class="tk-feature-grid lp-features">
            ...
            ${feature('stats-chart', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
            ${feature('people', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
            ${feature('extension-puzzle', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
            ${feature('film', 'Sinematik', 'Setiap interaksi adalah adegan dengan transisi yang halus.')}
        </div>
    </div>
</section>
<section class="tk-scene" id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-section">
        <div class="lp-proof">
            <div>
                <p class="tk-kicker tk-reveal">Scene 03 &mdash; Bukti</p>
                <h2 class="tk-h2 tk-reveal" style="margin: 0 0 8px" id="lp-h-bukti">Tumbuh bersama ribuan tim</h2>
                <p class="tk-muted tk-reveal" style="margin: 0; max-width: 440px">Pengguna aktif naik empat kuartal berturut-turut. Angkanya bicara sendiri &mdash; dan tim Anda bisa jadi bagian berikutnya.</p>
                <div class="lp-proof-stats tk-reveal">
            ...
            <div class="tk-card tk-reveal"><div class="tk-card-body">
...
<section class="tk-scene" id="harga" aria-labelledby="lp-h-harga">
    <div class="lp-section">
        <p class="tk-kicker tk-reveal">Scene 04 &mdash; Harga</p>
        <h2 class="tk-h2 lp-section-title tk-reveal" id="lp-h-harga">Harga yang sederhana</h2>
        <p class="lp-section-sub tk-reveal">Mulai gratis, naikkan paket saat tim Anda tumbuh.</p>
...
<section class="tk-scene" style="min-height: 60vh" id="cta" aria-labelledby="lp-h-cta">
    <div class="lp-section">
        <div class="tk-cta tk-reveal">
            ...
            <button class="tk-btn tk-btn-primary">Coba Sekarang ${ic('arrow-forward', 15)}</button>
        </div>
    </div>
</section>
</main>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#2A2A38', '#38C8B4', '#FFD9A8', t.primary], borderRadius: 6, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 800; font-size: var(--tk-text-h4); text-align: center; margin-bottom: 20px; }
...
<head>
${HEAD_COMMON}
<title>Immersive — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Immersive', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Immersive — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #1B1B25; border-radius: 48px; background: #1B1B25; box-shadow: var(--tk-shadow-lg), var(--tk-im-glow); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Immersive', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const IMMERSIVE_EXAMPLES: Record<string, string> = {
