...
/**
 * Fragmen `components/` kit Monochrome: markup murni dengan kelas kontrak, teks placeholder
 * netral, ikon Iconify (set jam — garis setipis hairline dengan sudut geometris, satu bobot
 * untuk semua, senada dengan garis 1px yang memisahkan hampir semua hal di kit ini).
 * Gayanya seluruhnya di `styles.css` — fragmen inilah yang disalin agen saat merakit halaman,
 * jadi jangan menaruh style atribut di sini.
 *
 * Perhatikan fragmen alert: ikonnya bagian dari markup, bukan hiasan. Kit ini tidak punya rona
 * untuk membedakan status, jadi ikon adalah salah satu dari tiga isyarat yang dipakai bersama —
 * nada bidang, tebal rel kiri, dan bentuk ikon.
 *
...
export const MONOCHROME_COMPONENTS: Record<string, string> = {
...
<p class="tk-caps">Label Kapital</p>
...
<div class="tk-field">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih satu opsi</span> ${icon('chevron-down', 14)}</button>
...
`,
    ...
    <span class="tk-search-icon">${icon('search', 15)}</span>
...
`,
    ...
    <button class="tk-select-trigger" type="button">Opsi pertama ${icon('chevron-down', 14)}</button>
    <div class="tk-select-menu">
        <button class="tk-option tk-option-selected" type="button">Opsi pertama <span class="tk-option-check">${icon('check', 15)}</span></button>
        <button class="tk-option" type="button">Opsi kedua</button>
        <button class="tk-option" type="button">Opsi ketiga</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih anggota</span> ${icon('chevron-down', 14)}</button>
    <div class="tk-select-menu">
        <div class="tk-select-menu-search">${icon('search', 13)}<input type="text" placeholder="Cari anggota..."></div>
        <button class="tk-option" type="button">Jane Smith</button>
        <button class="tk-option tk-option-selected" type="button">John Doe <span class="tk-option-check">${icon('check', 15)}</span></button>
        <button class="tk-option" type="button">Maya Putri</button>
    </div>
</div>
<div class="tk-select">
    <button class="tk-select-trigger" type="button">Peran &middot; 2 dipilih ${icon('chevron-down', 14)}</button>
    <div class="tk-select-menu">
...
`,
    ...
    <button class="tk-btn tk-btn-outline">Aksi ${icon('chevron-down', 14)}</button>
    <div class="tk-dropdown-menu">
        <span class="tk-dropdown-label">Aksi baris</span>
        <button class="tk-dropdown-item">${icon('eye', 15)} Lihat detail</button>
        <button class="tk-dropdown-item">${icon('write', 15)} Edit</button>
        <button class="tk-dropdown-item">${icon('files', 15)} Duplikat</button>
        <hr class="tk-dropdown-divider">
        <button class="tk-dropdown-item tk-dropdown-item-danger">${icon('trash', 15)} Hapus</button>
    </div>
</div>
`,
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua status ${icon('chevron-down', 14)}</button>
        <div class="tk-select-menu">
            <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${icon('check', 15)}</span></button>
            <button class="tk-option" type="button">Aktif</button>
            <button class="tk-option" type="button">Nonaktif</button>
        </div>
    </div>
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua peran ${icon('chevron-down', 14)}</button>
        <div class="tk-select-menu">
            <label class="tk-option"><input class="tk-checkbox" type="checkbox" checked> Designer</label>
            <label class="tk-option"><input class="tk-checkbox" type="checkbox"> Developer</label>
        </div>
    </div>
    <button class="tk-btn tk-btn-outline">${icon('filter', 15)} Filter <span class="tk-filter-count">2</span></button>
</div>
<div class="tk-filter-active">
    <span class="tk-caps">Filter aktif</span>
...
<div class="tk-card tk-card-stat">
    <div class="tk-card-body">
        <p class="tk-caps">Total Pengguna</p>
...
<nav class="tk-breadcrumb">
    <a href="#">Beranda</a>
    <span class="tk-breadcrumb-sep">${icon('chevron-right', 12)}</span>
    <a href="#">Proyek</a>
    <span class="tk-breadcrumb-sep">${icon('chevron-right', 12)}</span>
...
<aside class="tk-sidebar">
    ...
    <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${icon('dashboard', 17)} <span class="tk-sidebar-label">Dashboard</span></a>
    <a class="tk-sidebar-item" href="#">${icon('folder', 17)} <span class="tk-sidebar-label">Proyek</span></a>
    <span class="tk-sidebar-group">Analitik</span>
    <a class="tk-sidebar-item" href="#">${icon('bar-chart', 17)} <span class="tk-sidebar-label">Laporan</span></a>
    <span class="tk-sidebar-group">Lainnya</span>
    <a class="tk-sidebar-item" href="#">${icon('cog', 17)} <span class="tk-sidebar-label">Pengaturan</span></a>
</aside>
`,
...
`,
    'components/alert.html': `<div class="tk-alert tk-alert-success">${icon('check', 17)}<span><span class="tk-alert-title">Berhasil!</span> Perubahan Anda sudah tersimpan.</span></div>
<div class="tk-alert tk-alert-warning">${icon('alert', 17)}<span><span class="tk-alert-title">Perhatian.</span> Periksa kembali informasi Anda.</span></div>
<div class="tk-alert tk-alert-danger">${icon('triangle-danger', 17)}<span><span class="tk-alert-title">Gagal.</span> Terjadi kesalahan, coba lagi.</span></div>
<div class="tk-alert tk-alert-info">${icon('info', 17)}<span><span class="tk-alert-title">Info.</span> Ada pembaruan penting untuk Anda.</span></div>
`,
    ...
    <tbody>
        <tr>
            ...
            <td class="tk-table-actions">
                <button class="tk-action-btn" aria-label="Lihat">${icon('eye', 15)}</button>
                <button class="tk-action-btn" aria-label="Edit">${icon('write', 15)}</button>
                <button class="tk-action-btn tk-action-btn-danger" aria-label="Hapus">${icon('trash', 15)}</button>
            </td>
        </tr>
        <tr>
            ...
            <td class="tk-table-actions">
                <div class="tk-dropdown">
                    <button class="tk-action-btn" aria-label="Menu aksi">${icon('more-horizontal', 15)}</button>
...
`,
    ...
    <span class="tk-empty-icon">${icon('inbox', 24)}</span>
...
`,
    ...
    <p class="tk-caps">Studio &middot; Sejak 2014</p>
...
<div class="tk-feature-grid">
    <div class="tk-feature">
        <span class="tk-feature-icon">${icon('rocket', 22)}</span>
