...
/**
 * `showcase.html` kit Hyperreality: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const c = HYPERREALITY_COMPONENTS;
...
const SWATCHES = [
    ['primary', '#6B2BFF'], ['primary-hover', '#8149FF'], ['secondary', '#00BCD8'],
    ['accent', '#FF57C7'], ['background', '#F4F0FF'], ['surface', '#FFFFFF'],
    ['surface-2', '#ECE5FF'], ['text', '#190C36'], ['text-muted', '#6A5B95'],
    ['border', '#DACCFF'], ['success', '#00A87A'], ['warning', '#E07B00'],
    ['danger', '#F32C63'], ['info', '#6B2BFF']
].map(([name, hex]) =>
    `<div class="sc-swatch"><div class="sc-swatch-fill" style="background: var(--tk-color-${name})"></div><span>${name}</span><code>${hex}</code></div>`
).join('');

const TYPE_SCALE = [
    ['Display', 'tk-display', '58px'], ['Heading 1', 'tk-h1', '42px'], ['Heading 2', 'tk-h2', '32px'],
    ...
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Lebih nyata dari aslinya</p></div>`
).join('');
...
const ICONS = ['home', 'search', 'person', 'settings', 'alert', 'heart', 'star', 'delete', 'edit',
    'checkmark', 'dismiss', 'add', 'arrow-right', 'flash', 'calendar-ltr', 'mail', 'lock-closed', 'cube']
    .map(name => `<span class="sc-icon" title="fluent:${name}-24-filled"><iconify-icon icon="fluent:${name}-24-filled" width="22" height="22"></iconify-icon></span>`)
    .join('');

export const HYPERREALITY_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Hyperreality — Design Kit Showcase</title>
...
<style>
    ...
    .sc-swatch-fill { height: 52px; border-radius: var(--tk-radius-sm); border: 1px solid var(--tk-color-border); box-shadow: var(--tk-shadow-sm), var(--tk-hr-gloss); }
    ...
    .sc-space-bar { height: 16px; background: linear-gradient(90deg, #6B2BFF 0%, #FF57C7 100%); border-radius: 999px; }
    ...
    .sc-radius-box { width: 136px; height: 76px; background: rgba(255, 255, 255, 0.92); border: 1px solid var(--tk-color-border); box-shadow: var(--tk-shadow-sm), var(--tk-hr-gloss); }
    ...
    /* Panggung objek: cakrawala mustahil dengan benda-benda berdiri di lantainya. */
    .sc-stage { display: flex; gap: 56px; align-items: center; justify-content: center; flex-wrap: wrap; padding: 64px 32px 72px; }
    .sc-stage-item { display: flex; flex-direction: column; align-items: center; gap: 22px; }
    .sc-stage-item .tk-caption { color: #4A2E85; }
    .sc-tools { display: grid; grid-template-columns: repeat(auto-fill, minmax(230px, 1fr)); gap: 16px; margin-top: 24px; }
    .sc-tool { background: rgba(255, 255, 255, 0.86); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm), var(--tk-hr-gloss); padding: 18px; }
    .sc-tool code { display: inline-block; margin-bottom: 6px; }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Hyperreality', 'components', '', { themeToggle: false })}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Hyperreality</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Visual yang sengaja melampaui kenyataan: langit mustahil dengan dua matahari berbeda warna, lantai grid yang melengkung ke cakrawala, bola krom yang memantulkan lingkungan yang tidak pernah ada, dan blob iridesen yang bentuknya tidak pernah berhenti berubah. Warnanya lebih jenuh daripada yang mungkin ada di alam, permukaannya sekilap benda hasil render, dan bayangannya berwarna &mdash; di dunia ini cahaya tidak pernah netral. Kartu menoleh dalam perspektif saat disentuh, tombol dilintasi kilau spekular. Kit satu-tema, terang secara bawaan. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Unbounded (heading display, lebar hurufnya tidak wajar) &middot; Figtree (body) &middot; DM Mono (caption, label, kode) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div><p class="tk-caption" style="margin-top: 16px">Violet ultra, cyan mustahil, dan pink iridesen &mdash; tiga warna yang kejenuhannya melewati apa pun yang bisa dipantulkan benda nyata.</p>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<div class="sc-row">${RADII}</div><p class="tk-caption" style="margin-top: 12px">Radius besar dan membulat penuh &mdash; benda hasil render tidak pernah punya sudut setajam kertas.</p>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div><p class="tk-caption" style="margin-top: 12px">Bayangannya violet, bukan hitam: sumber cahayanya berwarna, jadi gelapnya pun ikut berwarna.</p>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code class="tk-code">fluent</code> gaya <code class="tk-code">-24-filled</code> &mdash; tulis <code class="tk-code">&lt;iconify-icon icon="fluent:nama-24-filled"&gt;</code>. Bentuk pejal membulatnya terbaca sebagai objek padat, bukan gambar garis. Ratusan ribu ikon lain tersedia dengan set berbeda bila perlu; logo brand diambil dari <code class="tk-code">simple-icons</code> karena fluent tidak memuatnya.</p>`)}
${section('Design Language', `<div class="tk-horizon"><div class="sc-stage"><div class="sc-stage-item"><span class="tk-chrome"></span><span class="tk-caption">Krom</span></div><div class="sc-stage-item"><span class="tk-chrome tk-chrome-pink"></span><span class="tk-caption">Krom pink</span></div><div class="sc-stage-item"><span class="tk-chrome tk-chrome-cyan"></span><span class="tk-caption">Krom cyan</span></div><div class="sc-stage-item"><span class="tk-blob"></span><span class="tk-caption">Blob iridesen</span></div></div></div>
<div class="sc-tools">
    <div class="sc-tool"><code class="tk-code">.tk-chrome</code><p class="tk-body-sm tk-muted" style="margin: 0">Bola krom yang memantulkan lingkungan mustahil; varian <code class="tk-code">-pink</code> dan <code class="tk-code">-cyan</code>.</p></div>
    <div class="sc-tool"><code class="tk-code">.tk-blob</code><p class="tk-body-sm tk-muted" style="margin: 0">Bentuk cair yang radiusnya terus berubah &mdash; tidak ada benda seluwes ini.</p></div>
    <div class="sc-tool"><code class="tk-code">.tk-iridescent</code><p class="tk-body-sm tk-muted" style="margin: 0">Permukaan pelangi conic untuk badge, label, atau panel sorotan.</p></div>
    <div class="sc-tool"><code class="tk-code">.tk-warp</code><p class="tk-body-sm tk-muted" style="margin: 0">Gerak melengkung pelan, seperti dilihat lewat lensa yang tidak stabil.</p></div>
    ...
    <div class="sc-tool"><code class="tk-code">.tk-mirror</code><p class="tk-body-sm tk-muted" style="margin: 0">Pantulan berwarna di lantai, seperti benda yang berdiri di permukaan basah.</p></div>
    <div class="sc-tool"><code class="tk-code">.tk-horizon</code><p class="tk-body-sm tk-muted" style="margin: 0">Panggung bercakrawala &mdash; langit bergradasi plus lantai grid melengkung.</p></div>
</div>
<div class="sc-row" style="gap: 20px; margin-top: 24px; align-items: flex-end"><span class="tk-badge tk-iridescent">Badge iridesen</span><button class="tk-btn tk-btn-outline tk-warp">Tombol melengkung</button><span class="tk-chrome tk-mirror" style="width: 54px; height: 54px"></span></div>
<p class="tk-body-sm tk-muted" style="margin: 28px 0 0; max-width: 620px">Tujuh perkakas ini dipakai hemat: satu halaman cukup dua sampai tiga di antaranya. Objek mustahil tinggal di halaman pemasaran &mdash; dashboard tetap alat kerja, jadi di sana yang dipertahankan hanya permukaan sekilap, bayangan berwarna, dan kartunya pun berhenti menoleh supaya tabel bisa dibaca. Seluruh animasi berhenti otomatis untuk pengguna yang menyalakan <code class="tk-code">prefers-reduced-motion</code>.</p>`)}
${section('Buttons', `<div class="sc-row">${c['components/button.html']}</div><p class="tk-caption" style="margin-top: 16px">Tiap tombol dilintasi kilau spekular dari kiri ke kanan saat disorot &mdash; permukaan mengkilap yang menangkap cahaya.</p>`)}
${section('Input & Form', `<div class="sc-col">${c['components/form.html']}</div><p class="tk-caption" style="margin: 24px 0 12px">Search bar &mdash; ikon dan pintasan menyatu di dalam field</p><div style="max-width: 480px">${c['components/search.html']}</div><p class="tk-caption" style="margin: 24px 0 12px">Select custom (ditampilkan terbuka) &mdash; satu pilihan &middot; dengan pencarian &middot; multi dengan checkbox &amp; aksi. Listbox digambar kit, bukan popup bawaan browser.</p><div class="sc-row" style="align-items: flex-start; gap: 24px; min-height: 350px">${c['components/select.html'].split('<div class="tk-select">').join('<div class="tk-select tk-select-open" style="width: 250px">')}</div><p class="tk-caption" style="margin: 0 0 12px">Dropdown aksi (ditampilkan terbuka; aslinya terbuka saat trigger diklik)</p><div style="min-height: 250px">${c['components/dropdown.html'].replace('class="tk-dropdown"', 'class="tk-dropdown tk-dropdown-open"')}</div><p class="tk-caption">Kontrol digambar sendiri: field sedikit tenggelam ke permukaan, dan tepinya memancarkan cahaya berwarna saat fokus. Kenop toggle adalah bola krom kecil.</p>`)}
...
${section('Tabs & Segmented', `<div class="sc-col">${c['components/tabs.html']}</div><p class="tk-caption" style="margin-top: 12px">Rel tenggelam ke permukaan; tab aktif adalah kapsul bergradasi violet-pink.</p>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.primary, backgroundColor: t.accent + '2E', fill: true, tension: 0.45, pointRadius: 4, pointBackgroundColor: t.accent, pointBorderColor: t.surface, pointBorderWidth: 2, borderWidth: 3 }] },
