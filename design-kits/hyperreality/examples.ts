...
/**
 * Halaman contoh (`examples/`) kit Hyperreality: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — dunia hiperreal ini bermandikan cahaya), jadi semua halaman
 * memanggil exNav dengan `themeToggle: false`.
 *
 * Objek mustahil (bola krom, blob iridesen) dipakai hemat dan hanya di halaman pemasaran:
 * dashboard adalah alat kerja, jadi di sana yang dipertahankan cuma permukaan sekilap dan
 * bayangan berwarnanya. Kelas `dm-*` / `lp-*` / `au-*` / `ex-*` adalah perancah halaman contoh,
 * bukan bagian kontrak kit.
 */
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="fluent:${name}-24-filled" width="${size}" height="${size}"></iconify-icon>`;
}

const HEAD_COMMON = `<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=DM+Mono:wght@400;500&family=Figtree:wght@400;500;600;700;800&family=Unbounded:wght@500;600;700&display=swap" rel="stylesheet">
...
<title>Hyperreality — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); letter-spacing: -0.04em; padding: 10px 16px 20px; white-space: nowrap; }
    .dm-sidebar-brand-mini { display: none; }
    .tk-sidebar-collapsed .dm-sidebar-brand { text-align: center; padding: 10px 0 20px; }
    ...
    .dm-foot-btn { display: flex; align-items: center; gap: 10px; width: 100%; padding: 8px; border: none; border-radius: var(--tk-radius-full); background: transparent; font-family: inherit; color: var(--tk-color-text); text-align: left; cursor: pointer; }
    .dm-foot-btn:hover { background: rgba(107, 43, 255, 0.09); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(20px); border-bottom: 1px solid var(--tk-color-border); box-shadow: 0 10px 30px rgba(107, 43, 255, 0.08); position: sticky; top: 48px; z-index: 10; }
    ...
    /* Avatar adalah bola krom kecil — satu-satunya objek mustahil yang lolos ke alat kerja. */
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: var(--tk-hr-chrome); box-shadow: 0 4px 12px rgba(107, 43, 255, 0.3); color: #FFFFFF; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 11px; font-weight: 700; flex: none; text-shadow: 0 1px 3px rgba(25, 12, 54, 0.6); }
    ...
    /* Kartu dashboard tidak ikut menoleh saat disorot: tabel dan angka harus diam. */
    .dm-content .tk-card:hover { transform: translateY(-3px); }
    ...
    .dm-sidebar.dm-sidebar-open { display: flex; position: fixed; top: 48px; left: 0; bottom: 0; height: auto; z-index: 90; background: rgba(255, 255, 255, 0.98); box-shadow: var(--tk-shadow-lg); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Hyperreality', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('board')} <span class="tk-sidebar-label">Dashboard</span></a>
        <a class="tk-sidebar-item" href="#">${ic('people')} <span class="tk-sidebar-label">Pengguna</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('data-trending')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('data-bar-vertical')} <span class="tk-sidebar-label">Statistik</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            ...
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('person', 15)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('settings', 15)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('sign-out', 15)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            ">${ic('line-horizontal-3', 20)}</button>
            ...
            <div class="dm-topbar-spacer">
                ...
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('alert', 18)}</button>
            </div>
        </header>
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    ...
                    <div class="tk-select-menu">
                        <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${ic('checkmark', 15)}</span></button>
            ...
            <div class="tk-card">
                <div class="tk-card-header" style="display: flex; align-items: center; justify-content: space-between"><h3 class="tk-title" style="margin: 0">Pengguna Terbaru</h3><button class="tk-btn tk-btn-primary tk-btn-sm">${ic('add', 14)} Tambah</button></div>
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-horizontal', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('delete', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('person', 15)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('checkmark-circle', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('warning', 15)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.accent + '2E', fill: true, tension: 0.45, pointRadius: 4, pointBackgroundColor: t.accent, pointBorderColor: t.surface, pointBorderWidth: 2, borderWidth: 3 }]
        },
...
tkChart('dm-chart-traffic', function (t) {
    return {
        type: 'doughnut',
        data: { labels: ['Direct', 'Organik', 'Sosial', 'Referral'], datasets: [{ data: [38, 31, 19, 12], backgroundColor: [t.primary, t.accent, t.secondary, t.muted], borderColor: t.surface, borderWidth: 3 }] },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('checkmark-circle', 15)} ${item}</li>`).join('\n                ');
    return `<div class="tk-card lp-plan${featured ? ' lp-plan-featured' : ''}">
        <div class="tk-card-body">
            ${featured ? '<span class="tk-badge tk-badge-info">Paling populer</span>' : ''}
...
function feature(iconName: string, title: string, text: string): string {
    return `<div class="tk-feature"><span class="tk-feature-icon">${ic(iconName, 24)}</span><h4 class="tk-title" style="margin: 0">${title}</h4><p class="tk-body-sm tk-muted" style="margin: 0">${text}</p></div>`;
}
...
<style>
    ...
    ${EX_NAV_CSS}
    .lp-skip { position: absolute; left: -9999px; top: 0; z-index: 30; background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); padding: 8px 16px; border-radius: var(--tk-radius-sm); text-decoration: none; font-size: var(--tk-text-body-sm); }
    ...
    /* Objek mustahil yang mengambang di cakrawala hero. */
    .lp-hero-chrome { position: absolute; left: 7%; bottom: 20%; z-index: 1; }
    .lp-hero-blob { position: absolute; right: 6%; top: 16%; width: 88px; height: 88px; z-index: 1; }
    .lp-hero-chrome-sm { position: absolute; right: 17%; bottom: 26%; width: 44px; height: 44px; z-index: 1; }
    ...
    .lp-proof-stat { background: rgba(255, 255, 255, 0.9); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm), var(--tk-hr-gloss); padding: 14px 22px; }
    .lp-proof-stat .tk-h3 { margin: 0; background: linear-gradient(120deg, #6B2BFF 0%, #FF57C7 100%); -webkit-background-clip: text; background-clip: text; color: transparent; }
    ...
    .lp-section { max-width: var(--tk-container); margin: 0 auto; padding: 76px 24px; }
    .lp-section-title { text-align: center; margin-bottom: 8px; }
    .lp-section-sub { text-align: center; color: var(--tk-color-text-muted); max-width: 540px; margin: 0 auto 48px; }
    .lp-features { grid-template-columns: repeat(3, 1fr); }
    .lp-pricing { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; align-items: start; perspective: 1600px; }
    /* Paket unggulan berdiri paling dekat ke mata; dua lainnya menoleh ke dalam ruang. */
    .lp-plan { transition: transform var(--tk-transition), box-shadow var(--tk-transition), border-color var(--tk-transition); }
    .lp-pricing .lp-plan:first-child { transform: perspective(1600px) rotateY(7deg) scale(0.97); }
    .lp-pricing .lp-plan:last-child { transform: perspective(1600px) rotateY(-7deg) scale(0.97); }
    .lp-plan-featured { border-color: rgba(255, 87, 199, 0.55); box-shadow: var(--tk-shadow-lg), var(--tk-hr-gloss); transform: scale(1.05); }
    .lp-pricing .lp-plan:hover { transform: translateY(-6px); }
    .lp-pricing .lp-plan-featured:hover { transform: scale(1.05) translateY(-6px); }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); letter-spacing: -0.04em; margin: 0 0 8px; }
    ...
    .lp-footer { background: rgba(25, 12, 54, 0.95); border-top: 1px solid var(--tk-color-border); }
    ...
    .lp-footer-inner .tk-caption { color: rgba(255, 255, 255, 0.7); }
    .lp-footer-links { display: flex; gap: 20px; }
    .lp-footer-links a { color: rgba(255, 255, 255, 0.72); text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 600; }
    ...
    @media (max-width: 860px) {
        ...
        .lp-pricing .lp-plan:first-child, .lp-pricing .lp-plan:last-child { transform: none; }
        .lp-plan-featured { transform: none; }
        .lp-pricing .lp-plan-featured:hover { transform: translateY(-6px); }
        .lp-proof { grid-template-columns: 1fr; gap: 24px; }
        .lp-chart-box { height: 220px; }
        .tk-hero { padding: 64px 20px 104px; }
        .tk-hero .tk-display { font-size: 38px; }
        .lp-hero-chrome { left: -18px; bottom: 12%; width: 64px; height: 64px; }
        .lp-hero-blob { right: -20px; top: 8%; width: 64px; height: 64px; }
        .lp-hero-chrome-sm { display: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Hyperreality', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('line-horizontal-3', 20)}</button>
</nav>
</header>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-chrome lp-hero-chrome" aria-hidden="true"></span>
    <span class="tk-blob lp-hero-blob" aria-hidden="true"></span>
    <span class="tk-chrome tk-chrome-cyan lp-hero-chrome-sm" aria-hidden="true"></span>
    <span class="tk-badge tk-badge-info" style="margin-bottom: 20px">Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display" id="lp-h-hero">Kelola proyek tanpa drama</h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; dalam ruang kerja yang warnanya lebih hidup daripada aslinya.</p>
    <div class="tk-hero-actions">
        <button class="tk-btn tk-btn-primary tk-btn-lg">Mulai Gratis ${ic('arrow-right', 16)}</button>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    <h2 class="tk-h2 lp-section-title" id="lp-h-fitur">Semua yang tim Anda butuhkan</h2>
    ...
    <div class="tk-feature-grid lp-features">
        ...
        ${feature('shield-checkmark', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('data-trending', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ${feature('people-team', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('link', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('cube', 'Berdimensi', 'Ruang kerja yang terasa punya isi, bukan halaman datar.')}
    </div>
</section>
    ...
    <div class="lp-proof">
        <div>
            ...
            <div class="lp-proof-stats">
                <div class="lp-proof-stat"><p class="tk-h3">36rb</p><span class="tk-caption">Pengguna aktif</span></div>
                <div class="lp-proof-stat"><p class="tk-h3">99,9%</p><span class="tk-caption">Uptime</span></div>
                <div class="lp-proof-stat"><p class="tk-h3">4,8/5</p><span class="tk-caption">Rating pengguna</span></div>
            </div>
        </div>
...
<section class="lp-section" style="padding-top: 0" id="cta" aria-labelledby="lp-h-cta">
    <div class="tk-cta">
        ...
        <button class="tk-btn tk-btn-secondary tk-btn-lg">Coba Sekarang ${ic('arrow-right', 16)}</button>
    </div>
</section>
</main>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: [t.secondary, t.accent, t.primary + 'B3', t.primary], borderRadius: 12, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    /* Dibiarkan tembus pandang supaya langit dan lantai grid di body tetap terlihat:
       halaman auth pun berdiri di dalam dunia yang sama. */
    .au-wrap { position: relative; min-height: calc(100vh - 48px); box-sizing: border-box; display: flex; align-items: center; justify-content: center; padding: 44px 16px; background: transparent; }
    .ex-embedded .au-wrap { min-height: 100vh; }
    .au-card { width: 100%; max-width: 408px; }
    .au-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); letter-spacing: -0.04em; text-align: center; margin-bottom: 20px; }
    .au-title { text-align: center; margin: 0 0 4px; }
    .au-sub { text-align: center; margin: 0 0 20px; }
    .au-full { width: 100%; box-sizing: border-box; }
    .au-divider { display: flex; align-items: center; gap: 12px; margin: 16px 0; color: var(--tk-color-text-muted); font-family: var(--tk-font-mono); font-size: var(--tk-text-caption); }
    ...
    .au-link:hover { color: var(--tk-color-accent); text-decoration: underline; }
...
<head>
${HEAD_COMMON}
<title>Hyperreality — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Hyperreality', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Hyperreality — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #2A1660; border-radius: 52px; background: #2A1660; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 40px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Hyperreality', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const HYPERREALITY_EXAMPLES: Record<string, string> = {
