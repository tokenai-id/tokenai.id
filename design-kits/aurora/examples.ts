...
/**
 * Halaman contoh (`examples/`) kit Aurora: admin dashboard dan landing page utuh yang dirakit
 ...
 *
 * Cahaya dipakai hemat: mesh dan tirai aurora hanya di halaman pemasaran; dashboard adalah alat
 * kerja sehingga di sana yang tersisa cuma permukaan kaca dan pendar pada elemen aktif. Kelas
 * `dm-*` / `lp-*` / `au-*` / `ex-*` adalah perancah halaman contoh, bukan bagian kontrak kit.
 */
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="heroicons:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Aurora — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); letter-spacing: -0.035em; padding: 10px 16px 20px; white-space: nowrap; }
    ...
    .dm-foot-btn:hover { background: var(--tk-au-tint); }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: var(--tk-au-veil); -webkit-backdrop-filter: blur(22px); backdrop-filter: blur(22px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: var(--tk-au-grad); color: var(--tk-au-on-grad); display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 700; flex: none; box-shadow: 0 4px 14px rgba(111, 77, 235, 0.28); }
    ...
    .dm-sidebar.dm-sidebar-open { display: flex; position: fixed; top: 48px; left: 0; bottom: 0; height: auto; z-index: 90; background: var(--tk-au-veil-strong); box-shadow: var(--tk-shadow-lg); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Aurora', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' } })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('squares-2x2')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('arrow-trending-up')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('chart-bar')} <span class="tk-sidebar-label">Statistik</span></a>
        <span class="tk-sidebar-group">Lainnya</span>
        <a class="tk-sidebar-item" href="#">${ic('cog-6-tooth')} <span class="tk-sidebar-label">Pengaturan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('question-mark-circle')} <span class="tk-sidebar-label">Bantuan</span></a>
        <div class="tk-dropdown dm-sidebar-foot">
            ...
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                ...
                <button class="tk-dropdown-item" type="button">${ic('cog-6-tooth', 15)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('arrow-right-start-on-rectangle', 15)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <div class="dm-topbar-spacer">
                <div class="tk-search">
                    <span class="tk-search-icon">${ic('magnifying-glass', 15)}</span>
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('ellipsis-horizontal', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil-square', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        ...
                        <div class="dm-activity-item">${ic('check-circle', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('exclamation-triangle', 15)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.secondary + '26', fill: true, tension: 0.45, pointRadius: 4, pointBackgroundColor: t.secondary, pointBorderColor: t.surface, pointBorderWidth: 2, borderWidth: 3 }]
        },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('check-circle', 15)} ${item}</li>`).join('\n                ');
...
<style>
    ...
    /* Bola cahaya yang mengambang di langit hero — kabur, tanpa tepi. */
    .lp-orb { position: absolute; border-radius: 50%; filter: blur(2px); pointer-events: none; }
    .lp-orb-a { left: 9%; top: 26%; width: 78px; height: 78px; background: radial-gradient(circle at 34% 30%, rgba(255, 255, 255, 0.9) 0%, rgba(79, 224, 176, 0.75) 36%, rgba(76, 201, 240, 0.25) 100%); }
    .lp-orb-b { right: 8%; top: 18%; width: 54px; height: 54px; background: radial-gradient(circle at 34% 30%, rgba(255, 255, 255, 0.9) 0%, rgba(232, 95, 168, 0.7) 38%, rgba(111, 77, 235, 0.25) 100%); }
    .lp-orb-c { right: 17%; bottom: 16%; width: 36px; height: 36px; background: radial-gradient(circle at 34% 30%, rgba(255, 255, 255, 0.9) 0%, rgba(111, 77, 235, 0.65) 40%, rgba(111, 77, 235, 0) 100%); }
    ...
    .lp-proof-stat { background: var(--tk-au-veil); -webkit-backdrop-filter: blur(14px); backdrop-filter: blur(14px); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius); box-shadow: var(--tk-shadow-sm), var(--tk-au-sheen); padding: 14px 22px; }
    .lp-proof-stat .tk-h3 { margin: 0; }
    ...
    .lp-plan-featured { transform: scale(1.04); }
    .lp-plan-featured::before { opacity: 1; }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h2); letter-spacing: -0.035em; margin: 0 0 8px; }
    ...
    .lp-footer { background: var(--tk-au-veil); -webkit-backdrop-filter: blur(18px); backdrop-filter: blur(18px); border-top: 1px solid var(--tk-color-border); }
    ...
    @media (max-width: 860px) {
        ...
        .tk-hero { padding: 64px 20px 72px; }
        .tk-hero .tk-display { font-size: 38px; }
        .lp-orb-a { left: -14px; top: 20%; width: 56px; height: 56px; }
        .lp-orb-b { right: -12px; top: 12%; width: 44px; height: 44px; }
        .lp-orb-c { display: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Aurora', 'landing', '../', { device: { base: 'landing', active: 'web' } })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('bars-3', 20)}</button>
</nav>
</header>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="lp-orb lp-orb-a tk-float" aria-hidden="true"></span>
    <span class="lp-orb lp-orb-b tk-float" aria-hidden="true"></span>
    <span class="lp-orb lp-orb-c tk-float" aria-hidden="true"></span>
    <span class="tk-badge tk-badge-info" style="margin-bottom: 20px">Baru &middot; versi 2.0 sudah rilis</span>
    <h1 class="tk-display" id="lp-h-hero">Kelola proyek <span class="tk-gradient-text">tanpa drama</span></h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; dengan antarmuka yang tenang dipandang sepanjang hari.</p>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ${feature('bolt', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
        ${feature('shield-check', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('arrow-trending-up', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ${feature('user-group', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('puzzle-piece', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('sparkles', 'Nyaman', 'Warna lembut dan kontras aman, terang maupun gelap.')}
    </div>
</section>
    ...
    <div class="lp-proof">
        <div>
            ...
            <div class="lp-proof-stats">
                <div class="lp-proof-stat"><p class="tk-h3 tk-gradient-text">36rb</p><span class="tk-caption">Pengguna aktif</span></div>
                <div class="lp-proof-stat"><p class="tk-h3 tk-gradient-text">99,9%</p><span class="tk-caption">Uptime</span></div>
                <div class="lp-proof-stat"><p class="tk-h3 tk-gradient-text">4,8/5</p><span class="tk-caption">Rating pengguna</span></div>
            </div>
        </div>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: [t.primary + '80', t.primary, t.secondary, t.accent], borderRadius: 12, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    /* Latarnya dibiarkan tembus supaya mesh dan tirai di body tetap terlihat —
       halaman auth pun berdiri di bawah langit yang sama. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 700; font-size: var(--tk-text-h4); letter-spacing: -0.035em; text-align: center; margin-bottom: 20px; }
    ...
    .au-link:hover { color: var(--tk-color-secondary); text-decoration: underline; }
...
<head>
${HEAD_COMMON}
<title>Aurora — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Aurora', 'components', '../', { device: { base, active: 'web' } })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Aurora — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #131B33; border-radius: 52px; background: #131B33; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 40px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Aurora', view, '../', { device: { base, active: 'mobile' }, extraThemeJs: FRAME_THEME_EXTRA_JS })}
<div class="ex-stage">
...
export const AURORA_EXAMPLES: Record<string, string> = {
