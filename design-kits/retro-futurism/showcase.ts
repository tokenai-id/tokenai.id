...
/**
 * `showcase.html` kit Retrofuturism: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const c = RETRO_FUTURISM_COMPONENTS;
...
const SWATCHES = [
    ['primary', '#E8590C'], ['primary-hover', '#C94B05'], ['secondary', '#1F2D50'],
    ['accent', '#0B7285'], ['background', '#F4EFE6'], ['surface-2', '#EAE3D4'],
    ['text', '#201D17'], ['text-muted', '#6E6759'], ['border', '#CFC5AE'],
    ['success', '#2B8A3E'], ['warning', '#9A6700'], ['danger', '#C92A2A'], ['info', '#1971C2']
].map(([name, hex]) =>
    `<div class="sc-swatch"><div class="sc-swatch-fill" style="background: var(--tk-color-${name})"></div><span>${name}</span><code>${hex}</code></div>`
).join('');

const TYPE_SCALE = [
    ['Display', 'tk-display', '42px'], ['Heading 1', 'tk-h1', '32px'], ['Heading 2', 'tk-h2', '25px'],
    ['Heading 3', 'tk-h3', '19px'], ['Heading 4', 'tk-h4', '15px'], ['Title', 'tk-title', '14px'],
    ['Body', 'tk-body', '16px'], ['Body Small', 'tk-body-sm', '14px'], ['Caption', 'tk-caption', '13px']
].map(([label, cls, size]) =>
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Menuju orbit esok hari</p></div>`
).join('');
...
const ICONS = ['home', 'search', 'user', 'settings', 'bell', 'heart', 'star', 'trash', 'pencil',
    'check', 'x', 'plus', 'arrow-right', 'download', 'upload', 'calendar', 'mail', 'lock']
    .map(name => `<span class="sc-icon" title="tabler:${name}"><iconify-icon icon="tabler:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const RETRO_FUTURISM_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Retrofuturism — Design Kit Showcase</title>
...
<style>
    ...
    .sc-wrap { max-width: 1140px; margin: 0 auto; padding: 48px 24px 96px; }
    .sc-kit-title { font-family: var(--tk-font-mono); font-size: 13px; letter-spacing: 0.14em; text-transform: uppercase; color: var(--tk-color-text-muted); margin: 0 0 4px; }
    .sc-section { margin-top: 56px; }
    .sc-heading { font-family: var(--tk-font-heading); font-size: 12px; font-weight: 400; letter-spacing: 0.10em; text-transform: uppercase; color: var(--tk-color-text-muted); border-bottom: 1px solid var(--tk-color-border); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-space-bar { height: 16px; background: var(--tk-color-info-soft); border: 1px solid var(--tk-color-info); border-radius: 4px; }
    ...
    .sc-overlay-demo { position: relative; height: 320px; overflow: hidden; border-radius: var(--tk-radius-lg); }
    ...
    .sc-chrome-demo { font-family: var(--tk-font-heading); font-size: 34px; text-transform: uppercase; letter-spacing: 0.03em; background: var(--tk-rf-chrome-text); -webkit-background-clip: text; background-clip: text; color: transparent; filter: drop-shadow(0 2px 0 rgba(32, 29, 23, 0.45)); margin: 0; }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Retrofuturism', 'components', '')}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Retrofuturism</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Masa depan dalam bahasa visual masa lalu: chrome lettering, tipografi space-age Michroma, grid blueprint futuristik, tombol kapsul panel instrumen, ikon porthole, dan pita matahari senja 70s. Mode gelap berpindah ke synthwave 80s &mdash; langit ungu ber-grid neon. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Michroma (heading space-age keluarga Eurostile) &middot; Space Grotesk (body) &middot; Share Tech Mono (caption &amp; readout) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
...
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>tabler</code> &mdash; tulis <code>&lt;iconify-icon icon="tabler:nama"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-row"><span class="tk-badge">Chrome</span><span class="tk-badge">Space-age type</span><span class="tk-badge">Grid futuristik</span><span class="tk-badge">Atomic 60s&ndash;90s</span></div><p class="sc-chrome-demo" style="margin: 20px 0 0">Tomorrowland</p><hr class="tk-hr-stripes"><p class="tk-body-sm tk-muted" style="margin: 16px 0; max-width: 560px">Optimisme era antariksa: headline chrome bergradasi horizon, heading Michroma yang lebar seperti panel kontrol pesawat, kanvas krem ber-grid blueprint, tombol kapsul pil dengan cincin ganda saat hover, ikon porthole bergiliran warna atomic (jingga &middot; teal &middot; navy), speed stripes ala livery roket, angka telemetri <code class="tk-code">.tk-readout</code>, dan CTA papan matahari senja. Mode gelap membawa semuanya ke synthwave 80s: langit ungu pekat, grid neon magenta-cyan, dan cahaya horizon jingga.</p><div class="sc-row" style="align-items: center; gap: 10px"><span class="tk-readout">T-MINUS 10</span><span class="tk-readout">ORBIT 36K</span></div>`)}
...
${section('Card Components', `<div class="tk-grid tk-grid-3">${c['components/card.html']}<div class="tk-card"><div class="tk-card-body" style="text-align: center"><div class="tk-feature-icon" style="margin: 4px auto 12px"><iconify-icon icon="tabler:user" width="22" height="22"></iconify-icon></div><p class="tk-title" style="margin: 0">Jane Doe</p><p class="tk-caption">UI/UX Designer</p></div></div></div>`)}
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: t.primary, borderRadius: 8 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
