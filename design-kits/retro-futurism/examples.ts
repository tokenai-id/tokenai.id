...
/**
 * Halaman contoh (`examples/`) kit Retrofuturism: admin dashboard dan landing page utuh yang
...
<link href="https://fonts.googleapis.com/css2?family=Michroma&family=Space+Grotesk:wght@400;500;700&family=Share+Tech+Mono&display=swap" rel="stylesheet">
...
<title>Retrofuturism — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-size: var(--tk-text-title); letter-spacing: 0.06em; padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-avatar { width: 32px; height: 32px; border-radius: var(--tk-radius-full); background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 10px; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Retrofuturism', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' } })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('selector', 16)}</span>
            </button>
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('dots', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
...
<style>
    ...
    .lp-price { font-family: var(--tk-font-heading); font-size: var(--tk-text-h2); letter-spacing: 0.02em; margin: 0 0 8px; }
    ...
    .lp-stripes { max-width: var(--tk-container); margin: 0 auto; padding: 0 24px; }
    @media (max-width: 860px) {
        ...
        .tk-hero .tk-display { font-size: 28px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Retrofuturism', 'landing', '../', { device: { base: 'landing', active: 'web' } })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('menu-2', 20)}</button>
</nav>
</header>
<main id="konten-utama">
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-badge tk-badge-info" style="margin-bottom: 16px">${ic('rocket', 14)} Baru &middot; versi 2.0 sudah rilis</span>
...
<div class="lp-stripes"><hr class="tk-hr-stripes"></div>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: t.accent, borderRadius: 8, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-size: var(--tk-text-title); letter-spacing: 0.06em; text-align: center; margin-bottom: 20px; }
...
<head>
${HEAD_COMMON}
<title>Retrofuturism — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Retrofuturism', 'components', '../', { device: { base, active: 'web' } })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Retrofuturism — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
...
<body>
${exNav('Retrofuturism', view, '../', { device: { base, active: 'mobile' }, extraThemeJs: FRAME_THEME_EXTRA_JS })}
<div class="ex-stage">
...
export const RETRO_FUTURISM_EXAMPLES: Record<string, string> = {
