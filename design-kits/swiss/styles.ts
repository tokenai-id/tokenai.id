...
export const SWISS_STYLES = `/* Swiss — kit design TOKENAI.
   Estetika Swiss Style (International Typographic Style): grid sangat disiplin, sans-serif
   grotesque, penjajaran rata kiri yang kuat, whitespace luas, dan hierarki informasi yang
   sangat jelas. Sudut siku tanpa radius, nyaris tanpa bayangan, garis hairline sebagai
   pemisah, dan satu merah Swiss sebagai aksen presisi.
   Kontrak: docs/kontrak-kit-design.md. Semua nilai design tinggal di :root. */

/* == tk: tokens ================================================= */
:root {
    --tk-color-primary: #E30613;
    --tk-color-primary-hover: #C00511;
    --tk-color-primary-contrast: #FFFFFF;
    --tk-color-secondary: #111111;
    --tk-color-secondary-hover: #000000;
    --tk-color-secondary-contrast: #FFFFFF;
    --tk-color-accent: #111111;
    --tk-color-background: #FFFFFF;
    --tk-color-surface: #FFFFFF;
    --tk-color-surface-2: #F4F4F4;
    --tk-color-text: #111111;
    --tk-color-text-muted: #6E6E6E;
    --tk-color-border: #E2E2E2;
    --tk-color-success: #1A7F37;
    --tk-color-success-soft: #EDF7EF;
    --tk-color-warning: #9A6700;
    --tk-color-warning-soft: #FFF6E0;
    --tk-color-danger: #C00511;
    --tk-color-danger-soft: #FDECEA;
    --tk-color-info: #0550AE;
    --tk-color-info-soft: #EBF2FB;

    --tk-font-heading: 'Hanken Grotesk', 'Helvetica Neue', system-ui, sans-serif;
    --tk-font-body: 'Hanken Grotesk', 'Helvetica Neue', system-ui, sans-serif;
    --tk-font-mono: 'Fragment Mono', ui-monospace, monospace;
    ...
    /* Sudut siku: radius 0 adalah tanda tangan Swiss; full tetap ada untuk kontrol
       yang memang bundar (toggle, avatar). */
    --tk-radius-sm: 0px;
    --tk-radius: 0px;
    --tk-radius-lg: 0px;
    --tk-radius-full: 999px;

    /* Flat: kedalaman dari garis dan penjajaran, bukan bayangan; menu tetap diberi
       bayangan sangat tipis agar terpisah dari kanvas. */
    --tk-border-width: 1px;
    --tk-shadow-sm: 0 0 0 rgba(0, 0, 0, 0);
    --tk-shadow: 0 1px 2px rgba(0, 0, 0, 0.05);
    --tk-shadow-lg: 0 8px 24px rgba(0, 0, 0, 0.10);

    --tk-transition: 140ms ease;
    --tk-container: 1140px;

    /* Token khas swiss (bukan kontrak): garis tegas untuk kontrol form dan aturan
       tipografis (rule) yang menandai awal seksi. */
    --tk-sw-line-strong: #ADADAD;
    --tk-sw-rule: #111111;
}

/* Mode gelap: tinta dibalik — hitam pekat, teks putih, hairline abu tua; merah Swiss
   sedikit lebih terang agar tetap menyala di kanvas gelap. Aktifkan dengan atribut
   data-tk-theme="dark" pada <html> atau <body>. */
[data-tk-theme="dark"] {
    --tk-color-primary: #FF2B36;
    --tk-color-primary-hover: #FF4D56;
    --tk-color-primary-contrast: #FFFFFF;
    --tk-color-secondary: #FAFAFA;
    --tk-color-secondary-hover: #FFFFFF;
    --tk-color-secondary-contrast: #111111;
    --tk-color-accent: #FAFAFA;
    --tk-color-background: #0C0C0C;
    --tk-color-surface: #131313;
    --tk-color-surface-2: #1C1C1C;
    --tk-color-text: #F5F5F5;
    --tk-color-text-muted: #8F8F8F;
    --tk-color-border: #2A2A2A;
    --tk-color-success: #4ADE80;
    --tk-color-success-soft: #0E2417;
    --tk-color-warning: #FBBF24;
    --tk-color-warning-soft: #271D08;
    --tk-color-danger: #FF6B72;
    --tk-color-danger-soft: #2A100E;
    --tk-color-info: #6CA6F5;
    --tk-color-info-soft: #0E1B30;
    --tk-shadow-sm: 0 0 0 rgba(0, 0, 0, 0);
    --tk-shadow: 0 1px 2px rgba(0, 0, 0, 0.5);
    --tk-shadow-lg: 0 8px 24px rgba(0, 0, 0, 0.6);
    --tk-sw-line-strong: #4A4A4A;
    --tk-sw-rule: #F5F5F5;
}
...
.tk-display { font-size: var(--tk-text-display); font-weight: 800; letter-spacing: -0.025em; line-height: 1.02; }
.tk-h1 { font-size: var(--tk-text-h1); letter-spacing: -0.02em; }
.tk-h2 { font-size: var(--tk-text-h2); letter-spacing: -0.015em; }
.tk-h3 { font-size: var(--tk-text-h3); letter-spacing: -0.01em; }
...
/* == tk: button ================================================= */
/* Tombol persegi tegas: merah Swiss untuk aksi utama, hitam tinta untuk sekunder. */
.tk-btn {
    display: inline-flex;
    align-items: center;
...
.tk-help { font-size: var(--tk-text-caption); color: var(--tk-color-text-muted); font-family: var(--tk-font-mono); letter-spacing: 0.02em; text-transform: none; }
.tk-input, .tk-textarea, .tk-select-trigger {
    font-family: var(--tk-font-body);
    font-size: var(--tk-text-body-sm);
    color: var(--tk-color-text);
    background: var(--tk-color-surface);
    border: 1px solid var(--tk-sw-line-strong);
    border-radius: var(--tk-radius);
    padding: 12px 14px;
...
.tk-checkbox, .tk-radio {
    ...
    position: relative;
    background: var(--tk-color-surface);
    border: 1.5px solid var(--tk-sw-line-strong);
    cursor: pointer;
...
.tk-checkbox:focus-visible, .tk-radio:focus-visible, .tk-toggle:focus-visible {
    outline: none;
    box-shadow: 0 0 0 2px var(--tk-color-background), 0 0 0 4px var(--tk-color-primary);
}
.tk-toggle { position: relative; width: 42px; height: 24px; flex: none; margin: 0; appearance: none; border: 1.5px solid var(--tk-sw-line-strong); background: var(--tk-color-surface-2); border-radius: var(--tk-radius-full); cursor: pointer; transition: background var(--tk-transition), border-color var(--tk-transition); }
...
.tk-card-stat .tk-stat-value { font-size: var(--tk-text-h2); font-family: var(--tk-font-heading); font-weight: 800; letter-spacing: -0.015em; }
...
.tk-navbar {
    ...
    background: var(--tk-color-surface);
    border-bottom: 2px solid var(--tk-sw-rule);
}
.tk-navbar-brand { font-family: var(--tk-font-heading); font-weight: 800; font-size: 18px; letter-spacing: -0.015em; margin-right: auto; }
...
.tk-navbar-dark { background: #0C0C0C; border-bottom-color: #F5F5F5; }
...
.tk-sidebar-item-active { border-left-color: var(--tk-color-primary); background: var(--tk-color-surface-2); color: var(--tk-color-text); font-weight: 600; }
...
.tk-segmented { display: inline-flex; background: var(--tk-color-surface); border: 1px solid var(--tk-sw-line-strong); border-radius: var(--tk-radius); padding: 0; gap: 0; }
.tk-segment {
    padding: 8px 16px;
    font-family: var(--tk-font-body);
    font-size: var(--tk-text-body-sm);
    ...
    color: var(--tk-color-text-muted);
    border: none;
    border-right: 1px solid var(--tk-sw-line-strong);
...
/* == tk: table ================================================== */
/* Tabel Swiss klasik: aturan tebal di kepala, hairline antar baris, angka rata kanan
   diserahkan ke markup. */
...
.tk-page-active { background: var(--tk-color-secondary); color: var(--tk-color-secondary-contrast); }
...
.tk-progress { height: 6px; background: var(--tk-color-surface-2); border-radius: 0; overflow: hidden; }
...
.tk-empty {
    ...
    text-align: center;
    border: 1px dashed var(--tk-sw-line-strong);
    border-radius: var(--tk-radius-lg);
    background: var(--tk-color-surface);
}
...
/* Ikon fitur = kotak merah Swiss dengan ikon putih — penanda presisi di atas grid. */
.tk-feature-icon {
    width: 44px;
    height: 44px;
    display: flex;
...
/* CTA = blok tinta hitam dengan aturan merah di tepinya. */
.tk-cta {
    background: var(--tk-color-secondary);
    color: var(--tk-color-secondary-contrast);
    border: none;
    border-top: 4px solid var(--tk-color-primary);
    border-radius: 0;
...
.tk-cta .tk-h3 { color: var(--tk-color-secondary-contrast); margin: 0; }
.tk-cta .tk-muted { color: var(--tk-color-secondary-contrast); opacity: 0.7; }
.tk-cta .tk-btn-primary { background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); }
...
/* == tk: swiss flavor =========================================== */
/* Penimpa karakter swiss di atas struktur kontrak: menang kaskade karena berada di ekor
   berkas. Tanda tangan kit ini adalah penjajaran rata kiri yang tegas: hero tidak pernah
   di tengah, dan setiap seksi dibuka aturan (rule) tebal seperti lembar tipografi Swiss. */
.tk-hero { text-align: left; }
.tk-hero .tk-display {
    font-size: clamp(40px, 6.5vw, var(--tk-text-display));
    max-width: 800px;
    position: relative;
    padding-top: var(--tk-space-lg);
}
/* Aturan merah pendek di atas headline — penanda hierarki paling khas Swiss. */
.tk-hero .tk-display::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
...
.tk-hero-sub { margin-left: 0; margin-right: 0; }
.tk-hero-actions { justify-content: flex-start; }
/* Foto: siku dan polos — grid dan penjajaran yang bicara, bukan dekorasi. */
