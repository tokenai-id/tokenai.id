...
/**
 * `showcase.html` kit Organic: lembar peraga 22 seksi sesuai §6 kontrak — termasuk
...
const SWATCHES = [
    ['primary', '#3F7253'], ['primary-hover', '#356247'], ['secondary', '#DD7A50'],
    ['accent', '#A78BD4'], ['background', '#FBF8F2'], ['surface-2', '#F1ECE1'],
    ['text', '#2E3B33'], ['text-muted', '#6E7D72'], ['border', '#E4DDCE'],
    ['success', '#4C9A63'], ['warning', '#C98A2D'], ['danger', '#C75A4A'], ['info', '#4E88B8']
].map(([name, hex]) =>
    `<div class="sc-swatch"><div class="sc-swatch-fill" style="background: var(--tk-color-${name})"></div><span>${name}</span><code>${hex}</code></div>`
).join('');

const TYPE_SCALE = [
    ['Display', 'tk-display', '54px'], ['Heading 1', 'tk-h1', '40px'], ['Heading 2', 'tk-h2', '30px'],
    ...
    `<div class="sc-type-row"><span class="sc-type-meta">${label} &middot; ${size}</span><p class="${cls}" style="margin: 0">Aa &mdash; Mengalir seperti air</p></div>`
).join('');
...
const ICONS = ['home-4-line', 'search-line', 'user-3-line', 'settings-3-line', 'notification-line', 'heart-line', 'star-line', 'delete-2-line', 'edit-2-line',
    'check-line', 'close-line', 'add-line', 'arrow-right-line', 'download-2-line', 'upload-2-line', 'calendar-line', 'mail-line', 'lock-line']
    .map(name => `<span class="sc-icon" title="mingcute:${name}"><iconify-icon icon="mingcute:${name}" width="24" height="24"></iconify-icon></span>`)
    .join('');

export const ORGANIC_SHOWCASE = `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Organic — Design Kit Showcase</title>
...
<style>
    ...
    .sc-kit-title { font-family: var(--tk-font-mono); font-size: 13px; letter-spacing: 0.08em; text-transform: uppercase; color: var(--tk-color-primary); margin: 0 0 4px; }
    .sc-section { margin-top: 56px; }
    .sc-heading { font-family: var(--tk-font-mono); font-size: 12px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--tk-color-primary); border-bottom: 1px solid var(--tk-color-border); padding-bottom: 8px; margin: 0 0 24px; }
    ...
    .sc-swatch-fill { height: 48px; border-radius: var(--tk-radius-sm) var(--tk-radius-sm) var(--tk-radius-sm) 6px; border: 1px solid var(--tk-color-border); }
    ...
    .sc-space-bar { height: 16px; background: rgba(63, 114, 83, 0.25); border-radius: var(--tk-radius-full); }
    ...
    .sc-cell { background: var(--tk-color-surface-2); border-radius: var(--tk-radius-sm); height: 48px; }
    ...
    .sc-blob-row { display: flex; gap: 32px; align-items: center; flex-wrap: wrap; }
    .sc-stagger-note { max-width: 560px; }
${AUTH_CSS}
</style>
</head>
<body>
${exNav('Organic', 'components', '', { themeToggle: false })}
<div class="sc-wrap">
    <p class="sc-kit-title">TOKENAI Design Kit</p>
    <h1 class="tk-h1" style="margin: 0">Organic</h1>
    <p class="tk-body-lg tk-muted" style="margin-top: 8px">Design organik/fluid yang mengalir seperti alam: blob dengan bentuk tak beraturan yang bermorf pelan, kurva lembut tanpa sudut kaku, gradasi mesh pastel persik-mint-lila di atas krem hangat, hijau lumut dan koral terakota, layout asimetris dengan stagger vertikal, dan tipografi Fraunces yang hangat. Kit satu-tema &mdash; terang secara bawaan, tanpa mode gelap. Lihat juga contoh halaman nyata lewat bilah navigasi di atas.</p>

${section('Typography', `<p class="tk-caption" style="margin-bottom: 24px">Fraunces (heading hangat, italic untuk brand) &middot; Nunito (body membulat) &middot; Sono (caption &amp; label) &middot; skala di bawah adalah seluruh tingkat yang tersedia</p>${TYPE_SCALE}`)}
${section('Color Palette', `<div class="sc-grid">${SWATCHES}</div>`)}
${section('Spacing System', SPACES)}
${section('Border Radius', `<div class="sc-row">${RADII}</div><p class="tk-caption" style="margin-top: 12px">Radius besar di mana-mana; kartu memakai satu sudut kecil &mdash; bentuk daun yang tidak pernah simetris sempurna.</p>`)}
${section('Shadow System', `<div class="sc-row" style="gap: 24px">${SHADOWS}</div><p class="tk-caption" style="margin-top: 12px">Bayangan difus bersemu hijau &mdash; benda mengapung lembut, bukan garis keras.</p>`)}
${section('Iconography', `<div class="sc-row">${ICONS}</div><p class="tk-caption">Iconify, set <code>mingcute</code> (garis rounded, sufiks <code>-line</code>) &mdash; tulis <code>&lt;iconify-icon icon="mingcute:nama-line"&gt;</code>; ratusan ribu ikon lain tersedia dengan set berbeda bila perlu.</p>`)}
${section('Design Language', `<div class="sc-blob-row"><span class="tk-blob"></span><span class="tk-blob tk-blob-peach"></span><span class="tk-blob tk-blob-lilac"></span></div><h3 class="tk-h3" style="margin: 24px 0 8px">Kata penting diberi <span class="tk-underline-wave">garis gelombang</span></h3><p class="tk-body-sm tk-muted sc-stagger-note" style="margin: 16px 0 0">Bahasa design kit ini meniru alam: blob gradasi pastel yang bermorf pelan (<code class="tk-code">.tk-blob</code>, varian <code class="tk-code">-peach</code> / <code class="tk-code">-lilac</code>), garis bawah bergelombang seperti aliran air (<code class="tk-code">.tk-underline-wave</code>), ikon fitur berbentuk blob dengan warna yang bergiliran otomatis, grid fitur yang sengaja tidak rata (anak tengah turun, anak ketiga naik &mdash; batu pijakan di sungai), hero bergradasi mesh dengan blob dekoratif di tepinya, dan foto yang dipangkas jadi bentuk blob. Semua sudut membulat besar; checkbox pun berbentuk kerikil. Tidak ada mode gelap &mdash; kit ini hidup di cahaya matahari.</p>`)}
${section('Buttons', `<div class="sc-row">${c['components/button.html']}</div>`)}
${section('Input & Form', `<div class="sc-col">${c['components/form.html']}</div><p class="tk-caption" style="margin: 24px 0 12px">Search bar &mdash; ikon dan pintasan menyatu di dalam field</p><div style="max-width: 480px">${c['components/search.html']}</div><p class="tk-caption" style="margin: 24px 0 12px">Select custom (ditampilkan terbuka) &mdash; satu pilihan &middot; dengan pencarian &middot; multi dengan checkbox &amp; aksi. Listbox digambar kit, bukan popup bawaan browser.</p><div class="sc-row" style="align-items: flex-start; gap: 24px; min-height: 340px">${c['components/select.html'].split('<div class="tk-select">').join('<div class="tk-select tk-select-open" style="width: 250px">')}</div><p class="tk-caption" style="margin: 0 0 12px">Dropdown aksi (ditampilkan terbuka; aslinya terbuka saat trigger diklik)</p><div style="min-height: 240px">${c['components/dropdown.html'].replace('class="tk-dropdown"', 'class="tk-dropdown tk-dropdown-open"')}</div><p class="tk-caption">Semua kontrol digambar sendiri &mdash; select, checkbox kerikil, radio, dan toggle tidak memakai tampilan bawaan browser.</p>`)}
...
tkChart('sc-chart-line', function (t) {
    return {
        type: 'line',
        data: { labels: scLabels, datasets: [{ label: 'Pengunjung', data: [4200, 5100, 4800, 6300, 7100, 8400], borderColor: t.primary, backgroundColor: t.primary + '1F', fill: true, tension: 0.45, pointRadius: 0, borderWidth: 3 }] },
...
tkChart('sc-chart-bar', function (t) {
    return {
        type: 'bar',
        data: { labels: scLabels, datasets: [{ label: 'Pendapatan', data: [12, 19, 14, 22, 26, 31], backgroundColor: '#DD7A50', borderRadius: 12 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
