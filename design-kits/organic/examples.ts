...
/**
 * Halaman contoh (`examples/`) kit Organic: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — cahaya matahari, bukan layar), jadi semua halaman
...
function ic(name: string, size = 18): string {
    return `<iconify-icon icon="mingcute:${name}" width="${size}" height="${size}"></iconify-icon>`;
}

const HEAD_COMMON = `<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,600;0,9..144,700;1,9..144,700&family=Nunito:wght@400;600;700;800&family=Sono:wght@400;600&display=swap" rel="stylesheet">
...
<title>Organic — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-style: italic; font-weight: 700; font-size: var(--tk-text-h4); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(251, 248, 242, 0.9); backdrop-filter: blur(10px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-og-blob-1); background: linear-gradient(135deg, var(--tk-color-primary), #6BA37E); color: #FFF; display: inline-flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Organic', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('grid-line')} <span class="tk-sidebar-label">Dashboard</span></a>
        <a class="tk-sidebar-item" href="#">${ic('group-2-line')} <span class="tk-sidebar-label">Pengguna</span></a>
        <a class="tk-sidebar-item" href="#">${ic('folder-line')} <span class="tk-sidebar-label">Proyek</span></a>
        <span class="tk-sidebar-group">Analitik</span>
        <a class="tk-sidebar-item" href="#">${ic('chart-bar-line')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('chart-line-line')} <span class="tk-sidebar-label">Statistik</span></a>
        <span class="tk-sidebar-group">Lainnya</span>
        <a class="tk-sidebar-item" href="#">${ic('settings-3-line')} <span class="tk-sidebar-label">Pengaturan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('question-line')} <span class="tk-sidebar-label">Bantuan</span></a>
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('down-line', 16)}</span>
            </button>
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                <button class="tk-dropdown-item" type="button">${ic('user-3-line', 16)} Profil</button>
                <button class="tk-dropdown-item" type="button">${ic('settings-3-line', 16)} Pengaturan</button>
                <hr class="tk-dropdown-divider">
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('exit-line', 16)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            ">${ic('menu-line', 20)}</button>
            <nav class="tk-breadcrumb dm-crumb">
                <a href="#">Beranda</a>
                <span class="tk-breadcrumb-sep">${ic('right-line', 14)}</span>
                <a href="#">Analitik</a>
                <span class="tk-breadcrumb-sep">${ic('right-line', 14)}</span>
            ...
            <div class="dm-topbar-spacer">
                <div class="tk-search">
                    <span class="tk-search-icon">${ic('search-line', 16)}</span>
                    <input class="tk-input" type="search" placeholder="Cari apa saja...">
                    <kbd class="tk-search-kbd">Ctrl K</kbd>
                </div>
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('notification-line', 18)}</button>
            </div>
        </header>
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <div class="tk-select">
                    <button class="tk-select-trigger" type="button">Semua status ${ic('down-line', 16)}</button>
                    <div class="tk-select-menu">
                        <button class="tk-option tk-option-selected" type="button">Semua status <span class="tk-option-check">${ic('check-line', 16)}</span></button>
                        <button class="tk-option" type="button">Aktif</button>
                        <button class="tk-option" type="button">Nonaktif</button>
                    </div>
                </div>
                <button class="tk-btn tk-btn-outline">${ic('filter-line', 16)} Filter</button>
            </div>
            <div class="tk-card">
                <div class="tk-card-header" style="display: flex; align-items: center; justify-content: space-between"><h3 class="tk-title" style="margin: 0">Pengguna Terbaru</h3><button class="tk-btn tk-btn-primary tk-btn-sm">${ic('add-line', 14)} Tambah</button></div>
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('more-1-line', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye-line', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit-2-line', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('delete-2-line', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('user-3-line', 16)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('check-circle-line', 16)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('alert-line', 16)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '1F', fill: true, tension: 0.45, pointRadius: 0, borderWidth: 3 }]
        },
...
tkChart('dm-chart-traffic', function (t) {
    return {
        type: 'doughnut',
        data: { labels: ['Direct', 'Organik', 'Sosial', 'Referral'], datasets: [{ data: [38, 31, 19, 12], backgroundColor: [t.primary, '#DD7A50', '#A78BD4', '#C9D6CC'], borderColor: t.surface2, borderWidth: 3, borderRadius: 8 }] },
...
function plan(title: string, price: string, items: string[], featured: boolean): string {
    const list = items.map(item => `<li>${ic('check-line', 16)} ${item}</li>`).join('\n                ');
...
function feature(iconName: string, title: string, text: string): string {
    return `<div class="tk-feature"><span class="tk-feature-icon">${ic(iconName, 26)}</span><h4 class="tk-title" style="margin: 0">${title}</h4><p class="tk-body-sm tk-muted" style="margin: 0">${text}</p></div>`;
}
...
<style>
    ...
    /* Hero asimetris: teks di kiri, kolase blob di kanan — alur natural, bukan simetri. */
    .lp-hero-grid { display: grid; grid-template-columns: 7fr 5fr; gap: 48px; align-items: center; max-width: var(--tk-container); margin: 0 auto; }
    .tk-hero.lp-hero { text-align: left; }
    .lp-hero .tk-hero-actions { justify-content: flex-start; }
    .lp-hero .tk-hero-sub { margin-left: 0; }
    .lp-hero-art { position: relative; min-height: 320px; }
    .lp-hero-art .tk-blob { position: absolute; }
    .lp-hero-art .lp-blob-a { width: 210px; height: 210px; left: 8%; top: 4%; }
    .lp-hero-art .lp-blob-b { width: 160px; height: 160px; right: 6%; top: 34%; }
    .lp-hero-art .lp-blob-c { width: 120px; height: 120px; left: 26%; bottom: 0; }
    ...
    .lp-proof-stat { background: var(--tk-color-surface); border: 1px solid var(--tk-color-border); border-radius: var(--tk-radius) var(--tk-radius) var(--tk-radius) var(--tk-radius-sm); box-shadow: var(--tk-shadow-sm); padding: 14px 20px; }
    ...
    /* Paket unggulan sedikit terangkat — stagger asimetris khas kit. */
    .lp-plan-featured { border-color: var(--tk-color-primary); box-shadow: var(--tk-shadow); transform: translateY(-14px); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Organic', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<nav class="tk-navbar" aria-label="Navigasi utama">
    ...
    <button class="tk-action-btn lp-burger" aria-label="Menu">${ic('menu-line', 20)}</button>
</nav>
</header>
...
<section class="tk-hero lp-hero" id="beranda" aria-labelledby="lp-h-hero">
    <div class="lp-hero-grid">
        <div>
            ...
            <h1 class="tk-display" id="lp-h-hero">Kelola proyek <em>tanpa</em> <span class="tk-underline-wave">drama</span></h1>
            <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; mengalir senatural pekerjaan itu sendiri.</p>
            <div class="tk-hero-actions">
                <button class="tk-btn tk-btn-primary tk-btn-lg">Mulai Gratis ${ic('arrow-right-line', 16)}</button>
                <button class="tk-btn tk-btn-outline tk-btn-lg">Lihat Demo</button>
            </div>
        </div>
        <div class="lp-hero-art" aria-hidden="true">
            <span class="tk-blob lp-blob-a"></span>
            <span class="tk-blob tk-blob-peach lp-blob-b"></span>
            <span class="tk-blob tk-blob-lilac lp-blob-c"></span>
        </div>
    </div>
</section>
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    <h2 class="tk-h2 lp-section-title" id="lp-h-fitur">Semua yang tim Anda <span class="tk-underline-wave">butuhkan</span></h2>
    ...
    <div class="tk-feature-grid lp-features">
        ${feature('lightning-line', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
        ${feature('safety-certificate-line', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
        ${feature('chart-bar-line', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
        ${feature('group-2-line', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
        ${feature('plugin-2-line', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
        ${feature('leaf-line', 'Berkelanjutan', 'Rutinitas berulang dikerjakan otomatis, tim fokus tumbuh.')}
    </div>
</section>
<section class="lp-section" style="padding-top: 0" id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-proof">
        <div>
            ...
            <p class="tk-muted" style="margin: 0; max-width: 440px">Pengguna aktif naik empat kuartal berturut-turut &mdash; tumbuh pelan, akar yang kuat. Tim Anda bisa jadi bagian berikutnya.</p>
...
<section class="lp-section" style="padding-top: 0" id="cta" aria-labelledby="lp-h-cta">
    <div class="tk-cta">
        ...
        <button class="tk-btn tk-btn-primary">Coba Sekarang ${ic('arrow-right-line', 16)}</button>
    </div>
</section>
</main>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], borderColor: t.primary, backgroundColor: t.primary + '24', fill: true, tension: 0.45, pointRadius: 0, borderWidth: 3 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    .au-wrap { min-height: calc(100vh - 48px); box-sizing: border-box; display: flex; align-items: center; justify-content: center; padding: 40px 16px; background: var(--tk-og-mesh), var(--tk-color-background); }
    .ex-embedded .au-wrap { min-height: 100vh; }
    .au-card { width: 100%; max-width: 400px; }
    .au-brand { font-family: var(--tk-font-heading); font-style: italic; font-weight: 700; font-size: var(--tk-text-h4); text-align: center; margin-bottom: 20px; }
...
<head>
${HEAD_COMMON}
<title>Organic — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Organic', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Organic — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #2E3B33; border-radius: 44px; background: #2E3B33; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 32px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Organic', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const ORGANIC_EXAMPLES: Record<string, string> = {
