...
/**
 * Halaman contoh (`examples/`) kit Kinetic: admin dashboard dan landing page utuh yang
 ...
 *
 * Kit ini satu-tema (terang bawaan — huruf hitam paling hidup di atas kertas), jadi semua
 * halaman memanggil exNav dengan `themeToggle: false`. Hero landing punya trik khas kinetic:
 * bobot dan lebar font judul mengikuti posisi mouse lewat font-variation-settings.
 *
...
function ic(name: string, size = 17): string {
    return `<iconify-icon icon="carbon:${name}" width="${size}" height="${size}"></iconify-icon>`;
}
...
<title>Kinetic — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 900; font-variation-settings: 'wght' 900, 'wdth' 118; font-size: var(--tk-text-h4); text-transform: uppercase; padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-topbar { display: flex; align-items: center; gap: 12px; padding: 12px 24px; background: rgba(244, 242, 236, 0.9); backdrop-filter: blur(12px); border-bottom: 1px solid var(--tk-color-border); position: sticky; top: 48px; z-index: 10; }
    ...
    .dm-avatar { width: 34px; height: 34px; border-radius: var(--tk-radius-full); background: var(--tk-color-primary); color: #FFF; display: inline-flex; align-items: center; justify-content: center; font-family: var(--tk-font-heading); font-size: 12px; font-weight: 800; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Kinetic', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' }, themeToggle: false })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item" href="#">${ic('user-multiple')} <span class="tk-sidebar-label">Pengguna</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('text-font')} <span class="tk-sidebar-label">Spesimen</span></a>
        ...
        <div class="tk-dropdown dm-sidebar-foot">
            ...
            <div class="tk-dropdown-menu tk-dropdown-menu-up">
                ...
                <button class="tk-dropdown-item tk-dropdown-item-danger" type="button">${ic('logout', 15)} Keluar</button>
            </div>
        </div>
    </aside>
    <div class="dm-main">
        <header class="dm-topbar">
            ...
            <div class="dm-topbar-spacer">
                ...
                <button class="tk-action-btn" aria-label="Notifikasi">${ic('notification', 17)}</button>
            </div>
        </header>
        <main class="dm-content">
            <div>
                <h2 class="tk-h3" style="margin: 0; text-transform: uppercase">Dashboard</h2>
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('overflow-menu-horizontal', 15)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('view', 15)} Lihat detail</button><button class="tk-dropdown-item">${ic('edit', 15)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash-can', 15)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        <div class="dm-activity-item">${ic('user', 15)}<span><strong>Jane Smith</strong> bergabung ke tim &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">5 menit lalu</span></span></div>
                        <div class="dm-activity-item">${ic('checkmark-outline', 15)}<span>Laporan Juli disetujui &middot; <span class="tk-caption" style="text-transform: none; letter-spacing: 0">1 jam lalu</span></span></div>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.primary, backgroundColor: t.primary + '1F', fill: true, tension: 0.35, pointRadius: 4, pointBackgroundColor: '#FF5C33', pointBorderColor: t.surface2, pointBorderWidth: 2, borderWidth: 3 }]
        },
...
<style>
    ...
    /* Hero kinetic: judul raksasa yang bobot dan lebar hurufnya mengikuti mouse. */
    .lp-hero { min-height: 78vh; display: flex; flex-direction: column; justify-content: center; }
    .lp-hero .tk-display { font-size: clamp(48px, 9vw, 112px); max-width: 12ch; }
    #lp-kinetic-title { transition: none; }
    ...
    .lp-plan-featured { border-color: var(--tk-color-primary); box-shadow: var(--tk-shadow-lg); }
    .lp-price { font-family: var(--tk-font-heading); font-weight: 800; font-variation-settings: 'wght' 800, 'wdth' 110; font-size: var(--tk-text-h2); margin: 0 0 8px; }
    ...
    .lp-footer .tk-caption { color: rgba(244, 242, 236, 0.6); }
    .lp-footer-links { display: flex; gap: 20px; }
    .lp-footer-links a { color: rgba(244, 242, 236, 0.75); text-decoration: none; font-size: var(--tk-text-body-sm); font-weight: 700; }
    ...
    @media (max-width: 860px) {
        ...
        .lp-hero .tk-display { font-size: 44px; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Kinetic', 'landing', '../', { device: { base: 'landing', active: 'web' }, themeToggle: false })}
...
<section class="tk-hero lp-hero" id="beranda" aria-labelledby="lp-h-hero">
    <p class="tk-kicker">Spesimen 01 &mdash; Gerakkan mouse Anda</p>
    <h1 class="tk-display" id="lp-h-hero"><span id="lp-kinetic-title">Kelola proyek</span> <span class="tk-outline-text tk-outline-text-primary">tanpa</span> drama</h1>
    <p class="tk-hero-sub">Satu tempat untuk merencanakan, melacak, dan menyelesaikan pekerjaan tim Anda &mdash; hurufnya pun ikut bergerak.</p>
...
<div class="tk-type-marquee" aria-hidden="true">
    <div class="tk-type-marquee-track"><span>Rencanakan</span><em>&#9679;</em><span class="tk-outline-text">Lacak</span><em>&#9679;</em><span>Selesaikan</span><em>&#9679;</em><span class="tk-outline-text">Rayakan</span><em>&#9679;</em></div>
    <div class="tk-type-marquee-track" aria-hidden="true"><span>Rencanakan</span><em>&#9679;</em><span class="tk-outline-text">Lacak</span><em>&#9679;</em><span>Selesaikan</span><em>&#9679;</em><span class="tk-outline-text">Rayakan</span><em>&#9679;</em></div>
</div>
<section id="fitur" aria-labelledby="lp-h-fitur">
    <div class="lp-section">
        <p class="tk-kicker tk-kinetic-line">Spesimen 02 &mdash; Fitur</p>
        <h2 class="tk-h1 lp-section-title tk-kinetic-line" id="lp-h-fitur">Semua yang tim Anda butuhkan</h2>
        <p class="lp-section-sub tk-kinetic-line tk-kinetic-line-alt">Fitur yang benar-benar dipakai, bukan daftar panjang yang membingungkan.</p>
        <div class="tk-feature-grid lp-features">
            ${feature('lightning', 'Cepat', 'Halaman dan pencarian merespons dalam hitungan milidetik.')}
            ${feature('security', 'Aman', 'Enkripsi menyeluruh dan kontrol akses per peran.')}
            ${feature('chart-bar', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
            ${feature('user-multiple', 'Kolaboratif', 'Komentar, mention, dan tugas dalam satu alur.')}
            ${feature('plug', 'Terintegrasi', 'Terhubung dengan alat yang sudah tim Anda pakai.')}
            ${feature('text-font', 'Ekspresif', 'Setiap judul adalah huruf yang hidup dan bergerak.')}
        </div>
    </div>
</section>
<section id="bukti" aria-labelledby="lp-h-bukti">
    <div class="lp-section" style="padding-top: 0">
        <div class="lp-proof">
            <div>
                <p class="tk-kicker tk-kinetic-line">Spesimen 03 &mdash; Bukti</p>
                <h2 class="tk-h1 tk-kinetic-line" style="margin: 0 0 8px" id="lp-h-bukti">Tumbuh bersama ribuan tim</h2>
                <p class="tk-muted tk-kinetic-line tk-kinetic-line-alt" style="margin: 0; max-width: 440px">Pengguna aktif naik empat kuartal berturut-turut. Angkanya bicara sendiri &mdash; dan tim Anda bisa jadi bagian berikutnya.</p>
                <div class="lp-proof-stats tk-kinetic-line">
            ...
            <div class="tk-card tk-kinetic-line tk-kinetic-line-alt"><div class="tk-card-body">
...
<section id="harga" aria-labelledby="lp-h-harga">
    <div class="lp-section" style="padding-top: 0">
        <p class="tk-kicker tk-kinetic-line">Spesimen 04 &mdash; Harga</p>
        <h2 class="tk-h1 lp-section-title tk-kinetic-line" id="lp-h-harga">Harga yang sederhana</h2>
        <p class="lp-section-sub tk-kinetic-line tk-kinetic-line-alt">Mulai gratis, naikkan paket saat tim Anda tumbuh.</p>
...
<section id="cta" aria-labelledby="lp-h-cta">
    <div class="lp-section" style="padding-top: 0">
        <div class="tk-cta tk-kinetic-line">
            ...
            <button class="tk-btn tk-btn-primary">Coba Sekarang ${ic('arrow-right', 15)}</button>
        </div>
    </div>
</section>
</main>
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: ['#C9C5B6', '#111013', '#FF5C33', t.primary], borderRadius: 4, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
// Tanda tangan kinetic: bobot & lebar huruf judul hero mengikuti posisi mouse.
// Variabel font Archivo dianimasikan langsung — tanpa library, cukup satu listener.
(function () {
    var title = document.getElementById('lp-kinetic-title');
    ...
    hero.addEventListener('mousemove', function (e) {
        ...
        var wght = Math.round(300 + x * 600);
        var wdth = Math.round(75 + y * 50);
        title.style.fontVariationSettings = "'wght' " + wght + ", 'wdth' " + wdth;
    });
    hero.addEventListener('mouseleave', function () { title.style.fontVariationSettings = ''; });
})();
</script>
</body>
</html>
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 900; font-variation-settings: 'wght' 900, 'wdth' 118; font-size: var(--tk-text-h4); text-transform: uppercase; text-align: center; margin-bottom: 20px; }
...
<head>
${HEAD_COMMON}
<title>Kinetic — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Kinetic', 'components', '../', { device: { base, active: 'web' }, themeToggle: false })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Kinetic — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
    ...
    .ex-device { width: 412px; max-width: 100%; height: 780px; max-height: calc(100vh - 150px); min-height: 420px; border: 12px solid #111013; border-radius: 48px; background: #111013; box-shadow: var(--tk-shadow-lg); overflow: hidden; flex: none; }
    .ex-device iframe { width: 100%; height: 100%; border: none; border-radius: 36px; background: var(--tk-color-background); }
</style>
</head>
<body>
${exNav('Kinetic', view, '../', { device: { base, active: 'mobile' }, themeToggle: false })}
<div class="ex-stage">
...
export const KINETIC_EXAMPLES: Record<string, string> = {
