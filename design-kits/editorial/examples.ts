...
/**
 * Halaman contoh (`examples/`) kit Editorial: admin dashboard dan landing page utuh yang
...
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,500;0,600;0,700;1,500&family=Libre+Franklin:wght@400;500;600;700&family=Spline+Sans+Mono:wght@400;600&display=swap" rel="stylesheet">
...
<title>Editorial — Contoh Admin Dashboard</title>
<style>
    ...
    .dm-sidebar-brand { font-family: var(--tk-font-heading); font-weight: 600; font-size: var(--tk-text-title); padding: 8px 16px 20px; white-space: nowrap; }
    ...
    .dm-avatar { width: 32px; height: 32px; border-radius: var(--tk-radius-full); background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); display: inline-flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 600; flex: none; }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Editorial', 'dashboard', '../', { device: { base: 'dashboard', active: 'web' } })}
<div class="dm-shell">
    <aside class="tk-sidebar dm-sidebar">
        ...
        <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${ic('layout-dashboard')} <span class="tk-sidebar-label">Dashboard</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('bar-chart-3')} <span class="tk-sidebar-label">Laporan</span></a>
        <a class="tk-sidebar-item" href="#">${ic('line-chart')} <span class="tk-sidebar-label">Statistik</span></a>
        ...
        <a class="tk-sidebar-item" href="#">${ic('life-buoy')} <span class="tk-sidebar-label">Bantuan</span></a>
        <div class="tk-dropdown dm-sidebar-foot">
            <button class="dm-foot-btn" type="button" aria-label="Menu profil">
                ...
                <span class="dm-foot-chevron tk-sidebar-label">${ic('chevrons-up-down', 16)}</span>
            </button>
    ...
    <div class="dm-main">
        ...
        <main class="dm-content">
            ...
            <div class="tk-filter-bar">
                ...
                <button class="tk-btn tk-btn-outline">${ic('sliders-horizontal', 16)} Filter</button>
            </div>
            <div class="tk-card">
                ...
                <div class="tk-card-body dm-table-wrap">
                    <table class="tk-table">
                        ...
                        <tbody>
                            ...
                            <tr><td>Maya Putri</td><td>maya@contoh.com</td><td>Manajer</td><td><span class="tk-badge tk-badge-danger">Nonaktif</span></td><td class="tk-table-actions"><div class="tk-dropdown"><button class="tk-action-btn" aria-label="Menu aksi">${ic('ellipsis', 16)}</button><div class="tk-dropdown-menu tk-dropdown-menu-right"><button class="tk-dropdown-item">${ic('eye', 16)} Lihat detail</button><button class="tk-dropdown-item">${ic('pencil', 16)} Edit</button><hr class="tk-dropdown-divider"><button class="tk-dropdown-item tk-dropdown-item-danger">${ic('trash-2', 16)} Hapus</button></div></div></td></tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="dm-grid2">
                ...
                <div class="tk-card">
                    ...
                    <div class="tk-card-body dm-activity">
                        ...
                        <div class="dm-activity-item">${ic('file-check', 16)}<span>Laporan Juli disetujui &middot; <span class="tk-caption">1 jam lalu</span></span></div>
                        <div class="dm-activity-item">${ic('alert-triangle', 16)}<span>Kuota penyimpanan hampir penuh &middot; <span class="tk-caption">3 jam lalu</span></span></div>
                    </div>
                </div>
            </div>
        </main>
...
tkChart('dm-chart-revenue', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pendapatan (jt)', data: [52, 61, 58, 71, 78, 84], borderColor: t.accent, backgroundColor: t.accent + '26', fill: true, tension: 0.35, pointRadius: 3, pointBackgroundColor: t.accent }]
        },
...
<style>
    ...
    ${EX_NAV_CSS}
    .lp-skip { position: absolute; left: -9999px; top: 0; z-index: 30; background: var(--tk-color-primary); color: var(--tk-color-primary-contrast); padding: 8px 16px; border-radius: var(--tk-radius-md); text-decoration: none; font-size: var(--tk-text-body-sm); }
    ...
    .lp-proof-stat .tk-stat-value { display: block; }
    ...
    .lp-price { font-size: var(--tk-text-h2); font-weight: 600; letter-spacing: -0.02em; margin: 0 0 8px; }
    ...
    .lp-footer { border-top: 1px solid var(--tk-color-border); }
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Editorial', 'landing', '../', { device: { base: 'landing', active: 'web' } })}
...
<section class="tk-hero" id="beranda" aria-labelledby="lp-h-hero">
    <span class="tk-badge tk-badge-info" style="margin-bottom: 16px">${ic('sparkles', 14)} Baru &middot; versi 2.0 sudah rilis</span>
...
<section class="lp-section" id="fitur" aria-labelledby="lp-h-fitur">
    ...
    <div class="tk-feature-grid lp-features">
        ...
        ${feature('bar-chart-3', 'Terukur', 'Laporan kemajuan otomatis untuk setiap proyek.')}
...
tkChart('lp-chart-growth', function (t) {
    return {
        ...
        data: {
            ...
            datasets: [{ label: 'Pengguna aktif (ribu)', data: [14, 19, 27, 36], backgroundColor: t.accent, borderRadius: 6, maxBarThickness: 56 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    };
});
...
export const AUTH_CSS = `    /* Perancah halaman auth (au-*): bukan bagian kontrak kit. */
    ...
    .au-brand { font-family: var(--tk-font-heading); font-weight: 600; font-size: var(--tk-text-title); text-align: center; margin-bottom: 20px; }
...
<head>
${HEAD_COMMON}
<title>Editorial — Contoh ${title}</title>
<style>
    ${EX_NAV_CSS}
${AUTH_CSS}
</style>
...
<body>
${EX_EMBED_SCRIPT}
${exNav('Editorial', 'components', '../', { device: { base, active: 'web' } })}
<div class="au-wrap">
...
<head>
${HEAD_COMMON}
<title>Editorial — ${title} (Ponsel)</title>
<style>
    ${EX_NAV_CSS}
...
<body>
${exNav('Editorial', view, '../', { device: { base, active: 'mobile' }, extraThemeJs: FRAME_THEME_EXTRA_JS })}
<div class="ex-stage">
...
export const EDITORIAL_EXAMPLES: Record<string, string> = {
