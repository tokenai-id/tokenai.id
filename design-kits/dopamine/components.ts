...
/**
 * Fragmen `components/` kit Dopamine Design: markup murni dengan kelas kontrak, teks placeholder
 * netral, ikon Iconify (set mynaui — garis membulat yang ramah dan cocok dengan bentuk gemuk kit
 * ini). Gayanya seluruhnya di `styles.css` — fragmen inilah yang disalin agen saat merakit
...
export const DOPAMINE_COMPONENTS: Record<string, string> = {
...
<div class="tk-field">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih satu opsi</span> ${icon('chevron-down', 15)}</button>
...
`,
    ...
    <button class="tk-select-trigger" type="button">Opsi pertama ${icon('chevron-down', 15)}</button>
    <div class="tk-select-menu">
...
<div class="tk-select">
    <button class="tk-select-trigger" type="button"><span class="tk-select-placeholder">Pilih anggota</span> ${icon('chevron-down', 15)}</button>
    <div class="tk-select-menu">
...
<div class="tk-select">
    <button class="tk-select-trigger" type="button">Peran &middot; 2 dipilih ${icon('chevron-down', 15)}</button>
    <div class="tk-select-menu">
...
`,
    ...
    <button class="tk-btn tk-btn-outline">Aksi ${icon('chevron-down', 15)}</button>
    <div class="tk-dropdown-menu">
        ...
        <button class="tk-dropdown-item">${icon('edit-one', 16)} Edit</button>
        <button class="tk-dropdown-item">${icon('copy', 16)} Duplikat</button>
        <hr class="tk-dropdown-divider">
        <button class="tk-dropdown-item tk-dropdown-item-danger">${icon('trash', 16)} Hapus</button>
    </div>
</div>
`,
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua status ${icon('chevron-down', 15)}</button>
        <div class="tk-select-menu">
    ...
    <div class="tk-select">
        <button class="tk-select-trigger" type="button">Semua peran ${icon('chevron-down', 15)}</button>
        <div class="tk-select-menu">
            <label class="tk-option"><input class="tk-checkbox" type="checkbox" checked> Designer</label>
            <label class="tk-option"><input class="tk-checkbox" type="checkbox"> Developer</label>
        </div>
    </div>
    <button class="tk-btn tk-btn-outline">${icon('filter', 16)} Filter <span class="tk-filter-count">2</span></button>
</div>
...
<div class="tk-card tk-card-stat tk-pop-pink">
...
<nav class="tk-breadcrumb">
    <a href="#">Beranda</a>
    <span class="tk-breadcrumb-sep">${icon('chevron-right', 13)}</span>
    <a href="#">Proyek</a>
    <span class="tk-breadcrumb-sep">${icon('chevron-right', 13)}</span>
...
<aside class="tk-sidebar">
    ...
    <a class="tk-sidebar-item tk-sidebar-item-active" href="#">${icon('home', 18)} <span class="tk-sidebar-label">Dashboard</span></a>
    ...
    <a class="tk-sidebar-item" href="#">${icon('chart-bar', 18)} <span class="tk-sidebar-label">Laporan</span></a>
    <span class="tk-sidebar-group">Lainnya</span>
    <a class="tk-sidebar-item" href="#">${icon('cog', 18)} <span class="tk-sidebar-label">Pengaturan</span></a>
</aside>
`,
...
`,
    'components/alert.html': `<div class="tk-alert tk-alert-success">${icon('check-circle', 18)}<span><span class="tk-alert-title">Berhasil!</span> Perubahan Anda sudah tersimpan.</span></div>
<div class="tk-alert tk-alert-warning">${icon('danger-triangle', 18)}<span><span class="tk-alert-title">Perhatian.</span> Periksa kembali informasi Anda.</span></div>
<div class="tk-alert tk-alert-danger">${icon('x', 18)}<span><span class="tk-alert-title">Gagal.</span> Terjadi kesalahan, coba lagi.</span></div>
<div class="tk-alert tk-alert-info">${icon('info-circle', 18)}<span><span class="tk-alert-title">Info.</span> Ada pembaruan penting untuk Anda.</span></div>
`,
    ...
    <tbody>
        <tr>
            ...
            <td class="tk-table-actions">
                ...
                <button class="tk-action-btn" aria-label="Edit">${icon('edit-one', 16)}</button>
                <button class="tk-action-btn tk-action-btn-danger" aria-label="Hapus">${icon('trash', 16)}</button>
            </td>
        </tr>
        <tr>
            ...
            <td class="tk-table-actions">
                <div class="tk-dropdown">
                    <button class="tk-action-btn" aria-label="Menu aksi">${icon('dots', 16)}</button>
...
`,
    ...
    <span class="tk-empty-icon tk-bounce">${icon('confetti', 32)}</span>
...
`,
    ...
    <p class="tk-sticker">${icon('sparkles', 15)} Baru rilis!</p>
    <h1 class="tk-display">Bikin harimu <span class="tk-highlight">seru</span></h1>
...
<div class="tk-feature-grid">
    <div class="tk-feature">
        <span class="tk-feature-icon">${icon('lightning', 26)}</span>
