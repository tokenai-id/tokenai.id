...
export const BOLD_MINIMAL_STYLES = `/* Bold Minimal — kit design TOKENAI.
   Estetika bold minimalism: strukturnya tetap minimal — putih lapang, hairline, sedikit
   elemen — tetapi satu-dua elemen dibiarkan sangat dominan: headline raksasa weight 900
   dengan tracking rapat, dan satu biru elektrik sebagai satu-satunya warna kuat.
   Kontrak: docs/kontrak-kit-design.md. Semua nilai design tinggal di :root. */

/* == tk: tokens ================================================= */
:root {
    --tk-color-primary: #2B2BFF;
    --tk-color-primary-hover: #1A1AE6;
    --tk-color-primary-contrast: #FFFFFF;
    --tk-color-secondary: #0A0A0A;
    --tk-color-secondary-hover: #000000;
    --tk-color-secondary-contrast: #FFFFFF;
    --tk-color-accent: #1A1AE6;
    --tk-color-background: #FFFFFF;
    --tk-color-surface: #FFFFFF;
    --tk-color-surface-2: #F4F4F5;
    --tk-color-text: #0A0A0A;
    --tk-color-text-muted: #71717A;
    --tk-color-border: #E4E4E7;
    ...
    --tk-color-info: #2B2BFF;
    --tk-color-info-soft: #EEEEFF;
    ...
    --tk-weight-heading: 900;
    --tk-weight-body: 400;
    --tk-leading-heading: 1.04;
    --tk-leading-body: 1.65;

    /* Headline adalah elemen dominan kit ini: display raksasa, tracking rapat. */
    --tk-text-display: 88px;
    --tk-text-h1: 56px;
    --tk-text-h2: 38px;
    --tk-text-h3: 26px;
    --tk-text-h4: 19px;
    ...
    /* Hairline; kedalaman nyaris tanpa bayangan — kontrasnya dari warna, bukan bayangan. */
    --tk-border-width: 1px;
    --tk-shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.04);
    --tk-shadow: 0 2px 6px rgba(0, 0, 0, 0.06);
    --tk-shadow-lg: 0 16px 40px rgba(0, 0, 0, 0.12);

    --tk-transition: 160ms ease;
    --tk-container: 1200px;

    /* Token khas bold-minimal (bukan kontrak): garis lebih tegas untuk kontrol form. */
    --tk-bm-line-strong: #A1A1AA;
}

/* Mode gelap: hitam pekat dengan biru elektrik yang lebih menyala; tombol sekunder menjadi
   putih. Aktifkan dengan atribut data-tk-theme="dark" pada <html> atau <body>. */
[data-tk-theme="dark"] {
    --tk-color-primary: #5252FF;
    --tk-color-primary-hover: #6B6BFF;
    --tk-color-primary-contrast: #FFFFFF;
    --tk-color-secondary: #FAFAFA;
    --tk-color-secondary-hover: #FFFFFF;
    --tk-color-secondary-contrast: #0A0A0A;
    --tk-color-accent: #8080FF;
    --tk-color-background: #050505;
    --tk-color-surface: #0F0F10;
    --tk-color-surface-2: #18181B;
    --tk-color-text: #FAFAFA;
    --tk-color-text-muted: #8F8F98;
    --tk-color-border: #26262B;
    ...
    --tk-color-info: #7F7FFF;
    --tk-color-info-soft: #14142E;
    --tk-shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.5);
    --tk-shadow: 0 2px 6px rgba(0, 0, 0, 0.6);
    --tk-shadow-lg: 0 16px 40px rgba(0, 0, 0, 0.7);
    --tk-bm-line-strong: #4A4A52;
}
...
.tk-display { font-size: var(--tk-text-display); letter-spacing: -0.03em; line-height: 0.98; }
.tk-h1 { font-size: var(--tk-text-h1); letter-spacing: -0.025em; }
.tk-h2 { font-size: var(--tk-text-h2); letter-spacing: -0.02em; }
.tk-h3 { font-size: var(--tk-text-h3); letter-spacing: -0.01em; }
.tk-h4 { font-size: var(--tk-text-h4); font-weight: 700; }
...
.tk-link { color: var(--tk-color-primary); font-weight: 700; text-decoration: none; }
.tk-link:hover { text-decoration: underline; text-decoration-thickness: 3px; text-underline-offset: 4px; }
...
/* == tk: button ================================================= */
/* Tombol pil tegas: dua warna dominan kit — biru elektrik dan hitam pekat. */
.tk-btn {
    display: inline-flex;
    align-items: center;
    ...
    cursor: pointer;
    transition: background var(--tk-transition), color var(--tk-transition), border-color var(--tk-transition), transform var(--tk-transition), opacity var(--tk-transition);
    text-decoration: none;
    white-space: nowrap;
}
.tk-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.tk-btn:hover:not(:disabled) { transform: translateY(-1px); }
.tk-btn:not(:disabled):active { transform: translateY(0); opacity: 0.85; }
...
.tk-btn-text:hover:not(:disabled) { transform: none; text-decoration: underline; text-decoration-thickness: 3px; text-underline-offset: 4px; }
...
.tk-input, .tk-textarea, .tk-select-trigger {
    ...
    border: 1px solid var(--tk-bm-line-strong);
...
.tk-input:focus, .tk-textarea:focus {
    ...
    box-shadow: 0 0 0 2px var(--tk-color-primary);
}
...
.tk-field-error .tk-input, .tk-field-error .tk-select-trigger, .tk-field-error .tk-textarea {
    ...
    box-shadow: 0 0 0 2px var(--tk-color-danger);
}
.tk-field-error .tk-help { color: var(--tk-color-danger); }
.tk-field-success .tk-input { border-color: var(--tk-color-success); box-shadow: 0 0 0 2px var(--tk-color-success); }
...
.tk-checkbox, .tk-radio {
    ...
    position: relative;
    background: var(--tk-color-surface);
    border: 1.5px solid var(--tk-bm-line-strong);
    cursor: pointer;
...
.tk-toggle { position: relative; width: 42px; height: 24px; flex: none; margin: 0; appearance: none; border: 1.5px solid var(--tk-bm-line-strong); background: var(--tk-color-surface-2); border-radius: var(--tk-radius-full); cursor: pointer; transition: background var(--tk-transition), border-color var(--tk-transition); }
...
.tk-card-stat .tk-stat-value { font-size: var(--tk-text-h2); font-family: var(--tk-font-heading); font-weight: 900; letter-spacing: -0.02em; }
...
.tk-navbar-brand { font-family: var(--tk-font-heading); font-weight: 900; font-size: 19px; letter-spacing: -0.02em; margin-right: auto; }
...
.tk-navbar-dark { background: #050505; border-bottom-color: #26262B; }
.tk-navbar-dark .tk-navbar-brand { color: #FAFAFA; }
.tk-navbar-dark .tk-navbar-link { color: #FAFAFA; opacity: 0.65; }
.tk-navbar-dark .tk-navbar-link:hover, .tk-navbar-dark .tk-navbar-link-active { opacity: 1; color: #FAFAFA; }
.tk-sidebar {
    width: 240px;
    background: var(--tk-color-surface);
...
.tk-empty {
    ...
    text-align: center;
    border: 1px dashed var(--tk-bm-line-strong);
    border-radius: var(--tk-radius-lg);
    background: var(--tk-color-surface);
}
...
.tk-hero-sub { font-size: var(--tk-text-body-lg); color: var(--tk-color-text-muted); max-width: 540px; margin: 0 auto var(--tk-space-xl); }
...
/* Ikon fitur = kotak hitam pekat dengan ikon putih — blok kontras kecil yang tegas. */
.tk-feature-icon {
    width: 44px;
    height: 44px;
    display: flex;
...
/* CTA = blok biru elektrik raksasa: elemen paling dominan setelah headline. */
.tk-cta {
    background: var(--tk-color-primary);
    color: var(--tk-color-primary-contrast);
    border: none;
...
/* == tk: bold-minimal flavor ==================================== */
/* Penimpa karakter bold-minimal di atas struktur kontrak: menang kaskade karena berada di
   ekor berkas. Strukturnya tetap hening — flavor-nya hanya menajamkan dua elemen dominan:
   headline yang benar-benar raksasa dan biru elektrik yang tampil tanpa kompromi. */
/* Headline hero menyusut anggun di layar sempit, tetap dominan di layar lebar. */
.tk-hero .tk-display {
    font-size: clamp(48px, 8vw, var(--tk-text-display));
    max-width: 820px;
    margin-left: auto;
    margin-right: auto;
}
/* Foto: polos dengan sudut membulat besar — biarkan gambarnya sendiri yang dominan. */
